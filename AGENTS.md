...
- Finalize Manifest evidence is verified only from actual stored bytes. Migration-inferred evidence is `unverified`; an unverified authoritative completed-attempt Manifest blocks publication and controlled media delivery and may become `verified` only through an audited storage revalidation that records observed MIME, byte size and time. Superseded attempts remain historical evidence and cannot authorize or poison a later fenced attempt.
