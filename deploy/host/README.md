...
The runtime workflow requires the exact release commit, index digest, successful release-workflow evidence run ID and a fresh 128-bit lowercase Runner nonce. GitHub routes it only to a Runner carrying all of `self-hosted`, `linux`, `x64`, `cwt-tencent-singapore`, `cwt-single-use` and the unique `cwt-job-<nonce>` label; those cumulative labels cannot fall back to a GitHub-hosted Runner. After the separate provisioning payload succeeds, submit the exact reviewed `deploy/runtime-validation/register-and-start-ephemeral-runner.sh` bytes through the one authorized registration TAT invocation, whose exact non-secret execution settings are recorded in `deploy/runtime-validation/tencent-tat-runner-registration-invocation.v1.json`. The invocation must use TAT `RunCommand` with `username` exactly `ubuntu`; the payload independently refuses root, a different user or a mismatched UID before registration or start. Supply `CWT_REGISTRATION_TOKEN`, `CWT_RUNNER_NONCE`, `CWT_RUNNER_NAME` and `CWT_GITHUB_REPOSITORY` only through the already accepted one-time environment-input boundary. Do not reconstruct `config.sh` or `run.sh` commands, insert `sudo` or another wrapper, or put the token in command text, a file, a pipe, retained output or another custody mechanism. The payload invokes the official `config.sh` child directly, registers exactly one `--ephemeral --unattended --replace` Runner named `cwt-tencent-sg-<nonce>`, clears the short-lived token before its single direct `run.sh` start, and creates no token file, retry, second startup path or lifecycle authority. Token acquisition and the one accepted TAT transmission remain external operator actions; cloud create/delete, workflow dispatch and Runtime validation remain outside the payload. The VM must be destroyed after the job. The repository verifies the name, first-attempt manual event, OS/architecture and clean exact checkout before GHCR credentials or subject access; it does not provision, reuse or destroy the VM and does not claim that teardown occurred.

The runtime job downloads only the named detached evidence artifact, authenticates using its runtime-injected GitHub token, and materializes the read-only OCI evidence layout from the exact private GHCR digest. It then invokes the sole `preflight-linux-runtime.mjs` authority, whose Docker Engine remains fixed to `/var/run/docker.sock` and which pulls the runtime subject only as `ghcr.io/<repository>@sha256:<index>`. Missing or mismatched release/digest/Runner/tool/profile/Registry facts produce `NOT_PASS`; there is no automatic retry, classifier, revocation, lifecycle transition or promotion.
