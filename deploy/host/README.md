...
Use two canonical, clean sibling checkouts: `runtime-tools` at the reviewed `workflow_commit` (which must equal `GITHUB_SHA`) and `release-subject` at the frozen `release_commit`. Execute all host-side project scripts by absolute paths under `runtime-tools`. The Runtime entry derives its tools root from its executable and requires an explicit subject `--repository`; neither cwd nor a caller profile is an authority. Both paths must be exact Git top-level roots at their pinned commits, with no symlink, nesting or dirty source. Root-owned checkouts work directly; Runner-owned checkouts require root execution with the exact existing `SUDO_UID` owner bridge for both roots. Do not widen `safe.directory`, recursively change checkout ownership or overlay newer source files onto the Release.

Deterministic source, fixed tools profile and Release-header agreement checks run before Docker credential/host work or evidence-directory creation. Keep the detached Release record, OCI layout, credential directory/file and absent output directory outside both source roots; use an existing canonical parent for output. The existing Runtime outcome records the validated tools commit and fixed profile SHA-256 separately from the unchanged Build Once record’s `tools` field. Full image/evidence/lifecycle verification remains at the sole Runtime authority.

The tools checkout owns host-side validators, imports, compatibility policy and synthetic setup. The frozen subject owns `compose.yaml`, Compose project directory and relative configuration binds, proxy ranges and read-only `/app/drizzle` SQL/metadata. The exact immutable image owns the application, bundle checker and `/app/scripts/migrate.ts` runner. New Compose, SQL or pending backup work must never be copied into the emitted Release to pass validation.
...
`deploy/runtime-validation/provision-ubuntu-amd64-runner.sh` is the sole reviewed pre-registration setup payload for a future separately authorized Tencent Automation Assistant execution. Submit the exact reviewed file bytes unchanged as one zero-argument Bash command; do not reconstruct, prefix, append or replace any part of the payload in the console. The current operator invocation parameters are recorded in `deploy/runtime-validation/tencent-tat-provisioning-invocation.v1.json`: select an exact command timeout of `600` seconds. The Tencent default `60`-second timeout is forbidden for this payload and cannot authorize registration.

The payload accepts only a fresh Ubuntu 24.04 native `amd64` host and installs the exact reviewed Docker Engine `29.6.2`, Compose `5.3.1` and Actions Runner `2.337.0` identities. It fully captures Docker's signed Noble package catalog before built-in parsing, requires exactly one match for each complete package version, verifies the Runner archive SHA-256, converges the complete Runner tree to `ubuntu:ubuntu`, and performs actual `ubuntu` `_diag` create/check/remove and Docker access probes. Verbose installation output is kept only in a root-created mode-`0600` ephemeral local log: success removes the log before emitting one concise marker; failure returns the original nonzero code, emits at most a `4096`-byte sanitized diagnostic tail, and removes the log.

Registration is permitted only when Tencent TAT reports terminal `SUCCESS` status and exact process exit code `0` for that one `600`-second invocation. Missing exit code, timeout, cancellation, terminal failure or any nonzero exit forbids registration. `CWT_PRE_REGISTRATION_OK ...` may corroborate a success, but the marker is corroboration only: partial output or the marker alone never authorizes registration, and marker absence caused solely by TAT retained-output truncation does not contradict an independently reported terminal `SUCCESS` plus exit `0`.

The payload contains no credential input, Runner registration, Runner launch, workflow dispatch, Registry login, cloud provisioning/destruction, retry, fallback, DIND or SSH path. Do not add custom environment values to this invocation; no registration token, Runtime credential or other secret may exist during setup or enter its ephemeral log. Registration-token acquisition/transmission, the one ephemeral registration, workflow dispatch and mandatory teardown remain separate Owner-confirmed operator actions. A setup failure is terminal for that VM execution and does not authorize editing or rerunning the payload in place.
...
The runtime workflow requires the exact reviewed workflow/tools commit, frozen release commit, index digest, successful release-workflow evidence run ID and a fresh 128-bit lowercase Runner nonce. GitHub routes it only to a Runner carrying all of `self-hosted`, `linux`, `x64`, `cwt-tencent-singapore`, `cwt-single-use` and the unique `cwt-job-<nonce>` label; those cumulative labels cannot fall back to a GitHub-hosted Runner. After the separate provisioning payload succeeds, use only the versioned registration TAT invocation in `deploy/runtime-validation/tencent-tat-runner-registration-invocation.v1.json`: submit its exact Base64 `content` value as TAT `RunCommand` `Content`, keep parameter substitution enabled, and retain `username=ubuntu`, `workingDirectory=/home/ubuntu`, timeout `600`, `saveCommand=false` and COS output disabled. Supply `CWT_REGISTRATION_TOKEN`, `CWT_RUNNER_NONCE`, `CWT_RUNNER_NAME` and `CWT_GITHUB_REPOSITORY` only as that invocation's four one-time parameter values; `CWT_REGISTRATION_TOKEN` must use the embedded `tat-hidden` placeholder and the other three placeholders remain non-secret. The embedded wrapper is the sole source-materialization path: it creates one restricted temporary file, decodes the reviewed snapshot once, verifies its exact 3,752-byte size and SHA-256, executes it once with those four environment inputs, and removes it through its single cleanup trap. Any materialization, identity or execution failure is terminal for the attempt. Do not reconstruct or modify the wrapper, fetch registration source over a network, add another content credential or source, reconstruct `config.sh` or `run.sh` commands, insert `sudo` or another wrapper, or put the token in command text, a file, a pipe, retained output or another custody mechanism. The payload invokes the official `config.sh` child directly, registers exactly one `--ephemeral --unattended --replace` Runner named `cwt-tencent-sg-<nonce>`, clears the short-lived token before its single direct `run.sh` start, and creates no token file, retry, second startup path or lifecycle authority. Token acquisition and the one accepted TAT transmission remain external operator actions; cloud create/delete, workflow dispatch and Runtime validation remain outside the payload. The VM must be destroyed after the job. The repository verifies the name, first-attempt manual event, OS/architecture and both clean exact checkouts before GHCR credentials or subject access; it does not provision, reuse or destroy the VM and does not claim that teardown occurred.

Before any separately authorized resource creation, retrieve the original explicit Build Once run/artifact, verify its run/head/name/ID join and ZIP hash, and reconcile the proposed Release/index tuple with its unchanged `release.json`. Use the reviewed tools entry locally:

```text
/absolute/pinned/node /work/runtime-tools/deploy/scripts/release-registry-integration.mjs check-release \
  --release /protected/release-evidence/release.json \
  --release-id <exact-frozen-release-commit> \
  --index-digest sha256:<exact-original-index>
```

`check-release` accepts only those three inputs, reads no Registry credential, performs no network operation and writes no state. A refusal retains `release_identity_mismatch` with only literal `mismatchFields`: `input.releaseId`, `input.indexDigest`, `releaseId`, `source.commit`, `oci.indexDigest`, `state`. It never outputs arbitrary keys or expected/actual values. Header agreement does not establish full image validity, artifact availability or external authorization.

The runtime job downloads only the named detached evidence artifact, runs this same header check before installing ORAS or logging in, authenticates using its runtime-injected GitHub token, and materializes the read-only OCI evidence layout from the exact private GHCR digest. Materialization remains transport-only. The job then invokes the sole `preflight-linux-runtime.mjs` authority, whose Docker Engine remains fixed to `/var/run/docker.sock` and which pulls the runtime subject only as `ghcr.io/<repository>@sha256:<index>`. Missing or mismatched release/digest/Runner/tool/profile/Registry facts produce `NOT_PASS`; there is no automatic retry, classifier, revocation, lifecycle transition or promotion.

External prerequisites remain: protected GitHub Environments, exact Runner labels, a CWT-controlled trusted Build Once Runner, private GHCR package access/retention/immutability/audit and protected complete replica, one Tencent Cloud Singapore single-use Ubuntu 24.04 native `linux/amd64` VM with the accepted host Docker/Compose profile, runtime credentials, and independent destruction evidence. None is proven by repository implementation or Local/Synthetic tests.
