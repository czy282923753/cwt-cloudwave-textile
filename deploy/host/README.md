...
`deploy/scripts/preflight-release-compose.mjs` is the one versioned local/Synthetic post-emission validator. It is not a protected-host activation path. It receives one disposable Docker daemon endpoint whose containerd socket/root plus image namespace is exclusive to the run. The validator reads the owner and outer store tuples itself and refuses outer `moby`, a shared tuple, an ambient Docker selector or a tag-only Compose image.

Before a successor Build Once, run its `self-test` mode against a disposable owner and an existing small non-CWT image. The self-test must prove the owner-local tag is invisible and non-deletable through the explicit outer endpoint, the digest-qualified base resolves through the owner, and all temporary references, helper/controller processes, mounts and owner roots are absent afterward. This custody test does not load or run a CWT subject.
