...
`deploy/scripts/preflight-release-compose.mjs` is the one versioned local/Synthetic post-emission validator. It is not a protected-host activation path. Given one explicit `--outer-host` Unix endpoint and one safe run token, the validator creates and owns exactly one ephemeral private DIND controller from `docker:29.6.2-dind@sha256:bfec1f5159c63a81ca6fdedbd81404d2c0e16378ed0feec3bb3fbf3998847659`. The controller is privileged but joins outer `--network none`, publishes no port, mounts no host Docker/containerd socket and uses three run-exact volumes for its Unix API, Docker data and containerd data. It is never started with `--rm`, so first-run inspect/log evidence survives until exact cleanup.

All owner helpers mount only the run-unique API volume and address `unix:///run/cwt-owner-api/docker.sock`; there is no external `--owner-host`, TCP listener, host-network `nsenter`, context fallback or compatibility path. The controller owns private dockerd/containerd/data and ordinary inner networking. Conventional `moby` / `plugins.moby` namespaces are accepted only when the reported containerd address is under the exact run-unique DIND exec root and differs from outer containerd. Repository, workspace, `/etc/cwt`, `/srv/cwt` and `/run/systemd/journal` are projected at the same paths needed by root Compose.

Before a successor Build Once, seed and verify the exact DIND index once under explicit authority, then retain that pinned base image locally. Run `self-test` exactly once with explicit `--outer-host`, run-unique `--token` and an absent absolute `--evidence` directory. The self-test imports only local `alpine:3.22` through the owner, proves Unix API access, private store/outer invisibility, one internal Compose network, a same-path bind, journald, zero published ports and inner communication. It then captures timestamped DIND logs plus their SHA-256 before controller removal and proves zero exact controller, volumes, socket, Compose and Synthetic residue. It does not load or run a CWT subject and must not be rerun after failure without new coordinator authority.
