...
The runtime workflow requires the exact release commit, index digest, successful release-workflow evidence run ID and a fresh 128-bit lowercase Runner nonce. GitHub routes it only to a Runner carrying all of `self-hosted`, `linux`, `x64`, `cwt-tencent-singapore`, `cwt-single-use` and the unique `cwt-job-<nonce>` label; those cumulative labels cannot fall back to a GitHub-hosted Runner. The external provisioner must register exactly one Runner named `cwt-tencent-sg-<nonce>` and destroy that VM after the job. The repository verifies the name, first-attempt manual event, OS/architecture and clean exact checkout before GHCR credentials or subject access; it does not provision, register, reuse or destroy the VM and does not claim that teardown occurred.
