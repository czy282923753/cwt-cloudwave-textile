  ...
  return (
    <div className="prose-cwt grid gap-7">
      {document.blocks.map((block) => {
        switch (block.type) {
          case "heading": {
            const className = "font-semibold tracking-[-0.025em] text-[#143a34]";
            if (block.level === 2) return <h2 className={`${className} text-3xl`} key={block.id}>{block.text}</h2>;
            if (block.level === 3) return <h3 className={`${className} text-2xl`} key={block.id}>{block.text}</h3>;
            return <h4 className={`${className} text-xl`} key={block.id}>{block.text}</h4>;
          }
          case "paragraph":
          ...
          case "feature_list":
          case "bullet_list":
            return <ul className={block.type === "feature_list" ? "grid gap-3 rounded-3xl bg-[#e6eee9] p-7" : "list-disc space-y-2 pl-6"} key={block.id}>{block.items.map((item) => <li key={item}>{item}</li>)}</ul>;
          case "callout":
            return <aside className="rounded-3xl border-l-4 border-[#17695a] bg-[#e6eee9] p-7" key={block.id}>{block.title ? <h3 className="text-xl font-semibold text-[#143a34]">{block.title}</h3> : null}<p className={`${block.title ? "mt-3 " : ""}whitespace-pre-line`}>{block.text}</p></aside>;
          case "quote":
          ...
          case "faq":
            return <section aria-label="Frequently asked questions" className="divide-y divide-stone-200" key={block.id}>{block.items.map((item) => <details className="py-4" key={item.question}><summary className="cursor-pointer font-semibold text-[#143a34]">{item.question}</summary><p className="mt-3 whitespace-pre-line">{item.answer}</p></details>)}</section>;
          ...
          case "related_articles": {
            ...
            return <nav aria-label={block.type === "related_products" ? "Related products" : "Related articles"} className="grid gap-3 sm:grid-cols-2" key={block.id}>{links.map((item) => <Link className="rounded-2xl border border-stone-200 p-5 font-semibold text-[#17695a]" href={item.href} key={item.id}>{item.label}</Link>)}</nav>;
          }
          case "cta":
            return <aside className="rounded-3xl bg-[#143f38] p-8 text-white" key={block.id}>{block.supportingText ? <p className="mb-5 text-white/75">{block.supportingText}</p> : null}<Link className="button-primary" href={block.href}>{block.label}</Link></aside>;
          case "divider":
