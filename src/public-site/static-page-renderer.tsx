...
export function StaticHomeRenderer({
  ...
  return <main data-static-page="home">
    ...
    {config.modules.manufacturing_strength ? <section className="site-container py-24"><div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr]"><div><p className="eyebrow">{copy.manufacturingStrength.eyebrow}</p><h2 className="section-title mt-4">{copy.manufacturingStrength.title}</h2>{copy.manufacturingStrength.summary ? <p className="mt-5 leading-7 text-stone-600">{copy.manufacturingStrength.summary}</p> : null}{facts.length ? <ul className="mt-6 grid gap-3 text-stone-600">{facts.map((fact) => <li key={fact}>— {fact}</li>)}</ul> : null}</div><div><StaticMedia className="mb-6 aspect-[16/9]" placementKey="manufacturing_strength" placements={placements} /><div className="grid gap-3 sm:grid-cols-2">{strengthTitles.map((title) => <h3 className="rounded-2xl bg-[#e6eee9] p-5 font-semibold text-[#143a34]" key={title}>{title}</h3>)}</div></div></div></section> : null}
...
export function StaticAboutRenderer({ config, placements, facts }: Readonly<{ config: AboutConfig; placements: readonly PublicStaticPagePlacement[]; facts: readonly string[] }>) {
  ...
  return <main data-static-page="about">
    ...
    {config.modules.owned_manufacturing ? <section className="border-y border-stone-200 bg-white py-20"><div className="site-container grid gap-10 lg:grid-cols-2"><StaticMedia className="aspect-[4/3]" placementKey="owned_manufacturing" placements={placements} /><div><p className="eyebrow">{copy.ownedManufacturing.eyebrow}</p><h2 className="section-title mt-4">{copy.ownedManufacturing.title}</h2>{copy.ownedManufacturing.summary ? <p className="mt-5 leading-7 text-stone-600">{copy.ownedManufacturing.summary}</p> : null}{facts.length ? <ul className="mt-6 grid gap-3">{facts.map((fact) => <li key={fact}>— {fact}</li>)}</ul> : null}</div></div></section> : null}
