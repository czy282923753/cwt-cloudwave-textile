...
export function StaticAboutRenderer({ config, placements, facts }: Readonly<{ config: AboutConfig; placements: readonly PublicStaticPagePlacement[]; facts: readonly { key: string; statement: string }[] }>) {
  const copy = config.copy;
  if (!copy) {
  ...
  return <main data-static-page="about">
    ...
    {evidenceGates.owned_manufacturing ? <section className="border-y border-stone-200 bg-white py-20" data-fact-sensitive-module="owned_manufacturing"><div className="site-container grid gap-10 lg:grid-cols-2"><StaticMedia className="aspect-[4/3]" placementKey="owned_manufacturing" placements={placements} /><div><h2 className="section-title">{STATIC_PAGE_FACT_SENSITIVE_LABELS.owned_manufacturing}</h2><ul className="mt-6 grid gap-3">{facts.map((fact) => <li key={fact.key}>— {fact.statement}</li>)}</ul></div></div></section> : null}
    {evidenceGates.service_strength ? <section className="site-container py-20" data-fact-sensitive-module="service_strength"><h2 className="section-title">{STATIC_PAGE_FACT_SENSITIVE_LABELS.service_strength}</h2><StaticMedia className="mt-8 aspect-[16/7]" placementKey="service_strength" placements={placements} /><div className="mt-8 grid gap-4 sm:grid-cols-2">{strengthTitles.slice(1).map((title) => <h3 className="rounded-2xl bg-[#f4f0e5] p-6 text-xl font-semibold text-[#143a34]" key={title}>{title}</h3>)}</div></section> : null}
