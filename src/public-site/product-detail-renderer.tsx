...
export function ProductDetailRenderer({ product }: Readonly<{ product: ProductDetailViewModel }>) {
  ...
  return <main data-product-detail={product.id}>
    ...
    <section className="site-container grid gap-10 pb-20 pt-4 lg:grid-cols-[1.05fr_.95fr]"><div className="grid gap-4 sm:grid-cols-2">{product.images.length ? product.images.map((image, index) => <div className={`relative overflow-hidden rounded-[2rem] bg-stone-200 ${index === 0 ? "aspect-[4/3] sm:col-span-2" : "aspect-square"}`} key={image.id}><Image alt={image.alt} className="object-cover" fill priority={index === 0} sizes={index === 0 ? "(max-width: 1024px) 100vw, 55vw" : "(max-width: 1024px) 50vw, 28vw"} src={image.url} unoptimized /></div>) : <div aria-hidden="true" className="weave-placeholder aspect-[4/3] rounded-[2rem] sm:col-span-2" />}</div><div className="lg:sticky lg:top-28 lg:self-start"><p className="eyebrow">Fabric Record</p><h1 className="mt-4 text-4xl font-semibold leading-tight tracking-[-0.045em] text-[#143a34] sm:text-5xl">{product.name}</h1>{product.shortDescription ? <p className="mt-6 text-lg leading-8 text-stone-600">{product.shortDescription}</p> : null}{product.taxonomy.length ? <div className="mt-7 flex flex-wrap gap-2">{product.taxonomy.map((term) => term.path ? <Link className="rounded-full border border-stone-300 px-3 py-1.5 text-xs text-stone-600" href={term.path} key={term.name}>{term.name}</Link> : <span className="rounded-full border border-stone-300 px-3 py-1.5 text-xs text-stone-600" key={term.name}>{term.name}</span>)}</div> : null}<div className="mt-9 flex flex-wrap gap-3"><TrackedLink className="button-primary" eventName="quote_cta_click" href={`/get-quote/?product=${encodeURIComponent(product.name)}`} placement="product_hero">Find Your Fabric Solution</TrackedLink><TrackedLink className="button-secondary" eventName="quote_cta_click" href="/get-quote/#upload" placement="product_upload">Upload a Reference</TrackedLink></div></div></section>
    ...
    <section className="bg-[#143f38] py-20 text-white"><div className="site-container flex flex-wrap items-center justify-between gap-8"><h2 className="text-3xl font-semibold">Send the reference. Start the conversation.</h2><TrackedLink className="button-primary" eventName="quote_cta_click" href="/get-quote/" placement="product_footer">Find Your Fabric Solution</TrackedLink></div></section>
  </main>;
}
