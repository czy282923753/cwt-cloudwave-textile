    ...
    <article className="group overflow-hidden rounded-[1.5rem] border border-stone-200 bg-white">
      <Link href={product.path}>
        <div className="relative aspect-[4/3] overflow-hidden bg-[#e9e4d8]">
          {product.image ? <ResponsivePublicImage asset={product.image} className="object-cover transition duration-500 group-hover:scale-[1.03]" fill sizes="(max-width: 768px) 100vw, 33vw" /> : <div className="weave-placeholder h-full" aria-hidden="true" />}
        </div>
        <div className="p-5">
