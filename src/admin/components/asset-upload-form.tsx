...
const fieldClass = "min-w-0 w-full rounded-lg border border-white/10 bg-slate-950 p-3";
...
export function AssetUploadForm({ associations, returnTo = "/admin/assets/" }: Readonly<{
  ...
  return (
    <form className="grid min-w-0 gap-5 rounded-2xl border border-white/10 bg-slate-900 p-4 sm:p-6" onSubmit={submit}>
      <div className="min-w-0"><h2 className="text-xl font-semibold">Upload assets</h2><p className="mt-2 break-words text-sm text-slate-300">Files use authenticated Upload Intents, a bounded binary stream, quarantine, scanning, and atomic release.</p></div>
      <label className="grid min-w-0 gap-2">Files<input accept="image/jpeg,image/png,image/webp,image/avif,application/pdf" className={fieldClass} multiple name="files" required type="file" /></label>
      <label className="grid min-w-0 gap-2">Asset Category<select className={fieldClass} name="category" required><option value="product">Product</option><option value="fabric">Fabric</option><option value="market">Market</option><option value="company">Company</option><option value="factory">Factory</option><option value="application">Application</option><option value="certificate">Certificate</option><option value="content">Content</option><option value="other">Other</option></select></label>
      <label className="grid min-w-0 gap-2">Associate with<select className={fieldClass} name="association"><option value="">No association yet</option>{associations.map((association) => <option key={association.value} value={association.value}>{association.group} · {association.label}</option>)}</select></label>
      <div className="grid min-w-0 gap-4 sm:grid-cols-2"><label className="grid min-w-0 gap-2">Asset role<select className={fieldClass} defaultValue="gallery" name="role"><option value="hero">Hero image</option><option value="gallery">Gallery image</option><option value="cover">Cover image</option><option value="detail">Detail image</option><option value="thumbnail">Thumbnail image</option><option value="inline">Inline image</option><option value="document">Document / certificate</option><option value="download">Download attachment</option></select></label><label className="grid min-w-0 gap-2">Sort order<input className={fieldClass} defaultValue={0} min={0} name="sortOrder" type="number" /></label></div>
      ...
      {!declarationEnabled ? <p className="text-sm text-slate-400">OFF: source, copyright, authorization, relationship, review, and restriction fields stay null.</p> : (
        <fieldset className="grid min-w-0 gap-4 rounded-xl border border-amber-300/30 p-4 sm:grid-cols-2"><legend className="px-2 text-amber-200">Optional declaration fields</legend>
