...
export default async function FabricTypePage({ params }: Readonly<{ params: Promise<{ slug: string }> }>) {
  ...
  return (
    <PublicShell>
      <main><section className="page-hero py-20"><div className="site-container"><p className="eyebrow">{term.dimension.replaceAll("_", " ")}</p><h1 className="section-title mt-4">{term.name}</h1>{term.description ? <p className="mt-6 max-w-3xl text-lg leading-8 text-[#586B73]">{term.description}</p> : null}<TrackedLink className="button-primary mt-8" eventName="quote_cta_click" href="/get-quote/" placement="taxonomy_hero">Ask About This Fabric Type</TrackedLink></div></section><section className="site-container py-16"><h2 className="section-title">Related products</h2>{products.length ? <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <p className="mt-6 text-[#586B73]">No published product records are linked yet.</p>}</section></main>
