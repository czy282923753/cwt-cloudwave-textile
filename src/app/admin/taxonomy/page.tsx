        ...
        <section className="grid gap-5">
          {terms.map((term) => (
            <article className={panelClass} key={term.id}>
              ...
              <form action={updateTaxonomyAction} className="grid gap-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <form action={setTaxonomyIndexAction} className="flex gap-2"><input name="termId" type="hidden" value={term.id} /><select className={`${inputClass} min-w-0 flex-1`} defaultValue={term.indexStatus} name="indexStatus"><option value="noindex">Noindex</option><option value="index">Index</option></select><button className="rounded-xl border border-white/20 px-3">Apply</button></form>
                <form action={setTaxonomyActiveAction} className="flex gap-2"><input name="termId" type="hidden" value={term.id} /><input name="active" type="hidden" value={term.isActive ? "false" : "true"} /><button className="w-full rounded-xl border border-white/20 px-3 py-3">{term.isActive ? "Deactivate + Noindex" : "Activate"}</button></form>
              </div>
              <form action={changeTaxonomySlugAction} className="flex gap-2"><input name="termId" type="hidden" value={term.id} /><input className={`${inputClass} min-w-0 flex-1`} name="slug" placeholder="new-taxonomy-slug" required /><button className="rounded-xl border border-white/20 px-3">Change URL + 301</button></form>
            </article>
          ))}
        </section>
        <form action={createTaxonomyAction} className={`${panelClass} content-start`}>
