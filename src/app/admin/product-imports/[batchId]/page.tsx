...
export default async function ProductImportDetailPage({ params }: { params: Promise<{ batchId: string }> }) {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
    <AdminPageHeader action={<div className="flex flex-wrap gap-3">{result.batch.status === "validated" ? <><ProductImportApply batchId={batchId} /><ProductImportApply batchId={batchId} cancel /></> : null}{result.batch.status === "completed" && errors ? <ProductImportApply batchId={batchId} retry /> : null}{errors ? <a className="rounded-xl border border-white/20 px-4 py-3" href={`/api/admin/product-imports/${batchId}/errors/`}>Export Row Errors</a> : null}</div>} description={`Mode ${result.batch.mode} · ${rows.length} Product rows · ${errors} Row Errors. Successful items are immutable and are never replayed.`} title={`Import ${batchId.slice(0, 8)}`} />
