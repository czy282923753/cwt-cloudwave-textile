...
import { findRedirect, getPublishedProductByPath } from "@/public-site/data";
...
async function resolveProduct(slug: string) {
  const path = `/products/${slug.toLowerCase()}`;
  const product = await getPublishedProductByPath(path);
  if (!product) {
...
export async function generateMetadata({
  ...
  const indexAllowed = env.APP_ENV === "production" && product.indexStatus === "index";
  return {
