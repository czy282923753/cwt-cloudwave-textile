...
export async function generateMetadata({ params }: Readonly<{ params: Promise<{ slug: string }> }>): Promise<Metadata> {
  ...
  return {
    ...
    robots: derivedPageRobots(entry.indexStatus, entry.hasEligibleProducts),
...
export default async function FabricEntryPage({ params }: Readonly<{ params: Promise<{ slug: string }> }>) {
  ...
  return (
    <PublicShell>
      <main>
        <section className={`site-container grid gap-10 py-16 ${entry.image ? "lg:grid-cols-[1fr_.85fr]" : "max-w-4xl"}`}>
          {entry.image ? <div className="relative aspect-square overflow-hidden rounded-lg bg-[#EAF4F5]"><ResponsivePublicImage asset={entry.image} className="object-cover" fill priority sizes="(max-width: 1024px) 100vw, 55vw" /></div> : null}
          <div className="self-center"><p className="eyebrow">Fabric Library Entry</p><h1 className="section-title mt-4">{entry.title}</h1>{entry.description ? <p className="mt-6 text-lg leading-8 text-[#586B73]">{entry.description}</p> : <p className="mt-6 text-[#586B73]">Visual reference entry. This page remains Noindex unless standalone value is confirmed.</p>}<TrackedLink className="button-primary mt-8" eventName="quote_cta_click" href="/get-quote/#upload" placement="fabric_entry">Ask About This Fabric</TrackedLink></div>
        </section>
        {entry.relatedProducts.length ? <section className="border-t border-[#CCDDE1] bg-[#F5F9F9] py-16"><div className="site-container"><h2 className="section-title">Related products</h2><div className="mt-8 flex flex-wrap gap-3">{entry.relatedProducts.map((product) => <Link className="button-secondary" href={product.path} key={product.id}>{product.name}</Link>)}</div></div></section> : null}
      </main>
