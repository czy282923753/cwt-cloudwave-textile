...
import { retryProductImportErrors } from "@/imports/service";
...
export async function POST(request: Request, context: { params: Promise<{ batchId: string }> }): Promise<NextResponse> {
  try {
    ...
    if (databaseConnection.kind === "pglite") await retryProductImportErrors(databaseConnection.db, actor, batchId);
    else await retryProductImportErrors(databaseConnection.db, actor, batchId);
