...
export async function POST(request: Request, context: { params: Promise<{ batchId: string }> }): Promise<NextResponse> {
  try {
    ...
    if (databaseConnection.kind === "pglite") await cancelProductImportBatch(databaseConnection.db, actor, batchId);
    else await cancelProductImportBatch(databaseConnection.db, actor, batchId);
