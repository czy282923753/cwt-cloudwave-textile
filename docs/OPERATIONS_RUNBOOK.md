...
Use `deploy/scripts/preflight-release-compose.mjs`; do not reconstruct an ad hoc root-Compose harness. The validator accepts one explicit outer Unix endpoint and owns one exact pinned private DIND controller for the whole run. The controller uses outer `--network none`, no published port, no host Docker/containerd socket and six run-exact named volumes for API, Docker data, containerd data, config, storage and journal. Repository/workspace are the only host binds. Config/storage population uses pinned network-none helpers and supported volume mounts; no raw volume Mountpoint or VM-only `/etc/cwt`, `/srv/cwt` or `/run/systemd/journal` bind is allowed. The exact journal helper uses VM-host `socat` through `/proc/<helper-pid>/root` with `UNIX-RECVFROM` and `fork`, never `UNIX-RECV`, TCP or a new package/image.
...
3. Confirm exact pinned DIND, `docker:29.6.2-cli` and `alpine:3.22` are already local; do not pull. Verify exact index, arm64 child and embedded Docker `29.6.2`, then retain the pinned DIND base.
4. From the clean committed Candidate, run `self-test` exactly once with explicit `--outer-host`, run-unique `--token` and an absent absolute `--evidence` directory. Require config/storage/journal named volumes, `0444` config closure, UID/GID `10001:10001` storage write, `UNIX-RECVFROM` journal socket and journald emission, private containerd/Unix API, outer invisibility, owner-only Alpine import, internal Compose network, repository/workspace bind, no port, inner communication, diagnostics-before-removal and zero exact controller/helper/six-volume/socket/Compose/workspace residue. Failure consumes the run: preserve evidence, clean exactly and stop.
5. After that reproduction passes, run every frozen source gate against the unchanged committed Candidate. A separate Release Operator—not this correction task—receives the next single Build Once authority.
...
3. Preserve the evidence JSON and hash, then remove direct containers, mutation project, positive Compose consumers and networks. Only after zero consumers may the owner remove `Q`/`T` and pinned dependency references. Capture sanitized controller/journal-helper state, complete timestamped logs and hashes before controller stop/wait/removal; then remove the exact journal helper, all six named volumes and workspace, and prove the endpoint/controller/helper/volumes/socket/Compose/workspace are absent. Cleanup must not mask the primary failure.
...
The weekly executable uses a matching COS repository in protected environments and local Restic only in Synthetic tests, with no fallback. It fully validates the exact new snapshot, reads every repository pack once and checks older tree/manifest metadata before four-valid retention. Exit 2 explicitly means the new set verified but repository/retention maintenance failed; preserve the set and investigate maintenance. Production COS delivery, private repository policy, egress, independent alerts, host failure recovery and target-host timing remain External Validation Required. Restore into protected Staging and any Web/Worker activation stay behind the existing lifecycle/Access gate. The executable local restore uses a network-none Linux Synthetic lab with a Unix-socket-only temporary PostgreSQL target and never starts Web/Worker.
