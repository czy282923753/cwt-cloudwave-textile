...
Use `deploy/scripts/preflight-release-compose.mjs`; do not reconstruct an ad hoc root-Compose harness. The validator accepts one explicit outer Unix endpoint and owns one exact pinned private DIND controller for the whole run. The controller uses outer `--network none`, no published port, no host Docker/containerd socket and three run-exact volumes for its Unix API, Docker data and containerd data. Every release load, inspection, Compose config/up/inspection/down and image removal reaches only `unix:///run/cwt-owner-api/docker.sock` through helpers mounting that API volume. There is no external `--owner-host`, TCP, Docker-context, host-network `nsenter` or shared-containerd compatibility mode. The outer endpoint may own only the controller, exact volumes, Synthetic setup helpers and non-CWT source archive; it must never create, inspect for acceptance, tag or delete the CWT successor reference.
...
3. Confirm local `docker:29.6.2-cli` and `alpine:3.22` without pulling. Under the separately granted one-seed authority, pull only `docker:29.6.2-dind@sha256:bfec1f5159c63a81ca6fdedbd81404d2c0e16378ed0feec3bb3fbf3998847659`; verify arm64 child `sha256:48bd8cb4ce95d6c03004ee4fe06db27a49813fe0c3a55785a9bf06c941d9a9df` and embedded Docker `29.6.2`, then retain that pinned base image locally.
4. From the clean committed Candidate, run `self-test` exactly once with explicit `--outer-host`, run-unique `--token` and an absent absolute `--evidence` directory. The validator creates its owner; do not pass `--owner-host`. Require private containerd under the run exec root, conventional private `moby` namespaces, outer invisibility, Alpine owner-only import, one internal Compose network, same-path bind, journald, no port, inner communication, diagnostics-before-removal and zero targeted residue. Failure consumes the run: preserve evidence, clean exactly and stop.
...
3. Preserve the evidence JSON and hash, then remove direct containers, mutation project, positive Compose consumers and networks. Only after zero consumers may the owner remove `Q`/`T` and pinned dependency references. Capture sanitized controller state, complete timestamped logs and the log SHA before stop/wait/removal; then remove the exact API/Docker/containerd volumes, Synthetic state, journald sink and workspace, and prove the endpoint/controller/volumes/socket/Compose/Synthetic paths are absent. Cleanup must not mask the primary failure.
