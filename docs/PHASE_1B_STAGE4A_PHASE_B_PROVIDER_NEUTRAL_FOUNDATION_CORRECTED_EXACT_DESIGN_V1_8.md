# CWT Stage 4A Phase B — Provider-neutral Foundation Corrected Exact Design V1.8

- Status: **CORRECTED EXACT DESIGN CANDIDATE V1.8 / OWNER IMP2-NM01 DIRECTION INCORPORATED / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED**
- Design version: `1.8`
- Standalone basis: the complete V1.7 contract, which reproduces V1.6/V1.5 and accepted Design V1.4, is reproduced here with only the bounded IMP2-NM01 context-domain correction; an implementer does not combine older designs with remediation notes to recover the normative design
...
- V1.8 correction scope: `IMP2-NM01` only; replace whole-context lexical classification with one closed typed field-domain traversal while keeping the accepted context shape/bytes, selected M02 grammar, M03 discriminated seam, V1.7 lifecycle graph, and every other V1.7 clause unchanged
- Prepared: `2026-08-11` (Asia/Shanghai)
- Corrected Design branch: `codex/phase-1b-stage4a-phase-b-corrected-design-v1-8`
...
V1.8 retains V1.4's complete independently accepted dual authorization/binder and construction design, V1.5's Owner-selected M02/M03 seams, V1.6's unique classification of the original six executables, and V1.7's two-state `next-env.d.ts` lifecycle proof. Generic `ApplicationReadScope` remains an unbranded structural constraint containing only `readonly mode: string`; it owns no constructor, factory, database, lock, transaction, or authority. Draft and Synthetic each own their module-private nominal brand, private executor capability, direct object-literal factories, and callback lifetime. Factory-returned values bind to the same opaque staged closures before generic core entry. The mode string alone cannot enter a Draft binder because the Draft-private carrier is absent. Core still receives no application scope, target, repository, factory, or lock method. V1.8 changes only the context proof responsibility: a single application-owned typed traversal assigns every strict context node to exactly one validation domain; the machine association tuple is structurally decoded and recomputed, while every human/business/Provider-evidence surface keeps the exact selected M02 classifier identity.

## 2. Fixed authority and verified remediation identity

The mandatory pre-edit architecture/security gate for this V1.8 Candidate is:
...
The sole machine-readable context-domain authority is [IMP2-NM01 Context Domain Profile V1.0](review-evidence/phase-1b-stage4a-phase-b-corrected-exact-design-v1-8-imp2-nm01-v1/IMP2_NM01_CONTEXT_DOMAIN_PROFILE_V1_0.json), profile `cwt.phase1b.stage4a.phaseb.corrected-design.v1_8.imp2-nm01.context-domain.v1`. Its one traversal ID is `ctx.draft-assistance.v1.field-domain-traversal.v1`. The implementation must compile that closed typed schema/profile into the one context validator; prose, a consumer path list, or a second visitor cannot select a domain.

The traversal operates after fatal UTF-8/accepted JSON decoding and strict outer schema parsing, before JCS persistence acceptance, replay readiness, Prompt projection, or adapter resolution:

1. Visit the root and every object, array, field, array member, and accepted evidence-subtree node in deterministic schema/array order.
2. Match each node against the profile's closed path-pattern language. `/` is root, `*` is one array index, and the sole recursive form `**` is legal only at the end of `/sources/*/fields/*/value/**`.
3. Require exactly one matching assignment. Zero matches return `context_domain_unclassified`; two or more return `context_domain_ambiguous`. An unknown strict key therefore cannot be silently skipped, including an ignored/generated/consumer-injected property.
4. Execute that assignment's one validation domain. Domains are disjoint ownership labels, not ordered fallbacks. A container validates exact keys/cardinality/order; a machine field validates its exact grammar and cross-field integrity; a protected evidence root validates its field grammar/bounds and calls the selected M02 classifier on the original value/subtree.
5. Return the original accepted object and bytes unchanged. There is no redaction, normalization write-back, association projection rewrite, path exception, compatibility traversal, or consumer-local bypass.
...
| `/sources/*/fields/*/value/**` | `protected_human_business_provider_evidence` | complete Product/Fabric/Company Fact/explicit-human value subtree. The existing M02 preflight recursively covers every object key and string value while accepting only the existing finite JSON value grammar; numeric/boolean/null values remain structurally checked. Product names/category/Application labels, composition/GSM/width/MOQ/fabric style/colors/notes/availability, Fabric title/excerpt/narrative, Company Fact keys/subjects/statements/relationship labels, and explicit input are all inside this domain |
...
Acceptance and claimed replay both perform the same fail-closed order: strict context shape; strict durable association decode; rebuild the authorized snapshot from accepted target columns/authority; JCS canonicalize; recompute lowercase SHA-256; compare context kind/type/version; compare durable `target_snapshot_hash`; compare context `snapshotHash`; only then continue to full-context JCS/`input_hash`. Tamper, uppercase, wrong length, SHA-512/other algorithm, snapshot version other than `1`, target type/identity mismatch, expected-version mismatch, or replay mismatch returns provenance failure before Prompt or adapter resolution. A supplied 64-hex string is never trusted merely because it has the correct grammar.
...
Every protected-domain root invokes the exact Section 2.2 compiled object identity `(registryId, registryVersion, SHA-256)`. For source `value`, one call at the field-value root recursively scans its complete accepted subtree, including arbitrary evidence-object keys and all string leaves. Descendants remain assigned to the same protected domain but do not create a second classifier instance. For task narratives, field labels, and internal-link labels, one call scans the exact original scalar.
...
`input_context_json` shape, member order produced by the application, original string code points, JSONB logical value, and accepted persisted bytes are unchanged. The new traversal validates and returns the same object; it does not remove association metadata or serialize a second context. RFC 8785/JCS still covers the complete object, including association, and produces the same `input_hash` fixed vector. Claimed replay recomputes association integrity, every protected lexical root, and the complete context hash before Prompt/config/envelope checks and adapter resolution.
...
10. call the opaque claimed runtime's `decodeClaimedContext(input_context_json)`: strict-decode the unchanged shape; recompute/compare the authorized association snapshot and all three hash copies; run the one Section 13.2 typed domain traversal with zero missing/ambiguous fields; invoke the selected M02 identity on every protected evidence root; validate every alias/ref/provenance/limit; recompute full-context JCS `input_hash`; and require association/application/use-case/input-schema/policy identity. Every check completes before adapter resolution. The returned `OpaqueClaimedContextStageV1` captures the typed application context;
...
| IMP2-NM01 field-domain traversal | materialize every required/optional/container/evidence-subtree node; remove each assignment; duplicate each assignment; add unknown field; second traversal/exception/bypass mutations | one profile/interpreter; every node exactly one domain; `missing=[]`, `duplicate=[]`; all zero/ambiguous/silent-exclusion mutations fail closed |
| protected evidence coverage | phone, email, credential, DeepSeek/provider override, Inquiry/private-data payload at every task narrative, source field label, nested source value key/string, and link label | exact selected M02 identity rejects every protected placement; runtime/registry mutation fails; no consumer exception or second scanner |
...
- Run `VERIFY_CORRECTED_EXACT_DESIGN_V1_8.mjs` offline under the exact selected runtime tuple. It must interpret the single context-domain profile, report zero missing/duplicate nodes, accept 1,000/1,000 deterministic valid Revision hashes, accept the Reviewer hash only on recomputed equality, reject every integrity/domain/protected/machine/projection mutation, prove persisted bytes/JCS/`input_hash` identity, prove no association leak, verify the same M02 identity is named by context and A-07, and report M03 plus `21/21`/`96/96` non-regression. Its captured output and a fresh rerun must be byte-identical. This is design-consistency proof, not an implementation claim.
...
- the owned-file whitespace check and design scope gate show no historical Prompt mutation, preserve the documentation/evidence-only V1.7 delta, and require the V1.8 delta itself to contain docs/evidence only; immutable imported Reviewer hard-break bytes are reported separately rather than rewritten or misreported.
...
Only after the exact V1.8 commit receives Fresh Independent Design Review PASS and the Coordinator opens a new implementation task may the following implementation order be used:
...
## 23. Exact V1.8 design acceptance checklist

The Fresh Independent Design Review may return PASS only if all are true:

- [ ] exact V1.8 branch/start/parent/checkpoint-ref/ancestry/tag identity passes; checkpoint remains pinned to `b7ad96b...` before and after design; formal worktree is clean;
...
- [ ] accepted V1.7 design/audit/profile/verifier/31-item manifest remain byte-identical; V1.8 is standalone and the delta is exact docs/evidence-only scope;
- [ ] V1.8 accounts for every preserved `H-01`, `H-02`, `M-01..M-06`, `L-01`, and `N-M01..N-M04` closure plus exact IMP2-NH01/M01/M02/M03/M04/M05/L01/L02/NM01 review dispositions without implementation claims;
...
Phase B increases compiled validation/test maintenance but adds no cross-process/persistent coordination. V1.3's two explicit entry compositions remain; V1.4 deleted one unusable generic brand and gave each application one direct construction seam. V1.5 selected one compiled classifier authority and one exhaustive existing-union switch. V1.6 retained the 12-class selector; V1.7 added one exact member to its existing root-control class plus a two-state offline proof. V1.8 replaces one overly broad traversal contract with one typed domain traversal; it adds no classifier, exception path, runtime choice, database, wrapper, cache, Provider, adapter, dependency, persistent state, Schema, Migration, or ADR. Phase C retains the already approved operational complexity without a fake, duplicate authorization, generic factory hierarchy, or application-specific core path.

## 26. Design conclusion and next gate

Conclusion: **CORRECTED EXACT DESIGN CANDIDATE V1.8 / OWNER IMP2-NM01 DIRECTION INCORPORATED / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED.**

The next and only gate is **Fresh Independent Corrected Design V1.8 Review by the original Phase B Design Reviewer** against the exact V1.8 commit, IMP2-NM01 Owner Decision Record, pre-design checkpoint, imported byte-identical V2.2 FAIL package, fixed V1.7 authority, selected M02 and unchanged M03 V2.2 profiles, two-state official-generation proof, field-domain profile/verifier/mutations, type probes and actual-tree inventories. Phase B implementation and the remaining ordinary correction attempts must not start until that review returns PASS and a later task explicitly opens implementation. No self-review, Provider call, credential, network, Staging/Production, Deploy, formal import, Publish, Index, merge or Push is authorized or performed by this design.
