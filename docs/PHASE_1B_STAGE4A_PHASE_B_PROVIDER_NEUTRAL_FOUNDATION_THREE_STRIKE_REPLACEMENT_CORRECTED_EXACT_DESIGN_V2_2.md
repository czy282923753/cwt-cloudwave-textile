# CWT Stage 4A Phase B — Provider-neutral Foundation Three-Strike Replacement Corrected Exact Design V2.2

Status: **V2-M01 RAW CLI BOUNDED REMEDIATION CANDIDATE / REPLACEMENT-CYCLE CORRECTION ATTEMPT 2 / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED**
...
This is a complete standalone Phase B V2.2 successor. It preserves the independently
...
V2.1 already replaced the V2.0 loader boundary and closed the ignored copy,
hard-link and self-consistently resealed alternate-root defect. This V2.2
successor replaces V2.1's remaining raw CLI selection boundary: V2.1 resolved
the `--authority` value before equality and therefore accepted forbidden
spellings that normalized to the fixed root. V2.2 does not reopen Option A and
retains neither V2.0 nor V2.1 loader code as a fallback.
...
`V111_M01_CANONICAL_REVIEW_AUTHORITY_V2_2.json`. Its embedded subject names the
...
| fixed failed V2.0 Candidate | ref `codex/phase-1b-stage4a-phase-b-v111-m01-three-strike-replacement-design-v1`; commit `4b626fc9278f4c49957ecf165d7d5c5fc4058dca`; parent `3aaad46...`; tree `8e4d3b...` |
| controlling independent FAIL | report SHA-256 `6962e45985f17cccbd978d2069c7adf9b64163da3b7b9d3f05b56cd36b0c0205`; Reviewer manifest SHA-256 `ac31e83689fec456095ae85f10ec5881e74c22881a1456a24c190bb716ea854a`, `9/9 PASS` |
| V2-M01 remediation checkpoint | `codex/checkpoint/phase-1b-stage4a-phase-b-v111-m01-v2-m01-remediation-pre-l3-v1` -> exact `4b626fc...` |
| remediation checkpoint record | `3c387781cb0b8b6889e32883f637ddfe9d9c36e7`; direct one-path child of `4b626fc...`; tree `beee1774d9bc51ff700807de6f9b22fce3eda020` |
| byte-identical Reviewer FAIL import | `ac080b1d8b49906154ecbc44d381a84afe972bad`; parent `3c387781...`; tree `3871f32de97baed44e283aa145d1a70265534156` |
| fixed failed V2.1 Candidate | ref `codex/phase-1b-stage4a-phase-b-v111-m01-replacement-design-remediation-v1`; commit `3d424821aab67c03c3b8ec02a62b5577044837c9`; parent `ac080b1d...`; tree `2feaecc...` |
| controlling V2.1 Fresh FAIL | report SHA-256 `3056465d76a89bef97233fc4554ed7f754c316c20413a352aeaf7803430ab357`; evidence SHA-256 `fe15e07c7b587aa6c0d5961b5338a827ab7067dfe3c98758f255823c5712c34f`; Reviewer manifest SHA-256 `7a6ff7781bd43ef7658fa78e5d29c9a19910e6bf1df02132d1b8a9e19e4ebc57`, `10/10 PASS` |
| V2.1 raw-CLI remediation checkpoint | `codex/checkpoint/phase-1b-stage4a-phase-b-v111-m01-v21-cli-remediation-pre-l3-v1` -> exact `3d424821...` |
| raw-CLI checkpoint record | `eca7843fa3ced513cc199514bbf5afad30dc5553`; direct one-path child of `3d424821...`; tree `be9141a4062fe450b799222bc2c28c5407ef8a0e`; record SHA-256 `53da981aede0d4e1e0a96c0d9a64cc6ceff7966965b8acade8dc12946a2e699b` |
| byte-identical V2.1 Reviewer FAIL import | `626552c4b3eb2ef3f0dbeadddcf5202444102368`; parent `eca7843...`; tree `63eeb54c50cc516daff5fb65a305d6d2f2bf1ee2`; 11 paths including the 10-entry manifest |
...
## 17. Canonical review authority V2.2

The canonical root has exactly 14 top-level keys and closed nested schemas. It
...
executable current roles, five proof contracts, four checkpoints, three
ordinary failed-attempt history records and closed V2.0 plus V2.1
Candidate/Reviewer-FAIL records. Both failed successors and their imported
Reviewer envelopes have `nonExecutable=true`, `currentExecutableEdge=false`
and zero authority edge. The current roles are:
...
The `--authority` argument is an assertion of one constant, never a semantic
locator. At the actual CLI entry boundary, the raw JavaScript string is compared
by exact byte/code-point spelling with the sole constant before any
`path.resolve`, `path.normalize`, `path.relative`, URL/percent decoding,
filesystem lookup, `realpath`, `lstat`, `stat`, Unicode normalization or case
folding. Any non-identical spelling deterministically throws
`raw_authority_spelling_not_canonical`; NUL is rejected by the Node process
interface before the CLI where it cannot be represented.

The sole accepted raw assertion and envelope binding is the exact closed
repo-relative POSIX path
`docs/review-evidence/phase-1b-stage4a-phase-b-v111-m01-replacement-corrected-design-v2-2/V111_M01_CANONICAL_REVIEW_AUTHORITY_V2_2.json`.
Only after raw equality succeeds may the verifier resolve and read that fixed
constant. Before parsing its semantic JSON, it proves that this exact
case-sensitive path is a mode-`100644` blob in the observed `HEAD` tree, that
the stage-0 index object equals that `HEAD` blob, and that worktree bytes equal
the same blob.

One fixed V2.2 `SHA256SUMS.txt` is then read from its exact `HEAD` path. It
must contain exactly one entry for the canonical root; that entry's SHA-256 must
equal the exact `HEAD` blob and loaded bytes. The CLI and envelope cannot
redirect the manifest join. The manifest remains derived integrity evidence
and is not a second semantic authority.

The actually loaded canonical root, the manifest, all manifest members and all
checkpoint records enter one physical identity set. Distinct paths sharing a
realpath/device/inode fail, including hard links from the root to a current
role, proof/capture, checkpoint or manifest. All current/checkpoint/proof/
applicable-history paths are repo-relative POSIX, NFC, case-exact and unique.
Existing roles must be tracked regular files.
Absolute realpath, dot prefix/segment, cancelling or non-cancelling dot-dot,
duplicate or trailing slash, backslash, percent-bearing/encoded, NFD/Unicode
lookalike, case variant, control, whitespace, tilde/home/environment spelling,
symlink and shared device/inode all fail closed. No normalization-compatibility
alias or second path table exists.

The one-way graph is exactly root to embedded subject/current roles/proofs/
checkpoints/frozen inputs/history/closed V2.0 and V2.1 evidence; subject to the three
current roles; and proof contracts to the current technical profile. All other
nodes are leaves. Missing, extra or duplicate node/edge, cycle, or current/
history role/path overlap fails.
...
Package-only mode runs only against an exact committed `HEAD` root and always
emits `acceptanceEligible=false`; it cannot bootstrap or imply acceptance.
Full-review mode consumes every envelope leaf and
...
The successor retains the accepted `10 positive / 42 negative / 10 property`
matrix and the V2.1 `10/65/14` extension, then extends it to exact
`10 positive / 91 negative / 16 property`.
It executes the extended matrix twice with byte-identical output. It covers
duplicate-aware parsing,
...
no-cycle. The 23 added negatives cover the three exact Reviewer alternate-root
witnesses, visible and ignored paths, tracked wrong paths, case and symlink
aliases, all four root hard-link collision classes, sole-root manifest joins,
coordinated CLI/envelope redirection, `HEAD` mode/blob/membership, index and
worktree drift, envelope path mutation, resealing and fresh ignored-directory
names beyond `.next`. Twenty-four additional raw-spelling cases invoke the real
package CLI as child processes (with the NUL case rejected at the Node process
interface). Two mutation cases prove that restoring resolve-before-validation
or substituting helper-only tests is killed. Every spawned case must expose a
real child PID, exit `1`, empty stdout and the exact
`raw_authority_spelling_not_canonical` diagnostic. The canonical package-only,
attached exact-ref and detached exact-HEAD positives invoke the same real CLI
entry rather than a helper path.
...
| `V2-M01` | **RAW CLI BOUNDED REMEDIATION CANDIDATE CREATED — ATTEMPT 2**, same V111-M01 root; author cannot close it |
...
`codex/checkpoint/phase-1b-stage4a-phase-b-v111-m01-v21-cli-remediation-pre-l3-v1`
at `3d424821...`. The attempt-1 checkpoint remains `4b626fc...` and the V2.0
replacement pre-L3 boundary remains `c103682...`. The atomic
Candidate docs/evidence commit may be reverted as a unit. Underlying rollbacks
remain `0793948...`, accepted V1.10 `234cd902...`,
