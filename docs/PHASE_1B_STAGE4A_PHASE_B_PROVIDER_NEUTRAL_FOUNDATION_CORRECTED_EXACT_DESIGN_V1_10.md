# CWT Stage 4A Phase B — Provider-neutral Foundation Corrected Exact Design V1.10

- Status: **CORRECTED EXACT DESIGN CANDIDATE V1.10 / V18-M01 AND V18-M03 ATTEMPT-2 REMEDIATED / V18-M02 FROZEN CLOSED / OWNER IMP2-NM01 DIRECTION PRESERVED / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED**
- Design version: `1.10`
- Standalone basis: the complete V1.9 contract is reproduced here with only the bounded V18-M01 two-layer error proof and V18-M03 immutable compiled-product correction; an implementer does not combine V1.9 with remediation notes to recover the normative design
...
- Owner selections incorporated: `M02-D1-INCLUDE` and `M03-D1-DISCRIMINATED-SEAM`, approved 2026-08-10; `IMP2-NM01-STRUCTURAL-INTEGRITY-DOMAIN`, approved for corrected-Design authorship 2026-08-11
- Preserved closed findings: `H-01`, `H-02`, `M-01`, `M-02`, `M-03`, `M-04`, `M-05`, `M-06`, `L-01`, `N-M01`, `N-M02`, `N-M03`, and `N-M04`
- V1.10 correction scope: `V18-M01` and `V18-M03` correction attempt 2 only; separate the normative 69-code runtime closure from the selected five-code TypeScript subset, and replace the mutable-source/cache shortcut with one input-detached, recursively frozen, module-private compiled closure while preserving the accepted context shape/bytes, selected M02 grammar, M03 discriminated seam, V1.7 lifecycle graph, every non-finding V1.8 clause, and the byte-identical V18-M02 `CR-01..CR-14` authority
- Prepared: `2026-08-11` (Asia/Shanghai)
- Corrected Design branch: `codex/phase-1b-stage4a-phase-b-corrected-design-v1-10`
- Exact V1.10 start / immutable V1.9 Candidate: `c0fe5b57100ff7fd83ef50b85288e6160397af80`
- V1.9 direct parent: `fcb060edb47c04187ccdaea4cb65b1032dffa828`
- Exact V1.8 ancestor: `a2cd31f53c34dad479862a1c67260ab71fda9805`
- Byte-identical V1.9 independent FAIL import commit / V1.10 authoring parent: `487a75a18e5f6f8b57cf13eba765657221aa1619`
- V1.8 direct parent: `8eac88210d41b5e37ab5963acc7ee28d007c4fd3`
- Byte-identical V1.8 independent FAIL import commit: `901e87c620c52458c0a56613ec87353dbe240319`
- Exact V1.8 start / immutable pre-design checkpoint target: `b7ad96b24da45de00cae2cdb961a9aefcbc99496`
- Exact V1.8 start parent: `846888a409b2b62869ff7ff8fca36b88b70d0bf9`
- Checkpoint record commit: `a90d642da38274ae3fab67ba4d8f284d8ddc5c35`
- Immutable V2.2 FAIL import commit / V1.8 authoring parent: `8eac88210d41b5e37ab5963acc7ee28d007c4fd3`
- Accepted V1.7 / full design rollback ancestor: `3f475e13d73d9d68a4dfb9a040941c57f1ee92e6`
- V1.5 exact failed-review Candidate: `da2143654a372f70a93ff22f9fcb6e999f1e528e`
- V1.6 exact failed-review Candidate: `49ba05ff0e40efce4ba8feb1bc87528414e3fad9`
- Exact V1.7 authoring parent after byte-identical FAIL import: `799b2dd4340c244d9f609942bd097acbb2ff6ecc`
...
- Next gate: **Fresh Independent Corrected Design V1.10 Review by the original independent Phase B Design Reviewer against the exact V1.10 commit**

> This document is a complete, standalone corrected Design Candidate and Complex Task Analysis only. The Owner decisions authorize authorship of this design, not implementation, merge, Push, Provider/API/credential/network/spend, Staging/Production, Deploy, Publish, Index, formal import, or Phase C/D/E. It does not approve itself. Every V1.9 and older design, remediation, review, evidence, and probe artifact remains byte-identical. Implementation correction attempt 3 remains forbidden until the exact V1.10 commit receives Fresh Independent Design Review PASS and a later task explicitly opens implementation.
...
V1.10 retains V1.4's complete independently accepted dual authorization/binder and construction design, V1.5's Owner-selected M02/M03 seams, V1.6's unique classification of the original six executables, V1.7's two-state `next-env.d.ts` lifecycle proof, V1.8's Owner-selected typed context-domain direction, and V1.9's closed error mappings plus sole claimed-replay order. Generic `ApplicationReadScope` remains an unbranded structural constraint containing only `readonly mode: string`; it owns no constructor, factory, database, lock, transaction, or authority. Draft and Synthetic each own their module-private nominal brand, private executor capability, direct object-literal factories, and callback lifetime. Factory-returned values bind to the same opaque staged closures before generic core entry. The mode string alone cannot enter a Draft binder because the Draft-private carrier is absent. Core still receives no application scope, target, repository, factory, or lock method. V1.10 changes only two proof boundaries: it states the 69-code runtime closure and five-code TypeScript subset as different layers, and traversal consumes only an input-detached recursively frozen compiled closure. Every human/business/Provider-evidence surface still uses the exact selected M02 classifier identity. V18-M02's machine order and §18.4/§18.5 projections are unchanged.

## 2. Fixed authority and verified remediation identity

The V1.10 remediation identity is closed before any normative contract below is considered:

| Check | Verified identity | Required result |
|---|---|---|
| exact V1.9 Candidate / V1.10 start | `c0fe5b57100ff7fd83ef50b85288e6160397af80` | PASS; V1.9 ref is not moved |
| V1.9 direct parent / V1.8 ancestor | `fcb060edb47c04187ccdaea4cb65b1032dffa828` / `a2cd31f53c34dad479862a1c67260ab71fda9805` | PASS |
| V1.9 Fresh independent FAIL report/evidence/challenge/output/identity/author-output/manifest | `d94716f0...` / `4756eb53...` / `9f1acd0a...` / `447f1750...` / `465917af...` / `0c4d4ed6...` / `fee6dbfc...` | PASS; imported byte-identically; manifest 6/6 |
| V1.8 Design / profile / vectors / verifier / capture / manifest | `4fc11c42...` / `16084d40...` / `435f8f6f...` / `22f304c5...` / `8cac7702...` / `ca51146d...` | PASS; V1.8 manifest 21/21 and all bytes immutable |
| V1.8 Fresh independent FAIL report/evidence/challenge/output/manifest | `556efabf...` / `9e29864e...` / `ed175fba...` / `b58ab3d1...` / `73a08831...` | PASS; imported byte-identically; manifest 7/7 |
| immutable checkpoint ref / target | `codex/checkpoint/phase-1b-stage4a-phase-b-nm01-pre-design-v1` / `b7ad96b24da45de00cae2cdb961a9aefcbc99496` | PASS before V1.10; must remain exact after V1.10; never move/rebuild |
| checkpoint record | `docs/PHASE_1B_STAGE4A_PHASE_B_IMP2_NM01_PRE_DESIGN_CHECKPOINT_V1_0.md`, SHA-256 `07e6c7a6...` | PASS; byte-identical |
| accepted V1.7 ancestor / frozen Tag peel | `3f475e13...` / `31c0e405...` | PASS |
| V1.10 branch | `codex/phase-1b-stage4a-phase-b-corrected-design-v1-10` | dedicated linear docs/evidence-only branch |

Rollback of the V1.10 remediation is its exact start `c0fe5b...`; rollback of the full IMP2-NM01 design chain remains the immutable checkpoint `b7ad96b...`; accepted V1.7 remains the full design rollback `3f475e13...`. No Tag, source, test, fixture, Schema, Migration, snapshot, journal, seed, ADR, dependency, package, lockfile, Prompt runtime, Provider or environment is changed.

The mandatory pre-edit architecture/security gate below is retained byte-for-byte in substance as V1.8 historical authority:

| Check | Verified identity | Result |
|---|---|---|
| exact start / source ref | `b7ad96b24da45de00cae2cdb961a9aefcbc99496` / `codex/phase-1b-stage4a-phase-b-foundation-implementation-v2-remediation-v2` | PASS |
| direct parent / code HEAD | `846888a409b2b62869ff7ff8fca36b88b70d0bf9` / `111301aea82569768661c6401b16054161ed19ff` | PASS |
| accepted V1.7 ancestor | `3f475e13d73d9d68a4dfb9a040941c57f1ee92e6` | PASS |
| immutable checkpoint ref / target | `codex/checkpoint/phase-1b-stage4a-phase-b-nm01-pre-design-v1` / `b7ad96b24da45de00cae2cdb961a9aefcbc99496` | PASS before mutation; reverify before/after design |
| checkpoint record commit | `a90d642da38274ae3fab67ba4d8f284d8ddc5c35` | PASS; first V1.8 commit and sole artifact in that commit |
| immutable V2.2 FAIL import commit | `8eac88210d41b5e37ab5963acc7ee28d007c4fd3` | PASS; byte-identical five-artifact import |
| V2.2 report/evidence/patch/output/manifest | `54a52f15...` / `106e547a...` / `d3c66f50...` / `5ab2ff49...` / `a2bcea31...` | PASS; manifest 4/4 |
| V1.7 design/audit/profile/verifier/manifest | `e432fbd9...` / `a89c8ee2...` / `1f0b56a8...` / `d4043c6d...` / `f10a9eac...` | PASS; manifest 31/31 and byte-identical |
| frozen tag peeled commit | `31c0e405acfdd0d05200d0fb2531e897a541a2c4` | PASS |
| formal worktree before checkpoint | `/Users/calvin/.codex/worktrees/07a1/CWT（CloudWave Textile）项目` | clean / exact HEAD |
| Owner IMP2-NM01 direction | one typed domain traversal; structural association integrity; unchanged protected evidence scanning | RECORDED; design-only |

The immutable checkpoint ref was absent before atomic creation, remains pinned to the exact start, and is not a Tag. A different target, dirty formal start, missing ancestry, or changed evidence hash is a blocking identity failure. Rollback for this design package is the exact start/checkpoint target; accepted V1.7 remains the complete design rollback ancestor.

The following table is the retained V1.7 authoring gate and earlier accepted lineage that V1.8 preserves byte-for-byte as historical authority:

| Check | Verified identity | Result |
|---|---|---|
| V1.7 authoring parent | `799b2dd4340c244d9f609942bd097acbb2ff6ecc` | PASS |
| V1.5 exact Candidate / parent | `da2143654a372f70a93ff22f9fcb6e999f1e528e` / `9cedefd618a168176f8d70a85e3ec8cb684967a7` | PASS |
| V1.5 Design / derivation / Owner record / manifest | `22d9820f1faa5b318bb6904adcefbb96ba59559f132c9dcd611e16c574643698` / `481947f06bd51139d081b46bf3ef852f1ce233191428e568b1c970ad36547d65` / `21c5db60154374f52546dae730dd2893d1fb53689757cf969106b9f1c7c96fa2` / `a98890959e9e24c522367b9e788cdd797cb6e98d5d1d57e35dcbdea01e853d6e` | PASS, byte-identical 22/22 |
| Fresh V1.5 FAIL report/evidence/manifest | `723a4d59af63c66117ed2c5a9dcb349a18c970b85b067bba6eda8bcb129e1aa4` / `3a75df5276e8fd8d34a2d88aa29f038a538870e6e9333b7d006106bb94d1d73a` / `5a8635b95a6b435ff3b5e7111a86d13c0a379ca2f908e434dd899e7118819f01` | PASS, byte-identical 17/17 |
| V1.6 Candidate / audit / manifest | `49ba05ff0e40efce4ba8feb1bc87528414e3fad9` / `d3bcf40c999672f7e7a8b3fb05c09a98457e68c9171294ac0c1ef5494ab73c31` / `36da610be6e29ef9fd461bd10b0d12c09fa48573384b33356aa1fc0da79af48b` | PASS, V1.6 design SHA-256 `06fb0795cc05e6651f63f46226c67290e108103cb567e5aa5e80a8b09a33eec2`, byte-identical 28/28 |
| Fresh V1.6 FAIL report/evidence/probe/output/manifest | `50289f51b93dc4a333db3e2fac8875fb0c2f5c6e6d8c4d126462446acfd122ab` / `00ccf4da3a61c3d46acb8bb81e49e749c061eb5658af1bc52e322a30fab7e833` / `1c389fee052e42b5513a12ce7a25d6ca01247854673c220228a60fbb798619f6` / `b085621c8f7b019e9fee0afd665ff2f667716693f413dea3c2ebd62f3cb88b61` / `0c33a9ca1e783c76a6bb7757557e67233285205b12995fc105e6fe649d848bef` | PASS, byte-identical 4/4 manifest |
...
The table below is retained as the complete V1.4 historical design lineage that V1.7 preserves:
...
The historical V1.4 worktree was attached to its required design branch and clean before V1.4 was added. These fixed Fresh Re-review inputs, including every manifest-protected positive/negative probe, remain part of the accepted V1.4 proof inherited by V1.7:
...
The [IMP2-NM01 Owner Decision Record V1.0](PHASE_1B_STAGE4A_PHASE_B_IMP2_NM01_OWNER_DECISION_RECORD_V1_0.md) is the sole authority for the V1.8 change. It selects `IMP2-NM01-STRUCTURAL-INTEGRITY-DOMAIN`: retain the byte-identical selected M02 classifier for all human/business/Provider evidence, but validate application-generated association metadata with exact typed structure plus target-snapshot recomputation. It requires one closed application-owned field-domain traversal and forbids a second scanner, path exception list, compatibility traversal, or consumer-local bypass. It preserves context bytes, JCS/`input_hash`, Schema/Migration, and Provider neutrality. The selection authorizes corrected-Design authorship only.

The exact [V2.2 Fresh Independent Implementation FAIL](PHASE_1B_STAGE4A_PHASE_B_INDEPENDENT_IMPLEMENTATION_REREVIEW_V2_2.md), SHA-256 `54a52f15b93de13bef6c9e0b692bf22c755ac592f0794914cebe643ad327b73a`, is the IMP2-NM01 causal authority. Its evidence, patch, output, and manifest hashes are `106e547aae4576ece7bc8dd12cc3c330b275e7541c34792dff4917eb39579d79`, `d3c66f50289a01e5d0d371a4f8845702c446ef28007b2d60fa6d0e0bd1068fd6`, `5ab2ff49aa5ed817606c7d2c8e839f3efd1d8e3adcd1ffde7b45f04a4b8669fe`, and `a2bcea3136dade24690a0db49d3dd49dbe2a7ba655d17d03cf3f6b4b2a6fbf93`; the imported manifest passes 4/4. The first deterministic collision is Revision `b534aed1-c661-4d8f-84cb-38c12157c622`, version `7`, hash `b574a4fc557400009c7cc935adad4ceda3b1dbaa7f07fd35972605390f06dafc`. The Reviewer measured 590/1,000 valid snapshot hashes rejected by the phone rule when hashes were incorrectly treated as natural language.

The authority for the two selected seams is the [Owner Selection Record V1.0](PHASE_1B_STAGE4A_PHASE_B_THREE_STRIKE_OWNER_SELECTION_RECORD_V1_0.md). It records exactly `M02-D1-INCLUDE` and `M03-D1-DISCRIMINATED-SEAM`. The selection authorizes corrected-Design authorship only. It does not authorize implementation.

The exact Fresh Independent Technical Escalation Re-review PASS is [Technical Escalation Remediation V1 Fresh Independent Re-review](PHASE_1B_STAGE4A_PHASE_B_THREE_STRIKE_TECHNICAL_ESCALATION_INDEPENDENT_REREVIEW_V1_0.md), SHA-256 `62089f2891049b3362876491b07e5ee22006629c905cf7d9d3360821414310d4`. Its five-item evidence manifest is SHA-256 `82a56ac807804f0765a6b3d30bd7dca77022d9e412f67838ba601ee3b0d834a8` and verifies 5/5. That PASS establishes the older M02/M03 decision readiness; it does not approve this V1.8 design.

The byte-identical [V1.5 Fresh Independent FAIL](PHASE_1B_STAGE4A_PHASE_B_CORRECTED_EXACT_DESIGN_INDEPENDENT_REVIEW_V1_0.md), SHA-256 `723a4d59af63c66117ed2c5a9dcb349a18c970b85b067bba6eda8bcb129e1aa4`, is the authority for the sole `V15-M01` correction. Its evidence and 17-item manifest are SHA-256 `3a75df5276e8fd8d34a2d88aa29f038a538870e6e9333b7d006106bb94d1d73a` and `5a8635b95a6b435ff3b5e7111a86d13c0a379ca2f908e434dd899e7118819f01`. The review keeps both Owner selections intact, confirms no further Owner decision is needed when the 12-class fail-closed guarantee is preserved, and authorizes neither remediation acceptance nor implementation.

The byte-identical [V1.6 Fresh Independent FAIL](PHASE_1B_STAGE4A_PHASE_B_CORRECTED_EXACT_DESIGN_INDEPENDENT_REVIEW_V1_1.md), SHA-256 `50289f51b93dc4a333db3e2fac8875fb0c2f5c6e6d8c4d126462446acfd122ab`, is the decisive Attempt-2 authority. Its evidence/probe/output/manifest hashes are `00ccf4da3a61c3d46acb8bb81e49e749c061eb5658af1bc52e322a30fab7e833`, `1c389fee052e42b5513a12ce7a25d6ca01247854673c220228a60fbb798619f6`, `b085621c8f7b019e9fee0afd665ff2f667716693f413dea3c2ebd62f3cb88b61`, and `0c33a9ca1e783c76a6bb7757557e67233285205b12995fc105e6fe649d848bef`. It proves that V2.1 classified the captured 402-node absent snapshot but failed the normal ignored-present lifecycle. It preserves both Owner selections and identifies no Schema, Migration, ADR, dependency, Complexity, or further Owner decision.

V1.7 preserves every V1.6 contract and corrects only this exact proof defect:

| Proof boundary | V1.6 retained contract/failure | V1.7 correction |
|---|---|---|
| `M02` / `DIAG-M01` | selected INCLUDE closed registry and full Unicode/gap/counter/consumer proof | unchanged; non-regression only |
| `M03` actual database seam | one outer discriminated root passes each branch-narrowed actual database type to the same generic factory | unchanged; strict positive and union/cross-driver negatives remain mandatory |
| `V15-M01` complete graph | same sealed 12 classes and original six fixed, but root `next-env.d.ts` becomes an ignored zero-class Candidate after normal Next type setup | add exact `next-env.d.ts` to the existing `root-control-file` matcher and prove both absent and officially generated present states with the same selector; no wildcard, deletion loop, second table, or scope narrowing |
...
V1.8 preserves that complete V1.7 result and changes only the causal IMP2-NM01 boundary:

| Proof boundary | V1.7 retained contract/failure | V1.8 correction |
|---|---|---|
| selected M02 | one 32-rule INCLUDE registry, fixed Unicode/runtime/AST/gap/counter semantics, shared with A-07 | byte-identical and unchanged; human/business/Provider-evidence domains still invoke its one compiled identity |
| association integrity | strict five-field context association plus canonical target snapshot and `target_snapshot_hash` | the five fields receive one machine structural/integrity domain; `snapshotHash` is exact lowercase SHA-256 of the authorized JCS snapshot and compared to durable/context copies before acceptance/replay |
| context traversal | one recursive classifier over every scalar/key in the whole accepted object, causing valid hashes to collide with natural-language phone recognition | one closed typed traversal assigns every node exactly one domain; zero/ambiguous fail closed; evidence subtrees scan, machine tuple does not; no exception path or second traversal |
| persistence/Provider projection | full strict context persisted and hashed; Prompt variables are pure projections | unchanged bytes/shape/JCS/`input_hash`; association remains excluded from all four Provider variable projections |
| M03 and V15-M01 lifecycle graph | selected discriminated database seam and V1.7 two-state actual-tree proof | unchanged; strict type/graph/profile non-regression only |

Implementation-review status remains exact: IMP2-NH01 is CLOSED; IMP2-M01/M02/M04 remain OPEN after attempt 2 with one final ordinary attempt each; IMP2-NM01 is the new design root addressed here with no implementation attempt; IMP2-M03/M05/L01/L02 remain frozen CLOSED. This design implements none of them.
...
```text
path       docs/review-evidence/phase-1b-stage4a-phase-b-corrected-design-v1-7-remediation-attempt-2-v1/M03_CAPABILITY_GRAPH_AND_DATABASE_SEAM_PROFILE_V2_2.json
profile    cwt.phase1b.stage4a.phaseb.m03-protected-graph.v2_2
version    2.2.0
SHA-256    1f0b56a870ecbab61c970e1c7000dff591674e0f8ad0a04341538c724a36c173
```

V2.2 is a standalone successor to immutable V2.1. It preserves the Owner-selected discriminated seam, protected/excluded roots, future adapter edge, acquisition bans, bundle zones, type shapes, 12 class IDs, closed match fields and original six dispositions. It changes only the framework-generated root-control lifecycle. V2.1, V1.5, V1.6 and both FAIL packages remain immutable evidence. V2.2 records the Owner selection as approved, this V1.7 design as an unapproved Candidate, and implementation as unauthorized.
...
The complete node classification retains exactly these 12 root classes: Phase B outer composition; reserved absent Phase D outer composition; reserved absent future Provider adapter zone; Synthetic/test AI; protected AI; business consumer; other Production source; AI proof tooling; other project tooling; other test fixtures; diagnostic documentation; and root control files. No class is added, renamed, merged or made optional.

The sole selector compiler is `cwt.closed-primary-root-selector.v1`. Each class owns one closed matcher using only `files`, `directories`, `testSemantic`, `excludeFiles`, `excludeDirectories`, and `excludeTestSemantic`. A candidate matches when an exact file, directory prefix, or declared test semantic is true and no declared exclusion is true. All 12 selectors are evaluated independently. Array order is stable report order only and never a tie-breaker. Exactly one true selector is required; zero and two-or-more both fail closed. Unknown fields/operators, implicit globbing, regex, basename inference, longest-prefix guessing, priority resolution, caller allowlists or compatibility classifiers are unsupported.

The actual-tree walk starts at repository root and uses sorted `lstat`, not `git ls-tree`. It skips only the exact top-level physical entries `.git`, `node_modules`, `.next`, `coverage`, `playwright-report`, `test-results`, `.data`, `.storage`, `dist`, `build`, and `tmp`; a similarly named nested path is not excluded. `.git` may be the exact worktree administrative file or directory; every other present excluded entry must be a physical directory. A symlink at any non-excluded path fails before candidate filtering and is never followed. A hard-link inode alias, case/canonical collision, repository escape, unreadable/special entry, excluded-root re-entry or undecidable realpath fails closed.

Candidate executable suffixes are exactly `.ts`, `.tsx`, `.mts`, `.cts`, `.js`, `.jsx`, `.mjs`, and `.cjs`. Candidate protected resources are `.json` and `.md` only below `src/ai/prompts/resources/`, `src/ai/prompts/generated/`, `test-fixtures/ai-architecture/`, `test-fixtures/ai-server-bundle/`, `test-fixtures/ai-types/`, or `docs/review-evidence/`. The ten exact root-control members are `package.json`, `pnpm-lock.yaml`, `tsconfig.json`, `next.config.ts`, `eslint.config.mjs`, `postcss.config.mjs`, `drizzle.config.ts`, `playwright.config.ts`, `vitest.config.mts`, and lifecycle-optional `next-env.d.ts`. The last path is still a Candidate through the ordinary `.ts` suffix; its exact root-control membership supplies disposition, not scope. A different root executable, ignored `.ts`, or config-like name is not swallowed by a wildcard; it remains unclassified until the reviewed single profile is versioned.

Tracked, untracked and ignored status is computed after inclusion and cannot narrow scope. Generated status also cannot narrow scope. Protected generated Prompt entries retain the V2.1 rule. The only framework-control generated path is exact root `next-env.d.ts`: when present it must be an ignored regular file, exactly 247 UTF-8 bytes with final LF and SHA-256 `7b550dda9686c16f36a17bf9051d5dbf31e98555b30d114ac49fc49a1e712651`; it must contain only the two sealed Next type references, `import "./.next/types/routes.d.ts";`, blank line and two standard generated comments recorded inline in V2.2; `tsconfig.json#include` must contain `next-env.d.ts`. It is marked generated and classified only as `root-control-file`. Any byte, path, type-reference, source-state, include, ignore or class mismatch fails closed. No operator may delete it before proof or require it to stay absent.

Test semantics are a path under `src/test/`, `tests/`, or `test-fixtures/`, or a declared `.test.*`, `.integration.test.*`, or `.spec.*` suffix for every executable extension. Intended nested classes are made disjoint through explicit exclusions; precedence does not repair overlap. This classifies setup/teardown and `.spec` by bounded semantics rather than six basename exceptions. The six decisive paths have the exact results:

| Existing path | Sole class | Authority ceiling |
|---|---|---|
| `drizzle.config.ts` | `root-control-file` | build/migration control only |
| `playwright.config.ts` | `root-control-file` | isolated test control only |
| `vitest.config.mts` | `root-control-file` | isolated test control only |
...
`next-env.d.ts`, when present, has sole class `root-control-file`. It owns no runtime execution, Provider/adapter, Provider credential/endpoint, external network, public entry, server entry or Production closure authority; its only allowed local specifier is the sealed framework type import. `src/test/**` is likewise test-only rather than Production source. Root-control and test-only classification never grants Provider/adapter, Provider credential/endpoint, external Provider network, public entry, server entry or Production runtime authority. Existing migration configuration, loopback E2E/browser traffic and synthetic test credentials remain their pre-existing non-Production test/build concerns, not AI authority. Tests may exercise Production modules in isolation; dependency direction from any Production/public/server node into test/config nodes fails closed.

The lifecycle proof uses a disposable detached worktree at one exact V1.7 proof checkpoint and the installed Next `16.2.12` public CLI with pinned Node `24.14.0`, telemetry disabled and no network/install. Before generation, `next-env.d.ts` and `.next/` are absent. The verifier freshly reports the complete tree, all 12 member arrays, `zeroClass=[]`, `ambiguous=[]`, and tracked/untracked/ignored counts. It then runs `next typegen .`; exit must be `0`. After generation, the same verifier must report `next-env.d.ts` as one `untracked-ignored`, generated, executable/root-control Candidate with sole class `root-control-file`, while zero/ambiguous remain empty. A second identical command must again exit `0` and reproduce every generated byte/hash. A handwritten file, direct private writer call, deletion step, different checkpoint, or single-state snapshot is not evidence.

The exact physical `.next/` root remains one of the 11 sealed top-level exclusions and is never a primary class selector. In the present lifecycle state the same proof separately audits that excluded volatile root: it must be a physical directory with exactly `.next/types/cache-life.d.ts`, `.next/types/routes.d.ts`, and `.next/types/validator.ts`, with respective SHA-256 values `d1986184a09a52db8228cb2bb2a61a8c05c9354e5b93cec8e2628d8579c892d7`, `e838150498c7e8464a1a0d7e25d7dfc79aa6f77358a8d83ac0aa7b28c5904fb4`, and `8ed142360153811ab434bbd2f2486b0052d9d5bbdcf067d206fc8d7eb15f28df`. Any extra/missing file, symlink, hard-link, special node, canonical escape or copied/aliased generated node outside exact `.next/` fails closed. This audit describes generated output and does not create a second architecture classifier or capability authority.

Each state prints the exact HEAD, lifecycle state, total candidates/executables, every sorted class including empty arrays, `zeroClass=[]`, `ambiguous=[]`, source states, excluded roots, protected/test/control/composition/Phase D/adapter status, `next-env.d.ts` facts, `.next` audit, and independent inventory/content/classification/generated-root SHA-256 domains. Design-Candidate mode requires Phase B composition/protected implementation absent because implementation is unauthorized; future implementation-gate mode instead requires exactly one declared Phase B root while still requiring Phase D and adapter counts zero. Evidence `.txt` capture and `SHA256SUMS.txt` are envelopes; every executable probe/verifier and `.json`/`.md` evidence profile is itself enumerated and hashed before both captures.
...
The graph verifier must retain all 17 V1.6 mutation failures: one new unclassified executable; one path matching two classes; removal of each of the original six dispositions; silent physical exclusion; symlink; canonical/hard-link alias; alias import; unmanifested generated resource; early Phase D; early adapter; second composition root; and sealed-exclusion integrity. Attempt 2 adds: present `next-env.d.ts` without a class; double class; silent ignored/excluded candidate; a different ignored root `.ts`; `next-env.d.ts` symlink and hard-link aliases; generated byte/path/type-reference changes; a deletion prerequisite; and a `.next` alias/escape. Each reaches its exact fail-closed reason in memory without modifying either worktree. A verifier that checks only class count, only named files, only Git-tracked paths, only an absent snapshot, or only a few acquisition spellings is non-conforming.

### 2.4 Phase allocation after selection

| Phase | V1.7 authority |
|---|---|
...
| Phase C | later accepted durable `ai_runs` enqueue/claim/lease/retry/cancel/budget/Audit composition; must consume the V1.7 seams and cannot create a temporary or parallel runtime |
...
Sources and fields are in deterministic selector/allowlist order, not database incidental order. Every Prompt JSON variable is a pure projection of this context: SEO uses task page fields, source values, link candidates; Fabric uses topic/source values; Product uses Product source values/media refs; Sourcing uses guide intent/source values. `buildPromptVariables` has no database port and is run both at preparation and claimed execution. JCS of the entire strict object—including the unchanged five-field association—is `input_hash`.

The sole machine-readable context-domain derivative, closed error projection, recursive evidence grammar, and claimed-replay-order authority is [V18 M01/M03 Attempt-2 Machine Profile V2.0](review-evidence/phase-1b-stage4a-phase-b-corrected-exact-design-v1-10-v18-m01-m03-attempt-2-v1/V18_M01_M03_ATTEMPT_2_MACHINE_PROFILE_V2_0.json), profile `cwt.phase1b.stage4a.phaseb.corrected-design.v1_10.v18-m01-m03-attempt-2.v1`, compiled identity `efdec023f886a9ca9d1a558bfba0e54781292e0e8215609c4b68a68268caba01`. Its one traversal ID remains `ctx.draft-assistance.v1.field-domain-traversal.v1`. `src/ai/errors.ts` remains the unique runtime error authority; the profile is a manifest-bound derivative checked against it, not a second authority. Prose, a consumer path list, a second visitor, a copied order array, a second error map, or a per-validation profile override cannot select a domain, error, or replay step.

One compiler accepts only the repository-reviewed profile bytes/plain JSON value. Before returning anything, it validates the fixed profile ID/version and identity projection, plain JSON shape with no cycle/custom prototype/alias sharing, the exact 35 assignments and three domains, recursive grammar, selected M02 identity, all 69 source error projections, the selected five traversal returns, the byte-identical V18-M02 order, and Provider-projection denial. It then builds a recursive detached copy, recursively freezes it, and closes it inside one module-private `validateContext(context, external)` function. The only product surface is the fixed identity, a recursively frozen count-only summary, and that validator closure. Source JSON, assignment/domain arrays, and the internal sealed value are not exported. Every traversal accepts that compiled product, never raw/imported profile JSON. The module may compile once; any cache may hold only the sealed product under the fixed identity and may never return or traverse the source object.

The exact context validation subsequence is one fail-closed pipeline and is the `CR-04..CR-08` projection of the same profile order:

1. `CR-04`: fatal UTF-8/accepted-JSON decode, reject unsupported/cyclic/sparse/non-finite/custom values, and strict-parse the unchanged outer shape and exact use-case keys. No field is yet accepted merely because outer parsing succeeds.
2. `CR-05`: obtain the sole module-private compiled closure whose reviewed profile projection has SHA-256 `efdec023f886a9ca9d1a558bfba0e54781292e0e8215609c4b68a68268caba01`. Compilation first validates the exact 35 assignment records, three domains, recursive grammar, selected M02 identity, full 69-code runtime projection, five-code traversal subset, and byte-identical 14-step order, then deep-copies and recursively freezes the closed value. Any source/profile demotion, reclassification, alias, prototype, table edit, or identity mismatch fails `context_provenance_mismatch` before a product exists. Post-compile source mutation cannot affect traversal because traversal has no source reference.
3. `CR-06`: visit the root and recursively visit every object, object key/value, array, array member, and evidence-subtree node in input order. Match against the closed pattern language: `/` is root, `*` is one array index, and `**` exists only at the end of `/sources/*/fields/*/value/**`. Each node must have exactly one assignment. Zero or multiple matches both return the existing closed `AiErrorCode` `context_provenance_mismatch`; `assignment_count_zero|multiple` is an offline proof reason only and is never a second public, telemetry, or persisted error code.
4. `CR-07`: execute the sole assigned domain. Domains are disjoint ownership labels, not ordered fallbacks. Containers validate keys/cardinality/order; machine fields validate exact grammars; protected roots validate bounds and call the exact selected M02 classifier over the complete original recursive subtree, including every object key and string leaf. Invalid-control, unsupported, or protected results return `context_prohibited_data`.
5. `CR-08`: rebuild and compare the authorized association snapshot, JCS, lowercase SHA-256 copies, complete-context JCS, and `input_hash`. Return the original object and persisted bytes unchanged. There is no redaction, normalization write-back, second serialization, path exception, compatibility traversal, or consumer bypass.

The recursive value grammar accepts only `null`, boolean, finite JSON number, string, dense arrays of recursive values, and plain objects whose values are recursive. Maximum depth is `16`, maximum nodes per source value is `4096`, and the complete context remains at most `64 KiB` UTF-8. Sparse arrays, cycles, non-finite numbers, `undefined`, BigInt, Symbol, Function, typed arrays, Date, Map, Set, or custom prototypes reject. Object-key order may vary without changing JCS; array order is semantic; neither case rewrites accepted persisted bytes.

The exhaustive domain map is:

| Exact profile pattern(s) | Sole domain | Exact authority and Provider meaning |
|---|---|---|
| `/`; `/association`; `/task`; `/sources`; `/sources/*`; `/sources/*/fields`; `/sources/*/fields/*`; `/internalLinkCandidates`; `/internalLinkCandidates/*`; `/mediaPlacementRefs` | `closed_container` | strict object/array keys, cardinalities and deterministic order; unknown/additional/sparse/cyclic/unsupported shapes fail; a container never exempts descendant evidence |
| `/version`, `/applicationClass`, `/capability`, `/useCase`, `/locale` | `machine_structural_integrity` | application-owned literals/enums `1`, `draft_assistance`, `text`, the four Production use cases, and `en`; they are control metadata, not business evidence |
| `/association/kind`, `/association/targetType`, `/association/targetAlias`, `/association/expectedVersion`, `/association/snapshotHash` | `machine_structural_integrity` | the complete association object; exact validators and recomputation are specified below; none is a human-language value or Prompt variable |
| `/task/tone` | `machine_structural_integrity` | application-owned exact two-member enum; Provider receives the selected enum, not arbitrary text |
| `/task/pageIntent`, `/task/primaryPhrase`, `/task/topic`, `/task/guideIntent` | `protected_human_business_provider_evidence` | bounded task narrative; selected M02 classifier runs on each present value; invalid control, unsupported, or protected match rejects |
| `/sources/*/alias`, `/sources/*/sourceClass`, `/sources/*/selectedBy` | `machine_structural_integrity` | sequential opaque `src_NN`, exact four-class enum, literal `request_actor`; application-generated provenance metadata |
| `/sources/*/fields/*/ref`, `/sources/*/fields/*/provenance` | `machine_structural_integrity` | exact `<alias>:<field>` unique ref and exact `structural|provided|verified`; refs are opaque pointers, never evidence or identity authority |
| `/sources/*/fields/*/field` | `protected_human_business_provider_evidence` | must also be the exact source-class allowlisted identifier; because it is a Provider-visible evidence label, the selected M02 classifier runs before acceptance |
| `/sources/*/fields/*/value/**` | `protected_human_business_provider_evidence` | complete recursively nested Product/Fabric/Company Fact/explicit-human JSON value. The same selected M02 instance covers every object key and string leaf at arbitrary accepted nesting; arrays retain order, objects use JCS key order only for hashing, and numeric/boolean/null leaves remain structural. Product names/category/Application labels, composition/GSM/width/MOQ/fabric style/colors/notes/availability, Fabric title/excerpt/narrative, Company Fact keys/subjects/statements/relationship labels, and explicit input are all inside this domain |
| `/internalLinkCandidates/*/candidateRef` | `machine_structural_integrity` | unique opaque application-generated `link_NN`; no URL or target identity and no Provider-evidence meaning |
| `/internalLinkCandidates/*/label` | `protected_human_business_provider_evidence` | bounded Provider-bound narrative label; selected M02 classifier required |
| `/mediaPlacementRefs/*` | `machine_structural_integrity` | unique opaque application-generated `media_NN`; never an Asset ID/key/URL and not evidence text |

There are no other strict context fields. In particular, the machine domain does not contain Product/Fabric/Company values, human input, target text, link labels, arbitrary object keys, Provider/model names supplied by a caller, endpoints, credentials, files, URLs, customer data, or free-form strings. A later schema field is invalid until the one typed traversal assigns it exactly one reviewed domain; defaulting it to machine metadata is forbidden.

#### 13.2.1 Complete association integrity contract

The persisted context association remains byte-for-byte the same ordered five-field object: `kind`, `targetType`, `targetAlias`, `expectedVersion`, `snapshotHash`. Its validators are:

| Field | Exact validator | Cross-authority check |
|---|---|---|
| `kind` | literal `draft_target.v1` | equals the strict decoded durable association kind |
| `targetType` | exact enum `product_draft | content_draft | editorial_revision` | selects exactly one accepted canonical target-snapshot variant and equals durable target type |
| `targetAlias` | literal `target_01` | Provider-safe opaque alias only; never a database ID |
| `expectedVersion` | integer `1..2147483647` | equals the decoded durable expected target version and snapshot `expected_target_version` |
| `snapshotHash` | regex `^[0-9a-f]{64}$` | equals lowercase SHA-256 of the authorized RFC 8785/JCS snapshot and the durable `target_snapshot_hash` |

The only hash algorithm/version is SHA-256 over the accepted JCS bytes of association snapshot version `1`. The authorized snapshot variants and exact keys remain V1.7's literals:

- Product: `association_version`, `expected_target_version`, `target_locale='en'`, `target_product_id`, `target_type='product_draft'`;
- Content: `association_version`, `expected_target_version`, `target_content_id`, `target_locale='en'`, `target_type='content_draft'`;
- Revision: `association_version`, `expected_target_version`, `target_revision_id`, `target_type='editorial_revision'`.

Acceptance and claimed replay use the same two-part fail-closed authority. Strict outer shape and association tuple decoding happen first; compiled profile identity, complete recursive assignment coverage, and every domain validator happen next; only then does `CR-08` rebuild the authorized snapshot from accepted target columns, JCS-canonicalize, recompute lowercase SHA-256, compare context kind/type/version, compare durable `target_snapshot_hash`, compare context `snapshotHash`, and recompute complete-context JCS/`input_hash`. Tamper, uppercase, wrong length, SHA-512/other algorithm, snapshot version other than `1`, target type/identity mismatch, expected-version mismatch, replay mismatch, or a profile assignment mutation returns provenance failure before config, Prompt, envelope, adapter resolution, or adapter call. A supplied 64-hex string is never trusted merely because it has the correct grammar.

The Reviewer vector `b574a4fc557400009c7cc935adad4ceda3b1dbaa7f07fd35972605390f06dafc` is accepted only for the exact authorized Revision `b534aed1-c661-4d8f-84cb-38c12157c622` at version `7`, whose canonical snapshot hashes to that value and whose persisted target hash matches. It fails under any other target/version/durable hash. Digit runs have no special meaning in this domain; all 1,000 deterministic valid Revision hashes in Section 20 must accept, including hashes containing seven or more consecutive decimal digits.

#### 13.2.2 Protected evidence and single M02 identity

Every protected-domain root invokes the exact Section 2.2 compiled object identity `(registryId, registryVersion, SHA-256)`. For source `value`, the single domain traversal recursively feeds every original object key and every string leaf to that same compiled classifier instance; arrays/objects provide recursion and numeric/boolean/null leaves receive structure only. Descendants remain assigned to the same protected domain and cannot create another classifier instance or consumer list. Task narratives, field labels and internal-link labels each feed their exact original scalar to the same identity.

The classifier's own fail-closed preflight, invalid-control precedence, Unicode tuple, 32-rule priority, rule-specific gaps/counters, structured recognizers, overflow semantics, and non-mutating bytes remain unchanged. Context evidence and output A-07 must hold the same compiled identity; they cannot compile separate registries with equal-looking JSON. A registry/runtime/identity mismatch rejects initialization. Phone, email, credential, DeepSeek/provider override, Inquiry/private-data, URL/tool/public-state, and every other selected category placed at any protected root must reject. Placing arbitrary text in a machine field instead fails its strict structural grammar; the machine domain is not a general safe-string bucket.

#### 13.2.3 Provider projection and persistence

The four Prompt projections remain exactly Section 12.6: source evidence plus the use-case task fields, link labels/opaque refs where applicable, media placement refs where applicable, locale, and tone. No projection reads `/association` or `input_sources_json`; `snapshotHash`, target type/version/alias, and durable IDs are not Provider variables. `input_sources_json` remains the bounded internal provenance list and is not a Provider payload.

`input_context_json` shape, member order produced by the application, original string code points, JSONB logical value, and accepted persisted bytes are unchanged. The traversal validates and returns the same object; it does not remove association metadata or serialize a second context. RFC 8785/JCS still covers the complete object, including association, and produces the same `input_hash` fixed vector. In claimed replay the sole order is `CR-01..CR-14`: application runtime selection is `CR-02`; durable association/context structure is `CR-03..CR-04`; exact profile identity, complete recursive assignment, and domain/M02 validation are `CR-05..CR-07`; association and full-context JCS/hash reconstruction is `CR-08`; config, Prompt, and envelope are `CR-09..CR-11`; adapter resolution/request reconstruction is `CR-12`; the sole call is `CR-13`; local completion/output protection is `CR-14`. Availability (§18.2) and preparation/enqueue (§18.3) are separate sequences and do not reorder claimed replay.

<!-- V1_10_CLAIMED_REPLAY_ORDER: CR-01,CR-02,CR-03,CR-04,CR-05,CR-06,CR-07,CR-08,CR-09,CR-10,CR-11,CR-12,CR-13,CR-14 -->
...
Strict objects reject unknown keys. Before JCS, the one Section 13.2 typed traversal assigns every context node exactly one domain. It invokes the one compiled selected M02 registry identity from Section 2.2 at every protected human/business/Provider-evidence root and nowhere else. The registry's 32 rules—not consumer-local fragments—own Inquiry/Contact/Organization/CRM/customer/PII, identity, private storage, credential/Secret, network location, tool/retrieval, Provider/model/endpoint override, and public-state categories. `invalid_control`, `unsupported_value`, or any protected match rejects rather than redacts; `allow` continues. The complete association object instead must pass the exact structural/recomputed integrity sequence in Section 13.2.1. Original keys, values, code points and UTF-8 bytes are never rewritten. There is no second context rule list, traversal, gap table, direct matcher, exception path, or general machine-string escape.
...
The unique closed `AiErrorCode` tuple remains the `aiErrorCodes` export and `aiFailure` projection in `src/ai/errors.ts`; V1.10 adds no error code and no map. The machine profile reproduces all `69` tuple members as a checked derivative. Error codes are lowercase snake case so Phase C can project an approved execution failure directly into the accepted `ai_runs.failure_code` representation:

| Category | Codes |
|---|---|
| Authorization | `authorization_denied`, `target_not_found`, `target_not_editable`, `target_scope_mismatch` |
| Availability | `environment_not_authorized`, `feature_disabled`, `feature_flag_missing`, `integration_not_ready` |
| Configuration | `use_case_unknown`, `registry_invalid`, `config_missing`, `config_disabled`, `config_default_missing`, `config_ambiguous`, `config_repository_invalid`, `config_invalid`, `budget_disabled`, `provider_unsupported`, `model_unsupported`, `parameters_invalid`, `fallback_forbidden` |
...
| Conflict | `target_version_conflict`, `claimed_run_required`, `claim_expired`, `state_conflict`, `idempotency_conflict` |
| Internal | `canonicalization_failed`, `internal_failure` |

For every member, category is the exact row above and the exact per-code machine-profile entry. `retryable=true` only for `provider_timeout`, `provider_transport_error`, `provider_rate_limited`, `provider_server_error`, and `state_conflict`; every other code is false. `manualEditorAvailable=true` only for `environment_not_authorized`, `feature_disabled`, `feature_flag_missing`, `integration_not_ready`, `config_missing`, `config_disabled`, `config_default_missing`, `config_ambiguous`, `config_repository_invalid`, `config_invalid`, `budget_disabled`, `provider_unsupported`, `model_unsupported`, `parameters_invalid`, `fallback_forbidden`, `prompt_not_found`, `prompt_invalid`, `prompt_manifest_invalid`, `prompt_bundle_invalid`, `prompt_hash_mismatch`, and `prompt_contract_mismatch`; every other code is false. The safe default message is exactly `AI assistance is unavailable.` unless a separately reviewed fixed safe message is supplied; raw messages are never copied.

Telemetry receives only safe `error_code` and category plus bounded non-body fields. A failure before a run insert creates no synthetic `ai_runs` row; when an authoritative run already exists, Phase C stores the same exact code in that run's accepted `failure_code` and never remaps it. Phase C scheduling uses retry class plus attempt/budget/lease/cancellation/run state; Phase B's boolean is advisory and never loops. Precedence is the first terminal failure in the active normative availability, preparation/enqueue, or claimed-replay sequence; later steps do not execute. Unknown caught values—not a closed traversal result—become `internal_failure` at the current step.

Traversal cardinality is not a new taxonomy. Zero assignments, multiple assignments, or a compiled-profile identity mismatch all return `context_provenance_mismatch` (`provenance`, non-retryable, manual false). `assignment_count_zero`, `assignment_count_multiple`, and `compiled_profile_identity_mismatch` are offline assertion reasons only: they are not `AiErrorCode`, telemetry, persistence, Phase C projection, or consumer-visible branches. A protected M02 rejection remains `context_prohibited_data`. This mapping is direct in the one profile; no implicit conversion or second error table exists.

The proof contract has exactly two non-interchangeable layers:

1. **Layer A — full 69-code authority closure (normative runtime proof).** The offline verifier imports `aiErrorCodes` and calls `aiFailure` for every source member. It requires the profile derivative to match all 69 codes in source order with zero missing, extra, duplicate, or reordered members, and requires exact `code`, `category`, `retryable`, `manualEditorAvailable`, and default `safeMessage` equality. Profile missing/extra/reorder/projection drift are Fresh executed failures. This is the only full-authority/profile closure proof.
2. **Layer B — selected traversal subset (normative TypeScript proof).** [ERROR_TAXONOMY_TWO_LAYER_POSITIVE_PROBE_V1_0.ts](review-evidence/phase-1b-stage4a-phase-b-corrected-exact-design-v1-10-v18-m01-m03-attempt-2-v1/ERROR_TAXONOMY_TWO_LAYER_POSITIVE_PROBE_V1_0.ts) fixes exactly `context_provenance_mismatch`, `context_prohibited_data`, `context_too_large`, `canonicalization_failed`, and `internal_failure` with `as const satisfies readonly AiErrorCode[]`. TypeScript proves only that every selected traversal member belongs to `AiErrorCode` and that the unknown catch returns `internal_failure`. It does not require the other 64 source codes to belong to the traversal subset.

Normal `resolveJsonModule` widens profile code strings, so V1.10 makes no claim that importing JSON directly creates a 69-member literal union. It also forbids the inverse `Exclude<AiErrorCode,TraversalReturnCode> extends never`. Three independent negative fixtures must actually fail TypeScript: restoring that inverse, pretending the widened JSON string is `AiErrorCode`, and adding an unknown traversal member. The runtime verifier materializes each fixture only for the local direct compile, records the nonzero result, and removes the temporary `.ts`; the checked fixture bytes remain evidence. There is no generated `as const` error authority and no hand-maintained second tuple of all 69 codes.

Manual degradation is represented as the same typed result only for the exact true set above. The UI later maps these to a fixed message and leaves all ordinary edit/save/review controls intact. There is no empty “AI result” placeholder or automatic Draft change.

## 18. Normative Service sequences and Phase C handoff

The sequences in this section are the only normative ordering. Availability (§18.2) is a read-only readiness query, preparation/enqueue (§18.3) is a governed transaction that may return an exact replay or insert `pending`, and claimed replay (§18.4) begins only after Phase C claim/dispatch-marker prerequisites. Text about one sequence cannot reorder another. The machine profile is the one claimed-replay order authority; §13.2.3, §18.4 and the claimed rows of §18.5 are mechanically checked projections of `CR-01..CR-14`, not independent arrays. Canonical statuses remain exactly `pending`, `processing`, `draft_ready`, `failed`, `cancelled`; retry state is `none`, `scheduled`, `exhausted`, `not_retryable`. `succeeded` and `dead` are invalid.
...
Claim availability is a Phase C precondition, not part of availability (§18.2), preparation/enqueue (§18.3), or the replay-validation order below. Before entering `CR-01`, the Worker must claim one due `ai_runs`, commit `processing`/lease, then in a fenced transaction validate cancellation/lease and atomically commit `actual_provider=requested_provider`, `active_attempt_dispatched_at`, first-fill `provider_dispatched_at`, and increment `state_version`. It then reads the exact accepted durable row projection. No network occurs before the marker commits. This precondition cannot reorder or skip any `CR` step.

The following is the sole claimed replay numbered sequence. The step IDs, order, operation names, owners, and call counts are a rendered projection of `machineProfile.executionOrders.claimedReplay`; §13.2.3 and the §18.5 claimed matrix must reproduce the same IDs in the same order:

1. `CR-01` — strict-parse the claimed projection, processing/lease/retry/attempt/state/dispatch markers and abort state; require actual Provider equals requested Provider. Adapter calls: `0`.
2. `CR-02` — resolve exactly one `OpaqueClaimedApplicationRuntimeV1` from the durable application/use-case/input+output-schema/policy tuple through `ClaimedApplicationRuntimeRegistryV1`; construct the claimed brand, never a request. Adapter calls: `0`.
3. `CR-03` — call the application closure to strict-decode only the accepted durable association columns and the unchanged context association tuple, with no hash acceptance yet; reject type/identity/version/alias disagreement. Adapter calls: `0`.
4. `CR-04` — fatal-decode and strict-parse the unchanged `input_context_json` outer shape and accepted recursive JSON value grammar. Adapter calls: `0`.
5. `CR-05` — compile the exact field-domain profile identity and exact 35 assignments/three domains; any byte/assignment/demotion/overlap drift returns `context_provenance_mismatch`. Adapter calls: `0`.
6. `CR-06` — recursively traverse every context object, array, key and value, requiring exactly one assignment per node; zero/multiple use the same `context_provenance_mismatch`. Adapter calls: `0`.
7. `CR-07` — execute every assigned domain validator, including exact machine grammars and the one selected M02 identity over each complete protected root; any protected payload rejects. Adapter calls: `0`.
8. `CR-08` — rebuild the authorized target snapshot, JCS-canonicalize, recompute/compare association SHA-256 and all durable/context copies, then recompute/compare full-context JCS `input_hash`; return the unchanged original context. Adapter calls: `0`.
9. `CR-09` — reconstruct the exact Phase A resolved-config JCS object—including `application_class` and excluding `actual_provider`—and verify stored hash, limits, fallback absence, requested/actual equality and immutable provenance. Do not read current default/config or resolve an adapter. Adapter calls: `0`.
10. `CR-10` — load exact raw Prompt bytes by stored tuple, recompute the raw hash, strict-parse, and require manifest/application/capability/use-case/schema/policy agreement. Adapter calls: `0`.
11. `CR-11` — require the compiled Provider-envelope version/hash to equal the stored tuple; no alternate envelope/provider/model. Adapter calls: `0`.
12. `CR-12` — resolve the exact requested adapter once, validate model/parameters through only its Provider-neutral policy, call the captured pure Prompt-variable projection, rebuild the Provider-neutral request, and recheck byte/token/output-schema ceilings. Adapter calls: `0`.
13. `CR-13` — invoke `TextAiProvider.generateText` exactly once from the sole generic core call site. No fallback, retry-to-provider, second adapter, tool, retrieval or alternate model exists. Adapter calls: exactly `1`.
14. `CR-14` — after return recheck abort and lease expiry; enforce returned model/completion, raw-parse exactly one object, run strict output schema and the captured application's output policy including the same M02 identity at A-07, and return the normalized attempt to the Worker. Worker alone performs fenced lifecycle persistence. Additional adapter calls: `0`.
...
Tamper tests independently change Prompt raw bytes/manifest tuple, Prompt hash, recursively nested context value/key/order/ref, field-domain assignment/profile identity, input hash, every config-hash field including `application_class`, pending/dispatch actual-Provider nullability, requested/actual Provider, parameter/limit/model, resolved config hash, envelope version/hash, policy/schema version, every target type/FK/locale/version/hash field, and an injected association/snapshot/pre-rendered request. Every case before `CR-13` fails with the corresponding closed code and adapter call count zero. Literal Product/Content/Revision round trips and the fixed config/dispatch vector each make one call only after `CR-01..CR-12` succeeds.

### 18.5 Cross-contract static caller and ownership audit

This matrix is normative for callers, owners, scopes and counts. For claimed replay, row order and operation identity are a verified projection of the one machine `executionOrders.claimedReplay` authority; the matrix cannot independently reorder a `CR` step. “DB count” is the maximum operation introduced by that row; context class batches and unique-loser fetches are separately named. No row requires a member absent from its static type.
...
| CR-01 | core -> strict claimed projection validator | generic claimed core | durable processing/lease/retry/attempt/state/dispatch projection | DB `0`; adapter `0` | valid claim authority | CR-02 |
| CR-02 | core -> `ClaimedApplicationRuntimeRegistryV1.resolve` | application-neutral claimed registry | durable application/use-case/schema/policy tuple | DB `0`; adapter `0` | exact `OpaqueClaimedApplicationRuntimeV1` | CR-03 |
| CR-03 | core -> runtime `decodeClaimedAssociation` plus context-association tuple decode | application-owned claimed closure | seven accepted target columns plus unchanged five-field context tuple | DB `0`; adapter `0` | strict unhashed association agreement | CR-04 |
| CR-04 | runtime -> outer context/recursive-JSON strict parser | application-owned claimed closure | original `input_context_json` bytes/value | DB `0`; adapter `0` | strict unchanged outer context | CR-05 |
| CR-05 | runtime -> compiled field-domain profile identity gate | one profile compiler | exact machine profile only | DB `0`; adapter `0` | exact 35 assignments/three domains/identity | CR-06 |
| CR-06 | runtime -> recursive field-domain traversal | one context traversal | every object/array/key/value | DB `0`; adapter `0` | zero missing/zero duplicate coverage | CR-07 |
| CR-07 | traversal -> assigned validators + selected M02 | application validators + exact selected classifier identity | each assigned original node/root | DB `0`; adapter `0` | structurally valid and protected-clean context | CR-08 |
| CR-08 | runtime -> target snapshot/JCS/SHA plus full-context JCS/input-hash validator | application association authority + generic canonical JSON | accepted target columns and unchanged context | DB `0`; adapter `0` | verified association and `OpaqueClaimedContextStageV1` | CR-09 |
| CR-09 | core -> resolved-config reconstruction/hash validator | generic claimed core | durable config fields; no current config read | DB `0`; adapter `0` | verified exact config provenance | CR-10 |
| CR-10 | core -> Prompt raw-byte loader/validator | Prompt byte authority | stored Prompt tuple | DB `0`; adapter `0` | verified raw Prompt/contract | CR-11 |
| CR-11 | core -> envelope identity validator | Provider-neutral registry | stored and compiled envelope tuples | DB `0`; adapter `0` | verified exact envelope | CR-12 |
| CR-12 | core -> exact adapter policy resolution + context-stage projection/renderer | generic core + exact adapter policy + captured pure context closure | no DB/scope; config/Prompt/envelope already verified | DB `0`; adapter call `0` | exact Provider-neutral request | CR-13 |
| CR-13 | core -> `TextAiProvider.generateText` | sole generic core call site | no DB transaction | adapter call exactly `1` | normalized untrusted result | CR-14 |
| CR-14 | core completion/raw parser -> context-stage `parseAndProtect` -> Worker normalized return | generic core + captured application output closure | no DB/scope | adapter `0` additional; Worker later fences persistence | protected result or typed failure | authorized Phase C lifecycle writer |
...
`scripts/verify-ai-architecture.ts` first compiles the V2.2 filesystem and selector AST. It performs one raw, sorted actual-filesystem `lstat` walk independent of Git index/ignore state. Only the 11 exact top-level physical entries in Section 2.3 are not walked; `.git` may be its worktree pointer file, every other present exclusion must be a physical directory, and symlink/special-file/hard-link/canonical collisions fail before candidate filtering. It then derives tracked/untracked/ignored metadata without using it as a scope gate, hashes every candidate's raw bytes, evaluates all 12 mutually exclusive selectors, and fails on zero or multiple matches. In the official lifecycle proof the same compiled verifier runs fresh in one detached worktree immediately before and after local Next `16.2.12` `typegen`; it does not change selector data, consult a generated-file compatibility rule, or request cleanup.

The same script then creates one TypeScript Program from repository `tsconfig.json` for included TypeScript modules, parses any remaining JS/MJS/CJS candidate with `ts.createSourceFile` and the matching ScriptKind, and uses the same inventory for JSON/generated resources. It scans:
...
- all Production code `src/**/*.{ts,tsx,mts,cts,js,jsx,mjs,cjs}` whose V2.2 class/stage is Production, excluding every declared test semantic and `src/ai/testing/**` from the Production module graph, plus classified JSON/resources in the same inventory;
...
- test/Synthetic/generated fixtures in separate test/isolation scans, including `src/test/**`, `tests/**`, `test-fixtures/**`, all declared `.test.*`/`.integration.test.*`/`.spec.*` suffixes and setup/teardown reached by those bounded roots;
- all ten exact root-control candidates, Prompt manifests/resources, generated files and bundle scripts.

The matcher vocabulary, candidate suffix/root sets and every exclusion live only in V2.2. The verifier may compile them but may not carry another directory table, test-suffix table, root-config allowlist or priority fallback. Profile schema/version, integrity seal, unknown fields and unsupported operators fail closed before walking. Every class is reported even when its member list is empty.

Resolution uses parsed compiler options and `ts.resolveModuleName`, then `ts.sys.realpath` and repository-relative POSIX path canonicalization. It handles `@/*` aliases, relative paths, explicit/implicit extensions, directory `index`, package exports, and the already symlink-free actual inventory; an alias to a different lexical node, path escape, canonical/inode collision or unresolved protected-zone edge fails. Package edges canonicalize to the top-level package name. Graph traversal follows all re-exports transitively with visited/cycle tracking.
...
- the exact original six `V15-M01` paths retain their Section 2.3 sole classes; `next-env.d.ts`, if present, has only `root-control-file`; every `src/test/**` node is test-only; and no test/root-control node is reachable from a Production/public/server entry. Test/config/framework-declaration classification grants no AI Provider/adapter, external Provider network, credential/endpoint or Production runtime authority;
- both detached lifecycle states record sorted members for all 12 classes, empty `zeroClass`/`ambiguous`, physical-exclusion status and inventory/content/classification hashes. The present state additionally records the exact ignored/generated `next-env.d.ts` bytes/class/tsconfig facts and the separately audited `.next/types` file set/generated-root hash. Adding an unknown root executable, silently adding an exclusion, using Git ignore to hide a file, requiring deletion, aliasing `.next`, or changing a matcher without updating the one sealed V2.2 profile fails before module analysis;
...
- **No implementation leakage from corrected Design:** the V1.7 Candidate delta from `377181cd...` contains only versioned `docs/` and `docs/review-evidence/` artifacts; accepted V1.4, escalation V1.1, imported PASS, source, Schema/Migration, ADR, package and lock bytes remain unchanged.
...
| IMP2-NM01 field-domain traversal | materialize every required/optional/container and recursively nested object/array/key/value node; remove/duplicate each assignment; add unknown/overlap; demote any protected assignment before compile and mutate both protected assignments on the original source after compile | one input-detached recursively frozen compiled closure; every node exactly one domain; zero/multiple/profile drift return only `context_provenance_mismatch`; pre-compile mutations fail compilation, post-compile source mutations cannot affect the closure, and protected payloads still produce actual caught `context_prohibited_data` failures |
| recursive evidence grammar | nested object-in-array-in-object, array-in-object-in-array, special keys, object-key permutation, array-order permutation, remove/duplicate/unknown/overlap, depth/node/unsupported-value boundary | complete recursion under one protected root; object permutations share JCS but preserve distinct original bytes, array permutation changes JCS; every key/string is scanned; no shallow primitive-array/one-level-object compatibility path |
| protected evidence coverage | phone, email, credential, DeepSeek/provider override, Inquiry/private-data payload at every task narrative, source field label, arbitrary-depth source value key/string, and link label | exact selected M02 identity rejects every protected placement; runtime/registry/profile mutation fails; no consumer exception or second scanner |
| closed error taxonomy | all 69 `aiErrorCodes`; every profile code/category/retry/manual projection; traversal return-code subset; zero/multiple/profile drift; unknown thrown value | profile tuple is byte/order-equal to unique source tuple; `aiFailure` projection equals every entry; traversal returns only closed codes; zero/multiple/profile drift map directly to `context_provenance_mismatch`; unknown alone maps `internal_failure` |
| machine metadata grammar | arbitrary text in every top-level literal/enum, association field, tone, source alias/class/selector/ref/provenance, link ref, and media ref | strict structural parsing rejects; machine domain cannot accept arbitrary safe-looking text |
...
| snapshot-hash domain | deterministic 1,000 Revision UUID/version snapshots; Reviewer `b574a4fc...`; lowercase/tamper/length/algorithm/snapshot-version/target/version/replay mutations | valid `1000/1000` accept regardless decimal digit runs; Reviewer hash accepts only on exact recomputation; every mutation fails before Prompt/adapter; snapshot hash receives zero lexical calls |
| reconstructible context | preparation -> persisted JSONB-shaped round-trip -> typed domains -> claimed parse/variables/request; target/context separation; all four projection key sets | identical persisted context bytes/JCS/input hash and request bytes; target hash covers identity/version only; association metadata and `input_sources_json` never enter Provider projection |
...
| claimed order projection | profile `CR-01..CR-14`, §13.2.3, §18.4 numbered list, and §18.5 claimed rows; swap context after config or adapter before envelope | all four projections exactly equal; `CR-03..CR-08` complete before config/Prompt/envelope/adapter resolution; config/Prompt/envelope complete before `CR-13`; every swap fails verifier |
...
- Run `VERIFY_CORRECTED_EXACT_DESIGN_V1_10.mjs` offline under the exact selected runtime tuple. It must compile one repository-reviewed profile into an input-detached recursively frozen module-private closure; prove the full 69-code runtime projection and the distinct five-code TypeScript subset; run all three independent TypeScript negatives; prove no exported source/assignment/domain reference; reject direct compiled method/summary/prototype mutation, custom-prototype and alias-sharing inputs; and execute both Reviewer post-compile demotions against `/task/guideIntent` and `/sources/*/fields/*/value/**`, with both protected emails rejected. It must extract the same `CR-01..CR-14` order from §13.2.3/§18.4/§18.5 and prove the profile execution-order subtree plus §18.4/§18.5 are byte-identical to V1.9. It recursively enumerates nested object/array evidence, reports zero missing/duplicate nodes, accepts 1,000/1,000 deterministic valid Revision hashes, accepts the Reviewer hash only on recomputed equality, and rejects every integrity/domain/demotion/protected/machine/order/projection mutation through an executed assertion. The parent runs a child self-test that deliberately restores V1.9's mutable-source/cache shortcut and requires child exit `97`; the two corrected Reviewer probes run in the parent and may never exit `97`. Persisted bytes/JCS/`input_hash`, no association leak, one selected M02 identity, M03, and `21/21`/`96/96` remain non-regressed. Two fresh normal outputs must be byte-identical. This is design-consistency proof, not an implementation claim.
- Run the V1.7 verifier under the exact Node/V8/ICU/Unicode/CLDR tuple in a disposable detached worktree at one exact proof checkpoint before generation as `source-clean-file-absent`. Its raw output must list every class member, total executable/candidate counts, empty zero/ambiguous arrays, zero ignored candidates, absent `next-env.d.ts`/`.next`, excluded-root/source-state/composition/Phase D/adapter status and stable inventory/content/classification/generated-root hashes.
- Using the installed Next `16.2.12` public CLI, run offline `next typegen .` in that same disposable worktree. Require exit `0`, then rerun the same verifier as `official-next-generated-file-present`: `next-env.d.ts` must be ignored, tsconfig-included, exact bytes, generated and sole `root-control-file`; `.next/types` must match the exact separately audited three-file set; zero/ambiguous remain empty. Run the same CLI a second time and require identical bytes and hashes. Do not modify, generate in, delete from, or clean the formal Candidate worktree.
- Retain all 17 V1.6 in-memory mutations and run all 11 Attempt-2 cases: next-env no class/double class/silent ignore-or-exclude/other ignored root TS/symlink/hard-link/byte/path/type-reference/deletion prerequisite plus `.next` alias escape. Every case must fail with its exact reason; changing priority, Git ignore state, lifecycle order or deletion cannot make a failure pass.
- Re-run M02's full Unicode transition challenge and runtime mismatch negative, the selected database branch-local positive plus unnarrowed-union/cross-driver/destructured negatives, the exact 21/21 + 96/96 Schema map, all 13 historical closures and V1.5/V1.6 byte-identity checks.
...
- the owned-file whitespace check and design scope gate show no historical Prompt mutation, preserve all V1.9 and older bytes, and require the V1.10 delta itself to contain docs/evidence only; immutable imported Reviewer hard-break bytes are reported separately rather than rewritten or misreported.
...
Only after the exact V1.10 commit receives Fresh Independent Design Review PASS and the Coordinator explicitly opens implementation correction attempt 3 may the following implementation order be used:
...
## 23. Exact V1.10 design acceptance checklist

The Fresh Independent Design Review may return PASS only if all are true:

- [ ] exact V1.10 branch/start/V1.9 parent/V1.8 ancestry/tag identity passes; checkpoint remains pinned to `b7ad96b...` before and after design; formal worktree is clean;
- [ ] checkpoint record is the first and sole artifact in commit `a90d642...`; V2.2 report/evidence/patch/output/manifest are byte-identical and its imported manifest passes 4/4;
- [ ] accepted V1.7 and immutable V1.8/V1.9 design/profile/vectors/verifier/capture/manifests remain byte-identical; V1.10 is standalone and the delta is exact docs/evidence-only scope;
- [ ] V1.10 accounts for every preserved `H-01`, `H-02`, `M-01..M-06`, `L-01`, and `N-M01..N-M04` closure plus exact IMP2-NH01/M01/M02/M03/M04/M05/L01/L02/NM01 review dispositions without implementation claims;
- [ ] V18-M01 is closed by the two-layer contract: runtime exact equality of all 69 `aiErrorCodes`/`aiFailure` projections, and TypeScript assignability of only the exact five traversal members; the inverse and JSON-literal derivation are explicitly false and fail Fresh negative probes; zero/multiple/profile-identity failures use only `context_provenance_mismatch`, and unknown alone maps `internal_failure`;
- [ ] V18-M02 is closed by one profile `CR-01..CR-14` authority; §13.2.3, §18.4 and §18.5 have identical IDs/order, context/integrity precedes config/Prompt/envelope/adapter resolution, and the sole adapter call is `CR-13`;
- [ ] V18-M03 is closed by one input-detached recursively frozen module-private compiled closure; traversal accepts no raw profile override; both Reviewer post-compile demotions still reject protected emails; method/summary/prototype/source-alias mutations fail; complete recursive object/array/key/value coverage remains; and the child that restores the exact mutable-source/cache shortcut exits `97` while corrected probes do not;
- [ ] one profile `ctx.draft-assistance.v1.field-domain-traversal.v1` assigns every strict context node exactly one of three domains; `missing=[]`, `duplicate=[]`; unknown/new fields, duplicate assignments, path exception, compatibility traversal and consumer bypass all fail closed;
- [ ] the complete association object has exact kind/type/alias/version/hash validators; SHA-256 is recomputed from the authorized version-1 JCS target snapshot and compared to durable/context copies before acceptance and replay;
- [ ] deterministic valid Revision hashes are `1000/1000` accepted regardless decimal digit runs; Reviewer `b574a4fc...` accepts only for the exact recomputed target/version; tamper/uppercase/length/algorithm/snapshot-version/target/version/replay mismatches all fail;
- [ ] every task narrative, source value/key/field label, Product/Fabric/Company Fact value, explicit human input and internal-link label invokes the selected M02 identity; phone/email/credential/Provider/private-data mutations reject at every protected pattern;
- [ ] every machine literal/enum/alias/ref/version/hash rejects arbitrary text structurally; machine metadata is not a general string exemption and no association field reaches a Provider projection;
- [ ] `input_context_json` shape/original bytes, full-object JCS/`input_hash`, and `input_sources_json` purpose remain unchanged against fixed vectors; claimed replay repeats integrity and lexical domain checks before adapter resolution;
...
- [ ] selected M03 authority is the unchanged V2.2 profile SHA-256 `1f0b56a8...` sealed in the V1.7 manifest; the unchanged 12 classes, closed selector compiler, complete protected/excluded roots, exact five-edge Phase B root, reserved absent Phase D root/adapter zone and empty Production Provider registry pass;
- [ ] raw actual-filesystem inventory includes tracked/untracked/ignored candidates independent of Git scope, handles exact physical roots/symlink/hard-link/canonical/generated rules, prints every class member, and proves `zeroClass=[]`, `ambiguous=[]` plus inventory/content/classification hashes;
- [ ] `drizzle.config.ts`, `playwright.config.ts`, and `vitest.config.mts` retain only `root-control-file`; all three `tests/e2e` paths and every `src/test/**` node retain only `other-test-fixtures`; test/control classes grant no Provider/adapter/external Provider network/credential/Production runtime authority and are unreachable from Production/public/server entries;
- [ ] the same exact detached checkpoint passes a fresh source-clean absent inventory, official Next `16.2.12` ignored-present inventory, and repeated generation; present `next-env.d.ts` is exact bytes, tsconfig-included, generated and only `root-control-file`; absent/present each have zero/ambiguous empty; `.next/types` exact file/hash audit passes; no deletion/cleanup/manual synthesis exists;
- [ ] all 17 V1.6 mutation negatives and all 11 Attempt-2 mutations fail closed, including each original six disposition removal, next-env class/ignore/alias/content/path/reference/lifecycle cases, other ignored root TS, `.next` bypass, early Phase D/adapter and second composition root;
...
V1.0 consolidated Provider mechanics but left Draft association/result concepts in core and several integrity checks implicit. V1.1 moved application semantics outward; V1.2 closed target/config/read/evidence gaps. V1.3 removed the single common-scope authorization path and assigned availability/read versus request/lock to two application-owned binders. Fresh Re-review then exposed one redundant authority marker: the generic module's private brand could not be populated by application modules. V1.4 deleted that generic brand and received independent PASS. V1.5 preserved that design, selected the one M02 registry and made the actual PGlite/Postgres outer handoff explicit, but its complete-graph matcher left six accepted executables with zero classes. V1.6 retained the 12 classes and closed those six, but proved only a file-absent snapshot; normal Next type setup generates ignored root `next-env.d.ts`, which the same selector left unclassified. V1.7 fixed that causal lifecycle boundary by giving the framework declaration one finite existing-class disposition and proving both real states with the same sealed selector. The failed implementation then revealed IMP2-NM01: V1.7's instruction to classify the whole accepted context conflated application-generated cryptographic integrity metadata with natural language, so valid hashes collided with a phone grammar. V1.8 moved responsibility to one typed application schema traversal, but its design package left two non-union error names, two contradictory claimed orders, and a verifier that counted an accepted demotion as detected while modeling only shallow evidence. V1.9 directly reused `context_provenance_mismatch`, established one machine claimed order, and added a profile identity gate, but its evidence falsely described a widened JSON import as a 69-member TypeScript union and returned the mutable source object from a compile cache. V1.10 removes both causal defects: the 69-code runtime proof and five-code TypeScript subset are explicit separate layers, and compilation produces one detached recursively frozen closure that traversal alone consumes. It changes neither Owner-selected M02/M03 seam, V18-M02, nor persistent architecture.
...
Maintainers must keep application codec/policy/schema versions, both binders, and same-module scope factories aligned; retain the three target/hash and config/dispatch vectors; preserve construction/assignability/negative fixtures; maintain the finite A-01–A-10 lexicon and human-review boundary; append Prompt manifest/resources and regenerate the checked derivative; maintain RFC/raw-parser vectors; version the one context-domain profile whenever a strict context field is added; and version the one M03 selector profile whenever a genuinely new executable root/control/test semantic appears. The zero/ambiguous field gate and raw actual-tree gate make both reviews visible instead of silently broadening a domain/class. This cost is bounded and application-local. A new use case reuses its application's factories and adds one definition; a new application supplies its own private scope module, typed registry/binders/codecs, and forward persistence support; a Provider adds one adapter/evidence set. None requires adding a generic base factory or core switch.
...
- the one context-domain traversal owns field-to-validator assignment. The M02 registry owns lexical protected-data language; association snapshot/JCS authority owns cryptographic integrity. Neither may infer the other's domain, and there is no second traversal/scanner/exception list.
...
Phase B increases compiled validation/test maintenance but adds no cross-process/persistent coordination. V1.3's two explicit entry compositions remain; V1.4 deleted one unusable generic brand and gave each application one direct construction seam. V1.5 selected one compiled classifier authority and one exhaustive existing-union switch. V1.6 retained the 12-class selector; V1.7 added one exact member to its existing root-control class plus a two-state offline proof. V1.8 replaced one overly broad traversal contract with one typed domain traversal. V1.9 corrected the taxonomy mappings and claimed order. V1.10 deletes the false JSON-literal/inverse claim and replaces the mutable-source cache with one closed immutable value; it adds no classifier, error map, generated 69-code tuple, copied order, exception path, runtime choice, database, wrapper, Provider, adapter, dependency, persistent state, Schema, Migration or ADR. The optional compile-once reference is only a module-private sealed-product singleton, not a source cache or second authority. Phase C retains the already approved operational complexity without a fake, duplicate authorization, generic factory hierarchy or application-specific core path.

## 26. Design conclusion and next gate

Conclusion: **CORRECTED EXACT DESIGN CANDIDATE V1.10 / V18-M01 AND V18-M03 ATTEMPT-2 REMEDIATED / V18-M02 FROZEN CLOSED / OWNER IMP2-NM01 DIRECTION PRESERVED / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED.**

The next and only gate is **Fresh Independent Corrected Design V1.10 Review by the original Phase B Design Reviewer** against the exact V1.10 commit, imported byte-identical V1.9 FAIL package, immutable checkpoint, IMP2-NM01 Owner Decision Record, fixed V1.7/V1.8/V1.9 authorities, selected M02 and unchanged M03 V2.2 profile, the single V1.10 machine profile, detached/frozen compiler proof, recursive verifier/mutations, honest two-layer TypeScript/runtime proof and captured output. Implementation correction attempt 3 must not start until that review returns PASS and a later task explicitly opens it. No self-review, Provider call, credential, network, Staging/Production, Deploy, formal import, Publish, Index, merge or Push is authorized or performed by this design.
