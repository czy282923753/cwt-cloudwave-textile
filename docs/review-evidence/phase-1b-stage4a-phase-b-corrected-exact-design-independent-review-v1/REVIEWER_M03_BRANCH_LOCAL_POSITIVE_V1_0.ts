...
export function constructReviewerService(): DraftAssistanceService {
  ...
  switch (databaseConnection.kind) {
    case "pglite": {
      const service = createPhaseBAvailabilityServiceV1({ database: databaseConnection.db, trustedEnvironment });
      return service;
    }
    case "postgres": {
      const service = createPhaseBAvailabilityServiceV1({ database: databaseConnection.db, trustedEnvironment });
