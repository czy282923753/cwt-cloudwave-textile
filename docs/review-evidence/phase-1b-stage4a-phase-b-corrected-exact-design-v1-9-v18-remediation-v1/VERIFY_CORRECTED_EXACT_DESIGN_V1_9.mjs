...
const profilePath = resolve(here, "V18_M01_M02_M03_MACHINE_PROFILE_V1_0.json");
const vectorsPath = resolve(here, "V18_M01_M02_M03_FIXED_VECTORS_V1_0.json");
const typeProbePath = resolve(here, "ERROR_TAXONOMY_CLOSED_UNION_PROBE_V1_0.ts");
const designPath = resolve(root, "docs/PHASE_1B_STAGE4A_PHASE_B_PROVIDER_NEUTRAL_FOUNDATION_CORRECTED_EXACT_DESIGN_V1_9.md");
...
const reviewManifestPath = resolve(root, "docs/review-evidence/phase-1b-stage4a-phase-b-independent-corrected-design-review-v1-8/SHA256SUMS.txt");
...
const SEALED_PROFILE_IDENTITY = "48d2b635a4719a20c2f3210e81e8bb4512745c0351a5bb8a7e66201b5f099026";
...
function identityProjection(candidateProfile) {
  return Object.fromEntries(candidateProfile.compiledAuthority.identityProjectionKeys.map((key) => [key, candidateProfile[key]]));
}
...
let baselineProfileCompiled = false;
function compileProfile(candidateProfile) {
  if (candidateProfile === profile && baselineProfileCompiled) return candidateProfile;
  if (candidateProfile.compiledAuthority.expectedSha256 !== SEALED_PROFILE_IDENTITY) fail("context_provenance_mismatch", "sealed_identity_changed");
  const recomputed = sha256(jcs(identityProjection(candidateProfile)));
  if (recomputed !== SEALED_PROFILE_IDENTITY) fail("context_provenance_mismatch", "compiled_profile_identity_mismatch");
  if (candidateProfile.traversalAuthority.closed !== true) fail("context_provenance_mismatch", "traversal_not_closed");
  for (const key of ["pathExceptionLists", "compatibilityTraversals", "consumerLocalBypasses"]) {
    if (candidateProfile.traversalAuthority[key] !== 0) fail("context_provenance_mismatch", key);
  }
  if (candidateProfile.protectedClassifier.secondClassifierAllowed !== false || candidateProfile.protectedClassifier.consumerExceptionAllowed !== false) fail("context_provenance_mismatch", "classifier_authority");
  const patterns = candidateProfile.contextNodeAssignments.map(({ pattern }) => pattern);
  if (new Set(patterns).size !== patterns.length) fail("context_provenance_mismatch", "assignment_pattern_duplicate");
  if (patterns.filter((pattern) => pattern.includes("**")).length !== 1 || !patterns.includes("/sources/*/fields/*/value/**")) fail("context_provenance_mismatch", "recursive_pattern");
  for (const assignment of candidateProfile.contextNodeAssignments.filter(({ domain }) => domain === "protected_human_business_provider_evidence")) {
    if (assignment.scanRoot !== true) fail("context_provenance_mismatch", assignment.pattern);
  }
  verifyErrorTaxonomy(candidateProfile);
  verifyOrderAuthority(candidateProfile);
  for (const projection of Object.values(candidateProfile.promptProjections)) {
    for (const sourcePath of Object.values(projection)) {
      if (candidateProfile.providerLeakDenyPrefixes.some((prefix) => sourcePath === prefix || sourcePath.startsWith(`${prefix}/`))) fail("context_provenance_mismatch", "provider_projection");
    }
  }
  if (candidateProfile === profile) baselineProfileCompiled = true;
...
function validateContext(candidateProfile, context, external, options = {}) {
  strictOuterShape(context, candidateProfile.recursiveEvidenceGrammar);
  const compiled = options.skipCompiledIdentity === true ? candidateProfile : compileProfile(candidateProfile);
  const coverage = verifyCoverage(compiled, context);
  const scans = scanProtectedDomains(compiled, context);
  validateDomainStructure(compiled, context);
  const associationHash = validateAuthorizedAssociation(context, external, compiled);
...
function runTypeProbe() {
  const tsc = resolve(root, "node_modules/.bin/tsc");
  const result = spawnSync(tsc, [
    "--noEmit", "--strict", "--target", "ES2022", "--module", "NodeNext", "--moduleResolution", "NodeNext",
    "--resolveJsonModule", "--esModuleInterop", "--skipLibCheck", typeProbePath,
  ], { cwd: root, encoding: "utf8" });
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`);
}

function simulateOldFalsePositiveCounter() {
  ...
  try {
    validateContext(candidate, context, external, { skipCompiledIdentity: true });
  } catch {
    process.exit(0);
  }
  console.error("OLD_FALSE_POSITIVE_DEMOTION_COUNTER_RESTORED:accepted_payload_would_be_counted");
  process.exit(97);
}

if (process.argv.includes("--simulate-old-false-positive-counter")) simulateOldFalsePositiveCounter();
...
const reviewManifestCount = verifyManifest(reviewManifestPath);
assert.equal(v18ManifestCount, 21);
assert.equal(reviewManifestCount, 7);
...
for (let index = 0; index < vectors.deterministicRevisionCorpus.count; index += 1) {
  ...
  validateAuthorizedAssociation(contextForExternal(external), external, profile);
}
...
for (const [id, mutate] of [
  ...
  mutate(context);
  expectFailure(() => validateContext(profile, context, reviewerVector));
...
for (const [id, mutate] of [
  ...
  mutate(external);
  expectFailure(() => validateContext(profile, external.context, external));
  malformedLabels.push(id);
}
...
for (const assignment of protectedAssignments) {
  for (const payload of vectors.protectedPayloads) {
    ...
    assert.equal(hashed.ok, true);
    external.expectedInputHash = hashed.value.hash;
    const rejection = expectFailure(() => validateContext(profile, context, external), "context_prohibited_data");
...

{
  ...
  detect("assignment_removed", () => validateContext(candidate, reviewerVector.context, reviewerVector), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  ...
  detect("assignment_duplicated", () => validateContext(candidate, reviewerVector.context, reviewerVector), "context_provenance_mismatch");
}
{
  const context = clone(reviewerVector.context);
  context.futureUnknown = "must fail";
  detect("unknown_context_field", () => validateContext(profile, context, reviewerVector), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  ...
  detect("overlapping_assignment", () => validateContext(candidate, reviewerVector.context, reviewerVector), "context_provenance_mismatch");
}
for (const [label, domain] of [["protected_to_machine_demotion", "machine_structural_integrity"], ["protected_to_closed_container_demotion", "closed_container"]]) {
  ...
  const context = clone(reviewerVector.context);
  context.task.guideIntent = vectors.protectedPayloads.find(({ id }) => id === "email").value;
  detect(label, () => validateContext(candidate, context, reviewerVector), "context_provenance_mismatch");
  expectFailure(() => validateContext(profile, context, reviewerVector), "context_prohibited_data");
}
{
  const context = clone(reviewerVector.context);
  context.sources[0].fields[0].value = vectors.recursiveEvidenceVector.protectedNestedPayload;
  detect("recursive_nested_protected_value", () => validateContext(profile, context, reviewerVector), "context_prohibited_data");
}
{
  const context = clone(reviewerVector.context);
  context.sources[0].fields[0].value = { [vectors.recursiveEvidenceVector.protectedNestedKey]: "Synthetic value" };
  detect("recursive_nested_protected_key", () => validateContext(profile, context, reviewerVector), "context_prohibited_data");
}
...
validateContext(profile, recursiveContextA, recursiveExternalA);
...
detect("recursive_object_key_permutation", () => assert.equal(JSON.stringify(vectors.recursiveEvidenceVector.valueA), JSON.stringify(vectors.recursiveEvidenceVector.valueB)));
{
  ...
  detect("recursive_array_order_permutation", () => validateContext(profile, context, recursiveExternalA), "context_provenance_mismatch");
}
{
  const context = clone(recursiveContextA);
  delete context.sources[0].fields[0].value;
  detect("recursive_value_removed", () => validateContext(profile, context, recursiveExternalA), "context_provenance_mismatch");
}
{
  const context = clone(recursiveContextA);
  context.sources[0].fields.push(clone(context.sources[0].fields[0]));
  detect("recursive_value_duplicated", () => validateContext(profile, context, recursiveExternalA), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  ...
  detect("profile_identity_forgery", () => compileProfile(candidate), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  candidate.protectedClassifier.secondClassifierAllowed = true;
  detect("second_classifier", () => compileProfile(candidate), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  candidate.protectedClassifier.consumerExceptionAllowed = true;
  detect("consumer_exception", () => compileProfile(candidate), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  candidate.promptProjections.sourcing_guide_draft.association = "/association";
  detect("prompt_association_leak", () => compileProfile(candidate), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  candidate.protectedClassifier.registryVersion = "2.1.1";
  detect("registry_identity_drift", () => compileProfile(candidate), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  ...
  detect("claimed_order_swap_context_after_config", () => compileProfile(candidate), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  ...
  detect("claimed_order_adapter_before_envelope", () => compileProfile(candidate), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  candidate.errorTaxonomy.traversalReturnCodes.push("context_domain_unclassified");
  detect("taxonomy_unknown_return_code", () => compileProfile(candidate), "context_provenance_mismatch");
}
{
  const candidate = clone(profile);
  candidate.errorTaxonomy.codes.find(({ code }) => code === "context_provenance_mismatch").retryable = true;
  detect("taxonomy_projection_drift", () => compileProfile(candidate), "context_provenance_mismatch");
}

const child = spawnSync(process.execPath, [...process.execArgv, fileURLToPath(import.meta.url), "--simulate-old-false-positive-counter"], { cwd: root, encoding: "utf8" });
assert.equal(child.status, 97, `${child.stdout}\n${child.stderr}`);
assert.match(child.stderr, /OLD_FALSE_POSITIVE_DEMOTION_COUNTER_RESTORED/);
detectedMutations.push("old_false_positive_demotion_counter_self_test");

for (const required of vectors.requiredMutationLabels) assert.equal(detectedMutations.includes(required), true, required);
assert.equal(new Set(detectedMutations).size, detectedMutations.length);
...
const schemaV19 = section(design, "## 11. Exact `0020` field mapping", "## 12. Prompt Registry");
const schemaV18 = section(designV18, "## 11. Exact `0020` field mapping", "## 12. Prompt Registry");
assert.equal(schemaV19, schemaV18);
const configRows = section(schemaV19, "### 11.1 `ai_model_config`", "### 11.2 `ai_runs`").match(/^\| `[^`]+` \|/gm) ?? [];
const runRows = schemaV19.slice(schemaV19.indexOf("### 11.2 `ai_runs`")).match(/^\| `[^`]+` \|/gm) ?? [];
...
console.log("TASK=CWT Stage 4A Phase B Corrected Exact Design V1.9 V18-M01/M02/M03 offline verification");
...
console.log(`ERROR_TAXONOMY=${profile.errorTaxonomy.codes.length}/69_EXACT:traversal_closed_PASS:zero_multiple_context_provenance_mismatch:unknown_internal_failure:type_probe_PASS`);
console.log("CLAIMED_REPLAY_ORDER=CR-01..CR-14_EXACT:section13_section18_matrix_MATCH:context_CR03-CR08_before_config_CR09_prompt_CR10_envelope_CR11_adapter_resolution_CR12:one_call_CR13");
...
console.log(`EXECUTED_MUTATIONS=${detectedMutations.length}/${detectedMutations.length}_ACTUAL_FAILURES:${detectedMutations.join(",")}`);
console.log("OLD_BUG_SELF_TEST=child_exit_97_EXPECTED_AND_CAPTURED");
...
console.log(`IMPORTED_MANIFESTS=V1_8_${v18ManifestCount}/${v18ManifestCount}:INDEPENDENT_FAIL_${reviewManifestCount}/${reviewManifestCount}`);
