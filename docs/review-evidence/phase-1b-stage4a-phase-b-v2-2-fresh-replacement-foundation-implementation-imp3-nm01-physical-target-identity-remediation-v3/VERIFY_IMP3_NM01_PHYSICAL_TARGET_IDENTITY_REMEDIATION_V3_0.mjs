...
const authorityPath = resolve(evidenceRoot, "IMP3_NM01_PHYSICAL_TARGET_IDENTITY_AUTHORITY_V3_0.json");
...
function fail(message) {
  throw new Error(`IMP3-NM01 physical-target identity verification failed: ${message}`);
}
...
function reproduceCaseVariantPhysicalTargetRejection(
  root,
  ...
  const canonicalStat = statSync(resolve(root, canonicalTarget));
  const variantStat = statSync(resolve(root, caseVariantTarget));
  if (canonicalStat.dev !== variantStat.dev || canonicalStat.ino !== variantStat.ino) {
    fail(`case-variant target is not the same physical file: ${caseVariantTarget}`);
  }
  const sourceRelative = "src/storage/index.ts";
  const sourcePath = resolve(root, sourceRelative);
  const source = readFileSync(sourcePath, "utf8");
  const relativeTarget = relative(dirname(sourceRelative), caseVariantTarget).replaceAll("\\", "/");
  const specifier = relativeTarget.startsWith(".") ? relativeTarget : `./${relativeTarget}`;
  writeFileSync(sourcePath, `${source}\nimport ${JSON.stringify(specifier)};\n`);
  const removeBridge = installDependencyBridge(root, installedNodeModules);
  let gate;
  try {
    gate = spawnSync(process.execPath, ["--import", "tsx", checkerPath], {
      cwd: root,
      encoding: "utf8",
      env: { ...process.env, CWT_INSTALLED_NODE_MODULES: installedNodeModules },
    });
  ...
  const expected = [
    ...
    `"requestedPath":${JSON.stringify(caseVariantTarget)}`,
    `"canonicalPath":${JSON.stringify(canonicalTarget)}`,
    '"reason":"non_case_exact_or_physical_alias_target_spelling"',
    '"nodeKind":"ImportDeclaration"',
  ];
  if (gate.status === 0 || expected.some((token) => !gate.stderr.includes(token))) {
    fail(`case-variant witness did not fail with physical identity diagnostics: ${gate.stderr || gate.stdout}`);
  }
  let strictTypecheckStatus = null;
  if (strictTypecheck) {
    const local = resolve(root, "node_modules");
    symlinkSync(installedNodeModules, local, "dir");
    try {
      const result = spawnSync(process.execPath, [resolve(installedNodeModules, "typescript/bin/tsc"), "--noEmit"], {
        cwd: root,
        encoding: "utf8",
      });
      strictTypecheckStatus = result.status;
      if (result.status !== 0) fail(`case-variant strict typecheck control failed: ${result.stderr || result.stdout}`);
    } finally {
      rmSync(local);
    }
  }
  return {
    result: "fail-closed",
    reason: "class_capability_violation",
    sameDeviceAndInode: true,
    strictTypecheckStatus,
  };
}
...
if (authority.status !== "IMP3_NM01_ATTEMPT3_CANDIDATE_REVIEW_REQUIRED_NOT_ACCEPTED" ||
...
let immutableCaseVariant;
let currentJsonCaseVariant;
let currentAuthorityCaseVariant;
let directoryCaseVariant;
try {
  ...
  immutableCaseVariant = reproduceCaseVariantPhysicalTargetRejection(
    detachedRoot,
    installedNodeModules,
    authority.immutableHistoricalProbe.path,
    authority.immutableHistoricalProbe.path.replace("REVIEWER_H02", "reviewer_H02"),
    false,
  );
  execFileSync("git", ["restore", "src/storage/index.ts"], { cwd: detachedRoot, stdio: "ignore" });
  currentJsonCaseVariant = reproduceCaseVariantPhysicalTargetRejection(
    detachedRoot,
    installedNodeModules,
    authority.currentEvidenceJson.path,
    authority.currentEvidenceJson.path.replace(
  ...
  execFileSync("git", ["restore", "src/storage/index.ts"], { cwd: detachedRoot, stdio: "ignore" });
  currentAuthorityCaseVariant = reproduceCaseVariantPhysicalTargetRejection(
    detachedRoot,
    installedNodeModules,
    authority.currentAuthority.path,
    authority.currentAuthority.path.replace(
      "IMP3_NM01_PHYSICAL_TARGET_IDENTITY_AUTHORITY_V3_0.json",
      "imp3_nm01_physical_target_identity_authority_v3_0.json",
    ),
    true,
  );
  execFileSync("git", ["restore", "src/storage/index.ts"], { cwd: detachedRoot, stdio: "ignore" });
  directoryCaseVariant = reproduceCaseVariantPhysicalTargetRejection(
    detachedRoot,
    installedNodeModules,
    authority.immutableHistoricalProbe.path,
    authority.immutableHistoricalProbe.path.replace("review-evidence", "review-Evidence"),
    false,
  );
  execFileSync("git", ["restore", "src/storage/index.ts"], { cwd: detachedRoot, stdio: "ignore" });
...
process.stdout.write(`${JSON.stringify({
  ...
  physicalTargetCaseVariants: {
    immutableCaseVariant,
    currentJsonCaseVariant,
    currentAuthorityCaseVariant,
    directoryCaseVariant,
  },
