...
const AUTHORITY_SCHEMA_ID = "cwt.phase1b.stage4a.phaseb.v111-m01.canonical-review-authority.v2_2";
const VERIFIER_SCHEMA_ID = "cwt.phase1b.stage4a.phaseb.v111-m01.replacement-verifier.v2_2";
const ENVELOPE_SCHEMA_ID = "cwt.phase1b.stage4a.phaseb.v111-m01.candidate-review-envelope.v2";
const PROFILE_ID = "cwt.phase1b.stage4a.phaseb.v111-m01.replacement-current-technical-profile.v2_2";
const PROFILE_VERSION = "2.2.0";
const CANDIDATE_STATUS = "v2-m01-raw-cli-remediation-corrected-design-candidate-awaiting-fresh-independent-review-not-implementation-authority";
const PACKAGE_DIR = "docs/review-evidence/phase-1b-stage4a-phase-b-v111-m01-replacement-corrected-design-v2-2";
const AUTHORITY_PATH = `${PACKAGE_DIR}/V111_M01_CANONICAL_REVIEW_AUTHORITY_V2_2.json`;
const MANIFEST_PATH = `${PACKAGE_DIR}/SHA256SUMS.txt`;
const PACKAGE_CAPTURE_PATH = `${PACKAGE_DIR}/PACKAGE_ONLY_VERIFICATION_OUTPUT_V2_2.txt`;
const MUTATION_CAPTURE_PATH = `${PACKAGE_DIR}/SCHEMA_PROPERTY_MUTATION_OUTPUT_V2_2.txt`;
const BOUNDARY_CAPTURE_PATH = `${PACKAGE_DIR}/IDENTITY_JOIN_FRESH_NEGATIVE_OUTPUT_V2_2.txt`;
const RAW_CLI_CAPTURE_PATH = `${PACKAGE_DIR}/ACTUAL_CLI_ENTRY_BOUNDARY_OUTPUT_V2_2.txt`;
const VERIFIER_PATH = `${PACKAGE_DIR}/VERIFY_V111_M01_REPLACEMENT_CORRECTED_DESIGN_V2_2.mjs`;
...
function validateRawAuthorityArgument(rawAuthorityArgument) {
  if (typeof rawAuthorityArgument !== "string" || rawAuthorityArgument !== AUTHORITY_PATH) {
    fail("raw_authority_spelling_not_canonical");
  }
  return {
    accepted: true,
    assertionOnlyNotLocator: true,
    canonicalPath: AUTHORITY_PATH,
    diagnostic: "raw_authority_spelling_exact",
    validationOrder: "raw-byte-code-point-equality-before-path-url-filesystem-unicode-processing"
  };
}
...
function validateRootSchema(root) {
  ...
  if (root.authorityId !== AUTHORITY_SCHEMA_ID || root.authorityVersion !== "2.2.0") fail("authority-identity");
  ...
  if (verifier.executableAuthority !== true || verifier.schemaId !== VERIFIER_SCHEMA_ID || verifier.schemaVersion !== "2.2.0") fail("verifier-role-authority");
  ...
  if (!Array.isArray(root.checkpoints) || root.checkpoints.length !== 4) fail("checkpoint-cardinality");
  const checkpointRoles = root.checkpoints.map((entry) => entry.role);
  if (jcs(checkpointRoles) !== jcs(["checkpoint.analysisPreL3", "checkpoint.correctedDesignPreL3", "checkpoint.v2M01RemediationPreL3", "checkpoint.v21CliRemediationPreL3"])) fail("checkpoint-role-identity");
  ...
  exactKeys(root.closedReplacementEvidence, ["failedV20", "failedV21"], "closed-replacement-evidence");
  for (const [version, evidence] of Object.entries(root.closedReplacementEvidence)) {
    exactKeys(evidence, [
    ...
    "importedReviewerEnvelopeIsCurrent", "nonExecutable", "reviewerEvidencePath",
    "reviewerEvidenceSha256", "reviewerManifestPath", "reviewerManifestSha256",
    "reviewerReportPath", "reviewerReportSha256"
    ], `closed-replacement-evidence:${version}`);
    if (evidence.nonExecutable !== true || evidence.currentExecutableEdge !== false || evidence.importedReviewerEnvelopeIsCurrent !== false || evidence.findingId !== "V2-M01") fail("closed-replacement-evidence-state", version);
    for (const key of ["candidateCommit", "candidateParent", "candidateTree"]) {
      if (!/^[0-9a-f]{40}$/.test(evidence[key])) fail("closed-replacement-git-shape", `${version}:${key}`);
    }
    for (const key of ["candidateManifestSha256", "candidateRootSha256", "reviewerEvidenceSha256", "reviewerManifestSha256", "reviewerReportSha256"]) {
      if (!/^[0-9a-f]{64}$/.test(evidence[key])) fail("closed-replacement-hash-shape", `${version}:${key}`);
    }
    for (const key of ["candidateManifestPath", "candidateRootPath", "historicalVerifierPath", "reviewerEvidencePath", "reviewerManifestPath", "reviewerReportPath"]) {
      validateRepoPath(evidence[key], `closed-replacement:${version}:${key}`);
    }
  }
  ...
  exactKeys(root.rolePolicy.pathContract, [
    "actualCliChildProcessProof", "authorityPathClosed", "caseExact", "headMode", "indexAndWorktreeEqualHead",
    "loadedRootPhysicalInjectivity", "nfc", "physicalIdentityUnique",
    "rawAuthorityAssertionOnly", "rawAuthorityExactBeforeProcessing",
    "repoRelativePosix", "resolveBeforeRawValidationAllowed",
    "soleManifestRootJoin", "symlinkAllowed",
    "trackedRegularExistingRoles"
  ], "path-policy");
  if (jcs(root.rolePolicy.pathContract) !== jcs({
    actualCliChildProcessProof: true,
    ...
    rawAuthorityAssertionOnly: true,
    rawAuthorityExactBeforeProcessing: true,
    repoRelativePosix: true,
    resolveBeforeRawValidationAllowed: false,
  ...
  if (!Array.isArray(root.rolePolicy.candidatePackagePaths) || root.rolePolicy.candidatePackagePaths.length !== 14) fail("package-inventory-cardinality");
  ...
  historyPaths.add(root.closedReplacementEvidence.failedV20.historicalVerifierPath);
  historyPaths.add(root.closedReplacementEvidence.failedV21.historicalVerifierPath);
...
function validateProfile(profile) {
  ...
  exactKeys(profile.authorityPolicy, [
    "actualCliChildProcessProof", "canonicalAuthorityPath", "canonicalRootCount", "compatibilityLayerAllowed",
    ...
    "rawAuthorityAssertionOnly", "rawAuthorityExactBeforeProcessing",
    "resolveBeforeRawValidationAllowed", "reviewerEnvelopeCandidateCommitted",
    "soleManifestRootJoin", "v20FallbackAllowed", "v21FallbackAllowed"
  ], "profile-authority-policy");
  if (
    profile.authorityPolicy.actualCliChildProcessProof !== true ||
    ...
    profile.authorityPolicy.rawAuthorityAssertionOnly !== true ||
    profile.authorityPolicy.rawAuthorityExactBeforeProcessing !== true ||
    profile.authorityPolicy.resolveBeforeRawValidationAllowed !== false ||
    ...
    profile.authorityPolicy.v21FallbackAllowed !== false
  ) fail("profile-authority-policy-value");
  if (profile.findingDispositions.V2_M01 !== "BOUNDED_RAW_CLI_REMEDIATION_CANDIDATE_AWAITING_FRESH_INDEPENDENT_PASS") fail("profile-v2-m01-disposition");
  ...
  for (const [name, entry] of Object.entries(profile.selectedAuthorities)) {
    ...
    if (entry.source.failedAncestrySuppliesAuthority !== false || entry.source.currentBy !== "v2-2-raw-cli-successor-canonical-root-plus-future-fresh-pass") fail("selected-currentness", name);
...
function captureDiagnostic(id, operation) {
  try {
    ...
    return { diagnostic: error instanceof Error ? error.message : String(error), id, rejected: true, transport: "direct-validator-defense-in-depth" };
  }
  fail("boundary-negative-did-not-reject", id);
}

function actualCliReject(repoRoot, id, rawAuthorityArgument) {
  const verifierAbsolute = path.join(repoRoot, VERIFIER_PATH);
  ...
  if (result.error) fail("actual-cli-child-error", `${id}:${result.error.code || result.error.message}`);
  if (!Number.isInteger(result.pid) || result.pid <= 0) fail("actual-cli-child-not-spawned", id);
  const diagnostic = (result.stderr || "").match(/Error: ([^\n]+)/)?.[1] ?? null;
  if (result.status !== 1 || diagnostic !== "raw_authority_spelling_not_canonical" || (result.stdout || "") !== "") {
    fail("actual-cli-negative-did-not-fail-closed", `${id}:${result.status}:${diagnostic}`);
  }
  return {
    childProcessObserved: true,
    diagnostic,
    exit: result.status,
    id,
    rejected: true,
    transport: "spawnSync-process-execPath-real-package-cli"
  };
}

function actualCliNulProcessBoundary(repoRoot) {
  const id = "v2-m01-raw-cli-nul-process-interface";
  const verifierAbsolute = path.join(repoRoot, VERIFIER_PATH);
  try {
    const result = spawnSync(process.execPath, [verifierAbsolute, "--authority", `${AUTHORITY_PATH}\u0000suffix`, "--package-only"], {
      cwd: repoRoot,
      encoding: "utf8",
      env: process.env,
      stdio: ["ignore", "pipe", "pipe"]
    });
    if (result.error?.code === "ERR_INVALID_ARG_VALUE") {
      return { diagnostic: "process_interface_rejected_nul_before_cli", exit: null, id, rejected: true, transport: "node-process-interface" };
    }
  } catch (error) {
  ...
  fail("nul-process-interface-did-not-reject");
}

function rawCliCaseDefinitions(repoRoot) {
  const packageName = path.posix.basename(PACKAGE_DIR);
  const authorityName = path.posix.basename(AUTHORITY_PATH);
  return [
    ["v2-m01-raw-cli-exact-absolute-realpath", path.join(repoRoot, AUTHORITY_PATH)],
    ["v2-m01-raw-cli-dot-prefix", `./${AUTHORITY_PATH}`],
    ["v2-m01-raw-cli-dot-segment", `${PACKAGE_DIR}/./${authorityName}`],
    ["v2-m01-raw-cli-cancelling-dot-dot", `${PACKAGE_DIR}/../${packageName}/${authorityName}`],
    ["v2-m01-raw-cli-noncancelling-dot-dot", `${PACKAGE_DIR}/../${authorityName}`],
    ["v2-m01-raw-cli-duplicate-slash", AUTHORITY_PATH.replace("docs/", "docs//")],
    ["v2-m01-raw-cli-trailing-slash", `${AUTHORITY_PATH}/`],
    ["v2-m01-raw-cli-backslash", AUTHORITY_PATH.replaceAll("/", "\\")],
    ["v2-m01-raw-cli-percent-encoded", AUTHORITY_PATH.replace("V111", "%56111")],
    ["v2-m01-raw-cli-percent-bearing", `${AUTHORITY_PATH}%`],
    ["v2-m01-raw-cli-nfd-lookalike", AUTHORITY_PATH.replace("V2_2.json", "V2_2-e\u0301.json")],
    ["v2-m01-raw-cli-unicode-slash-lookalike", AUTHORITY_PATH.replace("docs/", "docs\u2215")],
    ["v2-m01-raw-cli-case-only", AUTHORITY_PATH.replace("V111", "v111")],
    ["v2-m01-raw-cli-leading-control", `\u0001${AUTHORITY_PATH}`],
    ["v2-m01-raw-cli-trailing-control", `${AUTHORITY_PATH}\u007f`],
    ["v2-m01-raw-cli-interior-control", AUTHORITY_PATH.replace("docs/", "docs/\u0001")],
    ["v2-m01-raw-cli-leading-whitespace", ` ${AUTHORITY_PATH}`],
    ["v2-m01-raw-cli-trailing-whitespace", `${AUTHORITY_PATH} `],
    ["v2-m01-raw-cli-interior-whitespace", AUTHORITY_PATH.replace("docs/", "docs / ")],
    ["v2-m01-raw-cli-tilde-home", `~/${AUTHORITY_PATH}`],
    ["v2-m01-raw-cli-home-environment", `$HOME/${AUTHORITY_PATH}`],
    ["v2-m01-raw-cli-environment-assignment-like", `CWT_AUTHORITY=${AUTHORITY_PATH}`],
    ["v2-m01-raw-cli-other-nonidentical", `${AUTHORITY_PATH}.alternate`]
  ];
}

export function runRawCliActualEntryMatrix(repoRoot) {
  const cases = rawCliCaseDefinitions(repoRoot).map(([id, raw]) => actualCliReject(repoRoot, id, raw));
  cases.push(actualCliNulProcessBoundary(repoRoot));
  return {
    canonicalParentInvocation: {
      accepted: true,
      diagnostic: "raw_authority_spelling_exact",
      path: AUTHORITY_PATH,
      transport: "real-package-cli-process-argv"
    },
    cases,
    count: cases.length,
    ok: true
...
export function runBoundaryDiagnosticMatrix(repoRoot) {
  const baselineHead = syntheticHeadBinding();
  const cases = [
    actualCliReject(repoRoot, "v2-m01-ignored-untracked-exact-root-copy", ".next/reviewer-v111-m01-v2/alternate-authority-copy.json"),
    actualCliReject(repoRoot, "v2-m01-ignored-untracked-hardlink-root", ".next/reviewer-v111-m01-v2/alternate-authority-hardlink.json"),
    actualCliReject(repoRoot, "v2-m01-ignored-untracked-resealed-root", ".next/reviewer-v111-m01-v2/alternate-authority-resealed.json"),
    actualCliReject(repoRoot, "v2-m01-visible-untracked-root", "docs/reviewer-visible-untracked-root.json"),
    actualCliReject(repoRoot, "v2-m01-tracked-wrong-path-root", `${PACKAGE_DIR}/TRACKED_WRONG_PATH_ROOT.json`),
    actualCliReject(repoRoot, "v2-m01-case-only-root-path", `${PACKAGE_DIR}/v111_M01_CANONICAL_REVIEW_AUTHORITY_V2_2.json`),
    ...
    captureDiagnostic("v2-m01-root-hardlink-current-role", () => validatePhysicalEntries([
      ...
      { dev: 7, ino: 11, kind: "file", path: `${PACKAGE_DIR}/PHASE_B_CURRENT_TECHNICAL_PROFILE_V2_2.json` }
    ])),
    captureDiagnostic("v2-m01-root-hardlink-proof-artifact", () => validatePhysicalEntries([
      { dev: 7, ino: 12, kind: "file", path: AUTHORITY_PATH },
      { dev: 7, ino: 12, kind: "file", path: `${PACKAGE_DIR}/V111_M01_REPLACEMENT_PROOF_MATRIX_V2_2.json` }
    ])),
    ...
    actualCliReject(repoRoot, "v2-m01-coordinated-authority-envelope-redirection", ".next/coordinated-authority-and-envelope.json"),
    ...
    actualCliReject(repoRoot, "v2-m01-self-consistently-resealed-alternate-root", ".cache-v2-m01/resealed-root.json"),
    actualCliReject(repoRoot, "v2-m01-arbitrary-ignored-directory-root", ".fresh-unnamed-ignored-v2-m01/root.json")
  ];
  return { cases, count: cases.length, ok: true };
}

export function runProofMatrix(root, profile, matrix, authorityInfo, repoRoot, boundaryDiagnostics, rawCliDiagnostics) {
  const positive = [];
  const negative = [];
  const properties = [];
  ...
  negative.push(...rawCliDiagnostics.cases.map((entry) => entry.id));
  const dotPrefixActual = rawCliDiagnostics.cases.find((entry) => entry.id === "v2-m01-raw-cli-dot-prefix");
  assert.equal(dotPrefixActual?.diagnostic, "raw_authority_spelling_not_canonical");
  negative.push("mutation-resolve-before-raw-validation-killed");
  assert.equal(rawCliDiagnostics.cases.filter((entry) => entry.transport === "spawnSync-process-execPath-real-package-cli" && entry.childProcessObserved === true).length, rawCliDiagnostics.cases.length - 1);
  negative.push("mutation-helper-only-testing-killed");
  ...
  assert.equal(root.checkpoints.length, 4);
  assert.equal(root.historicalEvidence.length, 3);
  properties.push("closed-cardinality");
  assert.equal(new Set([...root.currentRoles.map((entry) => entry.path), ...root.proofContracts.map((entry) => entry.outputPath), ...root.checkpoints.map((entry) => entry.recordPath)]).size, 12);
  ...
  assert.equal(validateRawAuthorityArgument(AUTHORITY_PATH).validationOrder, "raw-byte-code-point-equality-before-path-url-filesystem-unicode-processing");
  properties.push("raw-cli-byte-exact-before-processing");
  assert.equal(rawCliDiagnostics.cases.length, 24);
  assert.equal(rawCliDiagnostics.canonicalParentInvocation.transport, "real-package-cli-process-argv");
  properties.push("actual-cli-entry-process-proof");
...
function loadPackage(authorityArgument) {
  ...
  if (matrix.positive.length !== 10 || matrix.negative.length !== 91 || matrix.properties.length !== 16) fail("proof-matrix-count");
  ...
  const boundaryDiagnostics = runBoundaryDiagnosticMatrix(repoRoot);
  const rawCliDiagnostics = runRawCliActualEntryMatrix(repoRoot);
  const proof = runProofMatrix(root, profile, matrix, authorityInfo, repoRoot, boundaryDiagnostics, rawCliDiagnostics);
  const secondProof = runProofMatrix(root, profile, matrix, authorityInfo, repoRoot, boundaryDiagnostics, rawCliDiagnostics);
  ...
  const rawCliCapture = fs.readFileSync(path.join(repoRoot, RAW_CLI_CAPTURE_PATH), "utf8");
  if (rawCliCapture !== normalized(rawCliDiagnostics)) fail("raw-cli-capture-mismatch");
  return { ...loaded, authorityInfo, boundaryDiagnostics, manifestEntries: manifestVerification.count, manifestVerification, matrix, profile, proof, rawCliDiagnostics };
}
...
function packageOnlyResult(loaded) {
  return {
    ...
    rawCliBoundary: {
      ...loaded.rawAuthorityEvidence,
      actualChildNegativeCount: loaded.rawCliDiagnostics.count,
      actualChildProcessProof: true
    },
...
function parseArgs(argv) {
  ...
  while (args.length > 0) {
    ...
    if (key === "--authority" || key === "--review-envelope") {
      ...
      if (value === undefined) fail("missing-cli-value", key);
      const field = key === "--authority" ? "authority" : "reviewEnvelope";
      if (Object.hasOwn(result, field)) fail("duplicate-cli-argument", key);
      result[field] = value;
      continue;
    }
    fail("unknown-cli-argument", String(key));
  }
  if (!Object.hasOwn(result, "authority")) fail("authority-required");
