...
const EXPECTED_REF = "refs/heads/codex/phase-1b-stage4a-phase-b-v111-m01-three-strike-replacement-design-v1";
const AUTHORITY_SCHEMA_ID = "cwt.phase1b.stage4a.phaseb.v111-m01.canonical-review-authority.v2";
const VERIFIER_SCHEMA_ID = "cwt.phase1b.stage4a.phaseb.v111-m01.replacement-verifier.v2";
const ENVELOPE_SCHEMA_ID = "cwt.phase1b.stage4a.phaseb.v111-m01.candidate-review-envelope.v2";
const PROFILE_ID = "cwt.phase1b.stage4a.phaseb.v111-m01.replacement-current-technical-profile.v2";
const PROFILE_VERSION = "2.0.0";
const PACKAGE_DIR = "docs/review-evidence/phase-1b-stage4a-phase-b-v111-m01-three-strike-replacement-corrected-design-v2";
const MANIFEST_PATH = `${PACKAGE_DIR}/SHA256SUMS.txt`;
const PACKAGE_CAPTURE_PATH = `${PACKAGE_DIR}/PACKAGE_ONLY_VERIFICATION_OUTPUT_V2_0.txt`;
const MUTATION_CAPTURE_PATH = `${PACKAGE_DIR}/SCHEMA_PROPERTY_MUTATION_OUTPUT_V2_0.txt`;
...
const ROOT_KEYS = [
  "authorityId", "authorityVersion", "canonicalization", "checkpoints",
  "currentRoles", "frozenInputs", "historicalEvidence", "proofContracts",
  "roleGraph", "rolePolicy", "seal", "status", "subject"
];

const EXPECTED_GRAPH = {
  authorityRoot: ["embeddedSubject", "renderedDesign", "currentTechnicalProfile", "currentVerifier", "proofContracts", "checkpoints", "frozenInputs", "historicalEvidence"],
  checkpoints: [],
...
function validateRootSchema(root) {
  ...
  if (root.authorityId !== AUTHORITY_SCHEMA_ID || root.authorityVersion !== "2.0.0") fail("authority-identity");
  if (root.status !== "corrected-design-candidate-awaiting-fresh-independent-review-not-implementation-authority") fail("authority-status");
  ...
  if (verifier.executableAuthority !== true || verifier.schemaId !== VERIFIER_SCHEMA_ID || verifier.schemaVersion !== "2.0.0") fail("verifier-role-authority");
  ...
  if (!Array.isArray(root.checkpoints) || root.checkpoints.length !== 2) fail("checkpoint-cardinality");
  const checkpointRoles = root.checkpoints.map((entry) => entry.role);
  if (jcs(checkpointRoles) !== jcs(["checkpoint.analysisPreL3", "checkpoint.correctedDesignPreL3"])) fail("checkpoint-role-identity");
  ...
  exactKeys(root.rolePolicy, ["candidateManifest", "candidatePackagePaths", "markdown", "pathContract", "proofMatrix", "rootCount"], "role-policy");
  ...
  exactKeys(root.rolePolicy.candidateManifest, ["derivedOnly", "hashedByAuthorityRoot", "path"], "manifest-policy");
  if (root.rolePolicy.candidateManifest.derivedOnly !== true || root.rolePolicy.candidateManifest.hashedByAuthorityRoot !== false || root.rolePolicy.candidateManifest.path !== MANIFEST_PATH) fail("manifest-policy");
  exactKeys(root.rolePolicy.pathContract, ["caseExact", "nfc", "physicalIdentityUnique", "repoRelativePosix", "symlinkAllowed", "trackedRegularExistingRoles"], "path-policy");
  if (jcs(root.rolePolicy.pathContract) !== jcs({ caseExact: true, nfc: true, physicalIdentityUnique: true, repoRelativePosix: true, symlinkAllowed: false, trackedRegularExistingRoles: true })) fail("path-policy");
  ...
  if (!Array.isArray(root.rolePolicy.candidatePackagePaths) || root.rolePolicy.candidatePackagePaths.length !== 12) fail("package-inventory-cardinality");
...
function validateProfile(profile) {
  ...
  if (profile.profileId !== PROFILE_ID || profile.profileVersion !== PROFILE_VERSION || profile.status !== "corrected-design-candidate-awaiting-fresh-independent-review-not-implementation-authority") fail("profile-identity");
  ...
  for (const [name, entry] of Object.entries(profile.selectedAuthorities)) {
    ...
    if (entry.source.failedAncestrySuppliesAuthority !== false || entry.source.currentBy !== "successor-canonical-root-plus-future-fresh-pass") fail("selected-currentness", name);
...
function verifyManifest(repoRoot, root) {
  const manifestPolicy = root.rolePolicy.candidateManifest;
  const manifest = existingTrackedRegular(repoRoot, manifestPolicy.path);
  const entries = parseManifest(fs.readFileSync(manifest.absolute, "utf8"));
  ...
  const physical = [];
  for (const entry of entries) {
    const file = existingTrackedRegular(repoRoot, entry.path);
    physical.push({ ...file, kind: "file", path: entry.path });
    if (sha256(fs.readFileSync(file.absolute)) !== entry.hash) fail("manifest-hash", entry.path);
  }
...
export function runProofMatrix(root, profile, matrix, authorityInfo, repoRoot) {
  ...
  assert.equal(root.checkpoints.length, 2);
  assert.equal(root.historicalEvidence.length, 3);
  properties.push("closed-cardinality");
  assert.equal(new Set([...root.currentRoles.map((entry) => entry.path), ...root.proofContracts.map((entry) => entry.outputPath), ...root.checkpoints.map((entry) => entry.recordPath)]).size, 10);
  ...
  assert.equal(root.rolePolicy.candidatePackagePaths.some((entry) => entry.endsWith("CANDIDATE_REVIEW_ENVELOPE_V2_0.json")), false);
...
function loadAuthority(authorityArgument) {
  const argumentAbsolute = path.resolve(authorityArgument);
  const repoRoot = repoRootFrom(path.dirname(argumentAbsolute));
  const absolute = fs.realpathSync(argumentAbsolute);
  const repoPrefix = `${fs.realpathSync(repoRoot)}${path.sep}`;
  if (!absolute.startsWith(repoPrefix)) fail("authority-outside-repo");
  const authorityPath = path.relative(repoRoot, absolute).split(path.sep).join("/");
  validateRepoPath(authorityPath, "authority-path");
  const bytes = fs.readFileSync(absolute);
  const { value: root } = strictJsonParse(bytes, "canonical-authority", true);
  validateRootSchema(root);
  verifySeals(root);
  return { absolute, authorityPath, bytes, repoRoot, root };
}

function loadPackage(authorityArgument) {
  const loaded = loadAuthority(authorityArgument);
  ...
  if (matrix.positive.length !== 10 || matrix.negative.length !== 42 || matrix.properties.length !== 10) fail("proof-matrix-count");
  const manifestEntries = verifyManifest(repoRoot, root);
  ...
  return { ...loaded, authorityInfo, manifestEntries, matrix, profile, proof };
}
