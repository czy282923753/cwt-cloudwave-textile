...
After attachments have uploaded, the public form freezes one component-memory request snapshot containing the exact Idempotency Key, normalized form/attribution payload, ordered Upload Tokens and safe attachment display names. A network loss, timeout, interrupted response or result-uncertain 5xx offers `Retry same submission`, which resends that exact snapshot without creating another Upload Intent or uploading again. Fields remain available as a readable frozen summary and cannot be silently edited under the old key. A definitive 4xx/409 requires the explicit `Edit and start over` path, which clears the old in-memory attempt; the next submit receives a new key and new uploads. Tokens are not written to URLs, Analytics, console output, localStorage or IndexedDB.
