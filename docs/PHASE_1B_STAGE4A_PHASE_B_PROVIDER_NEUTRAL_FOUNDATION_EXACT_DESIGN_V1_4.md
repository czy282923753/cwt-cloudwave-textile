...
- Status: **CORRECTED DESIGN CANDIDATE V1.4 / NOT SELF-APPROVED / IMPLEMENTATION NOT YET ELIGIBLE**
- Design version: `1.4`
- Supersedes for implementation eligibility: failed Design V1.3 at exact commit `7fdc92b880fda9dc264db5bc99b37a1fae65ddb4`; V1.0 through V1.3, all prior remediations, and all review histories remain immutable
- Remediates only Fresh Re-review finding `N-M04` scope construction; preserves the accepted dual-binder correction plus closed `M-04`, `N-M01`, `N-M02`, `N-M03`, `H-01`, `H-02`, `M-01`, `M-02`, `M-03`, `M-05`, `M-06`, and `L-01`
...
V1.4 retains V1.3's accepted dual authorization/binder composition and corrects only the construction seam on which those binders depend. Generic `ApplicationReadScope` is now an unbranded structural constraint containing only `readonly mode: string`; it owns no constructor, factory, database, lock, transaction, or authority. Draft and Synthetic each own their module-private nominal brand, private executor capability, direct object-literal factories, and callback lifetime. Factory-returned values bind to the same opaque staged closures before generic core entry. The mode string alone cannot enter a Draft binder because the Draft-private carrier is absent. Core still receives no application scope, target, repository, factory, or lock method.

## 2. Fixed authority and verified remediation identity

The mandatory pre-edit gate for this V1.4 Candidate is:
...
The worktree was attached to the required existing design branch and clean before V1.4 was added. These fixed Fresh Re-review inputs, including every manifest-protected positive/negative probe, were read completely and recomputed before revision:
...
Fresh Re-review V1.2 disposition was Blocker `0`, High `0`, Medium `1`, Low `0`; implementation remained **NOT ELIGIBLE**. It passed every architecture, association, config, evidence, dual-binder, sequence, count, opaque-core, and non-regression check. Its only open issue was that a separate application module could not populate generic `contracts.ts`'s inaccessible `applicationReadScopeBrand`. V1.4 removes that redundant generic brand rather than adding a base factory/class/exported token or co-locating Draft/DB code. The review recorded downloads `0`; this design task likewise performs no package install/download, registry access, or network action.
...
- **No implementation leakage from design remediation:** V1.4/Remediation V1.3 commit itself must contain documents only.
...
Only after V1.4 receives PASS from another Fresh Re-review:
...
## 23. Exact V1.4 design acceptance checklist

The Fresh Re-review may return PASS only if all are true:

- [ ] exact V1.4 remediation start, immutable V1.0–V1.3/prior-Remediation hashes, Fresh Re-review V1.2 report/evidence/manifest/four-probe hashes, branch/parent/ancestry/tag/acceptance identity pass;
- [ ] V1.4 and Remediation V1.3 are the only added files and all older artifacts are byte-identical;
...
V1.0 consolidated Provider mechanics but left Draft association/result concepts in core and several integrity checks implicit. V1.1 moved application semantics outward; V1.2 closed target/config/read/evidence gaps. V1.3 removed the single common-scope authorization path and assigned availability/read versus request/lock to two application-owned binders. Fresh Re-review then exposed one redundant authority marker: the generic module's private brand could not be populated by application modules. V1.4 deletes that generic brand. Draft/Synthetic construction is now wholly owned by their respective private factory modules, while generic core remains application-neutral and scope-free.
