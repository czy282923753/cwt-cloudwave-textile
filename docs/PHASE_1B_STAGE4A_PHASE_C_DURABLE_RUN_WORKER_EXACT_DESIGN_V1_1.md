# CWT Stage 4A Phase C — Durable `ai_runs` Lifecycle and Modular-Monolith Worker Corrected Exact Design V1.1
...
- Exact Candidate parent: failed V1.0 commit `6cdb0df3e9b0565e5cdfd6cac0ca4a315d0e272b`
- Finding authority: independent FAIL review commit `8572f678cb608ff7dc41fd60a241fcbb0b4b18f3`
- Developer verification: [`DESIGN_REMEDIATION_VERIFICATION_V1_1.md`](./review-evidence/phase-1b-stage4a-phase-c-exact-design-v1-1/DESIGN_REMEDIATION_VERIFICATION_V1_1.md)
...
6. V1.0 is immutable failed history. This V1.1 is one complete replacement Candidate, not a patch layer: where V1.1 differs, only V1.1 is current for re-review; V1.0 supplies no runtime or implementation authority.
7. The review found no Owner/ADR/Schema/Migration/dependency decision need. V1.1 therefore corrects H-01 and M-01 without adding a persistent mechanism or expanding Phase C.
...
### 4.3 Remediation findings being closed

| Finding | Causal correction | Primary sections |
|---|---|---|
| `H-01 HEARTBEAT-CLAIM-SERIALIZATION` | One fixed advisory serialization domain now covers claim, expired recovery, active count, admission, heartbeat, dispatch marker, settlement, cancellation, manual retry, late accounting and shutdown-originated persistence before any run row. Heartbeat lock-busy is a no-mutation ephemeral outcome with a bounded retry/safety window. | 6, 8, 9, 20, 21, 22, 24, 26 |
| `M-01 REPLACEMENT-INVENTORY-CLOSURE` | Exact-tree rescan closes all V1 claimed/result/availability consumers, deletes the old root unambiguously, names every existing mutation and exact new test, and replaces current V3.1 consumption with one complete Phase C V4.0 authority while keeping V3.1 immutable history. | 14, 19, 22–24, 26 |

These are design corrections only. Their real PostgreSQL and implementation-tree proofs remain future Phase C implementation evidence, not claims made by this document.
...
This document and its V1.1 evidence perform design/static verification only. They do not claim that the H-01 PostgreSQL schedule, lock plans or resource envelope have executed; those are mandatory EV-01/EV-02 implementation proofs.
...
### 23.1 Exact accepted-tree consumer rescan and disposition
...
| `src/ai/core/orchestrator.ts` | defines `createAiClaimedExecutionServiceV1` and consumes/drops V1 attempt evidence | **MODIFY**: delete V1 factory/result path; implement CR-01–CR-12 → advisory/fenced dispatch callback → one adapter call → V2 evidence |
...
| `src/ai/testing/synthetic-application/synthetic-application.test.ts` | directly constructs the orchestrator with `durableEnqueueAvailable=false` | **MODIFY IN PLACE**: remove the deleted flag and prove the Synthetic transaction-bound durable/no-Production-authority seam |
...
| `scripts/verify-ai-architecture.ts` | pins V3.1 raw/integrity hashes, Phase B root/factory and V3.1 proofs | **MODIFY** into the sole V4.0 checker described in Sections 19.4/23.5; no second script |
...
The same rescan inspected `src/ai/providers/text-provider.ts` and `src/ai/testing/fake-text-provider.ts`. Their Provider result already contains the safe optional HTTP/code/request/model/usage evidence and remains sufficient; both are **UNCHANGED**. The loss is in the V1 core result normalization. No other accepted-tree file directly references the deleted claimed-execution symbols, Phase B factories, `durableEnqueueAvailable`, Phase B root or V3.1 current profile path.
...
| `src/ai/testing/accepted-draft-atomicity-harness.ts` | test-only conspicuous Synthetic owning mutation, excluded from Production graph |
...
- failed V1.0 Design/evidence plus its independent review/evidence.

### 23.7 Dependency order

1. Add pure attempt/retry/pricing contracts, the exact two error tuples and the 71-code profile; replace the V1 claimed/result contracts and in-place claimed test.
2. Add the standalone V4.0 profile and update the sole checker first so every later Phase C file is classified/fail-closed; V3.1 remains a non-authority historical file whose bytes may be inventoried but whose profile is never parsed/consumed.
3. Add PostgreSQL repository and exact lock-order/idempotency/budget tests, including H-01 barrier before any Worker composition.
4. Add config writer/read service and required-Audit/default-switch tests.
5. Wire governed enqueue/replay/target/config locks, request facade and in-place provider-neutral/Synthetic successor tests; keep Worker root absent.
6. Add advisory-first marker/heartbeat/settlement/recovery, cancellation/manual retry/read/disposition and Synthetic acceptance harness.
...
| replacement closure | every Section 23 existing path has exact modify/delete disposition; old executable/current symbols/root absent; V3.1 immutable/non-current; no out-of-allowlist path |
...
The Fresh independent **design re-reviewer** must verify exact Candidate SHA/parent/base/branch/clean state; independently check H-01 and M-01 across every related section; reproduce the accepted-tree consumer scan and 21/96 plus 40/11/18 structure; challenge both advisory-owner commit orders; verify the closed allowlist/V4 hash contract/sole-root rule and all preserved boundaries; and report Blocker/High/Medium/Low plus External Validation separately. Real PostgreSQL execution remains a future implementation proof and must not be misreported as having run in this docs-only remediation. Developer checks cannot self-accept Phase C.
...
The accepted Schema and ADRs are sufficient; no Owner/ADR/Schema/dependency decision is presently required. This corrected V1.1 Candidate defines one implementable lifecycle/Worker authority, exact field/contract ownership, every transaction/fence/failure path, a closed replacement allowlist/V4 architecture authority, proportional proof obligations and rollback. It is not accepted and does not authorize implementation by itself.
...
### 26.2 M-01 closure and affected invariants

| Required closure | Corrected authority | Proof obligation |
|---|---|---|
| all actual direct consumers disposed | Sections 14 and 23.1–23.3 | exact-tree identifier/path rescan and compile/test closure |
...
| one versioned current architecture authority | Sections 19.4 and 23.5 | standalone V4.0 raw/integrity hashes, one checker, six proof/manifest paths, V3.1 non-current |
| accepted gates preserved | Sections 19, 23.5–23.6, 24 | inherited case/probe ID-set hashes plus Phase C no-public/fake/PGlite/Phase-D/evidence negatives |

The closed future allowlist now names every existing mutation/deletion and every new Product/test/script/fixture path. No implementation may invent a wildcard test, second checker/root/profile, shim or out-of-allowlist file.
...
**Remediation Candidate disposition:** H-01 and M-01 are addressed by this developer design Candidate, but only a different Fresh independent reviewer and later Coordinator acceptance can close them. V1.1 does not accept itself and does not unfreeze Phase C implementation.
