...
## Stage 2C joint-review final-state closure

- Migration `0017_redirect_graph_final_state.sql` adds no table, field, enum or persistent coordination state. It installs deferred Route/Redirect constraint triggers that reject an invalid graph at transaction commit while allowing the existing Domain Service to use temporarily inconsistent intermediate statements during a legal flattened Route move.
- The final-state predicate requires every active Redirect source to be absent from current Routes, every destination to be a current Route, and every destination to be absent from active Redirect sources. A migration-time scan refuses installation over an already-invalid graph.
- Route deletion and movement participate in the existing `cwt:redirect-graph:*` transaction advisory-lock namespace. Application Domain Services remain the only supported graph mutation authority; the database boundary only fails closed.
- Inquiry attachment retry adds no persisted field. The exact pending request is retained only in the current public-form component lifecycle; the existing Inquiry fingerprint remains the server authority.
- Phase 1A remains at 55 relational tables.

## Stage 2C discovery concurrency closure

- Migration `0016_lumpy_whistler.sql` adds nullable `inquiries.request_fingerprint` and `request_fingerprint_version` to the existing Inquiry authority. New public submissions store a lowercase SHA-256 fingerprint with version `1`; both fields may be null only for legacy rows. The database Check Constraint rejects partial, malformed or invalid-version identities.
- The same forward Migration replaces the existing Route and Redirect trigger functions so direct database writes acquire deterministic path-scoped PostgreSQL transaction advisory locks in the same namespace as the Route Domain Service. The triggers retain their last-defense collision, destination, loop and chain checks.
- Product Revision Apply uses the existing `editorial_revisions` state as its single compare-and-set owner. It adds no table or lifecycle state.
- Phase 1A remains at 55 relational tables. Migration 0016 adds no Worker, queue, lease, recovery type or parallel authority.
...
- `idempotency_key` is unique per Inquiry. An exact versioned request fingerprint replays the same Inquiry; the same key with different content is a conflict.
