...
- Status: **CORRECTED DESIGN CANDIDATE V1.2 / NOT SELF-APPROVED / IMPLEMENTATION NOT YET ELIGIBLE**
- Design version: `1.2`
- Supersedes for implementation eligibility: failed Design V1.1 at exact commit `3be7cda919cd8d5418d31e81499077cc9d590ac8`; V1.0, V1.1, and both review histories remain immutable
- Remediates Fresh Re-review findings: remaining original `M-04` plus new `N-M01`, `N-M02`, and `N-M03`; preserves closed `H-01`, `H-02`, `M-01`, `M-02`, `M-03`, `M-05`, `M-06`, and `L-01`
...
> This corrected document is a complete, standalone Design Candidate and Complex Task Analysis only. It does not implement Phase B, approve itself, create a Provider account or credential, call a Provider, use a network, spend money, modify Schema/Migration, deploy, import formal data, Publish, or enable Index. V1.0, V1.1, and the V1.0 Remediation record remain byte-identical. Phase B implementation must not begin until the original independent reviewer completes another Fresh Re-review of the exact V1.2 commit and returns PASS.
...
Phase B exit is met only when every Provider-neutral unit, integration, architecture, Prompt-bundle, raw-parser, Synthetic-extensibility, and public-bundle test in Section 20 passes; `ai_model_config` and `ai_runs` still map exactly `21/21` and `96/96`; V1.0, V1.1, the V1.0 Remediation, and review evidence are byte-identical; the repository contains no real Provider adapter/SDK/credential/network path; and the implementation Candidate later receives its own independent review. No DeepSeek fact or behavior is claimed, and `PD-04` through `PD-07` remain non-blocking reference items rather than reinstated gates.

V1.2 closes four remaining exactness gaps without adding mechanism or state: the Draft application defines a closed durable target union and a target hash reconstructible from accepted columns; resolved configuration uses the exact Phase A hash and separates pre-dispatch requested Provider from post-dispatch actual Provider; availability and governed enqueue share one assignable read capability; and automatic output checks are finite structural/provenance checks that never claim English entailment. Semantic fidelity and factual correctness remain explicitly `human_review_required` under PD-11 and the existing Draft review boundary.

## 2. Fixed authority and verified remediation identity

The mandatory pre-edit gate for this V1.2 Candidate is:

| Check | Verified identity | Result |
|---|---|---|
| Design branch | `codex/phase-1b-stage4a-phase-b-design-v1` | PASS |
| Exact remediation HEAD | `3be7cda919cd8d5418d31e81499077cc9d590ac8` | PASS |
| Direct parent | `de0c3e4ec63331837baefd1ff755ecd0ffb9d46f` | PASS |
...
The worktree was attached to the required existing design branch and clean before V1.2 was added. These fixed Fresh Re-review inputs were read completely and recomputed before revision:

| Remediation input | Exact SHA-256 |
|---|---|
| Fresh Re-review report V1.0 | `856e79f1652806f73a1327551b6ced7bf2b86c846225c409842500f676ca4841` |
| Fresh Re-review Evidence V1.0 | `678f228d3a06c269c2883860b5bc8e043a77e9d8b53d24bd927f17368bc99903` |
| Fresh Re-review `SHA256SUMS.txt` | `653d72b5132d453780949779294dc67891a43dbc177cef990320a43183edcba7` |
...
Fresh Re-review disposition was Blocker `0`, High `0`, Medium `4`, Low `0`; implementation remained **NOT ELIGIBLE**. It closed `H-01`, `H-02`, `M-01`, `M-02`, `M-03`, `M-05`, `M-06`, and `L-01`; those contracts are retained in full below. Only `M-04`, `N-M01`, `N-M02`, and `N-M03` are corrected. The reviewer's disclosed package-registry access is review-history evidence only: it did not alter the Candidate or establish a Candidate defect, and this design task performs no package install/download or registry access.
...
| `docs/PRODUCT_DATA_DICTIONARY.md`, Stage 4 plan §5.3 | Product Code is hidden/internal and Product Description output forbids Product Code/factual-field mutation. V1.2 excludes it from Provider context and all candidates. |
...
Fresh Re-review confirmed eight original findings closed but found four final contract seams. The root cause is not missing storage: it is that V1.1 still named values broader than accepted authorities could reproduce. V1.2 therefore narrows the target snapshot to stored identity/version, restores the accepted Phase A config object, makes read scope capability-based and assignable, and stops calling provenance refs semantic proof. Adding columns, a second snapshot store, a parallel read repository, or a language classifier would layer complexity over those authority mistakes and is expressly rejected.
     ...
     -> src/ai/applications/draft-assistance/facade.ts
        ...
        -> GenericAiOrchestrator.inspect | request
           -> DraftConsistentReadScope
              -> ReadOnlyDraftAvailabilityScope (one disclosed read-only snapshot)
              -> TransactionBoundDraftEnqueueScope (Phase C; adds replay/locks/insert/Audit)
...
| `src/ai/applications/draft-assistance/policies.ts` | Draft target authorization/context/result/disposition policy bundle and registration of the exact `0020` association persistence codec. Later Phase E supplies narrow Product/Content readers. |
...
| `src/ai/internal/claimed-run-authority.ts` | Strict branded constructor from the raw accepted row projection. It invokes the registered application persistence codec, forbids caller association/snapshot/request injection, and requires post-dispatch actual Provider/timestamps/lease/state; core then performs provenance reconstruction. |
...
| `test-fixtures/ai-types/read-scope/` | Positive and expected-compile-failure fixtures proving read-only/transaction scope assignability and forbidden lock/mutation/connection capabilities without casts or suppression comments. |
...
| `next.config.ts` | No Production route or AI integration. The test-only bundle fixture has its own standalone config; Production config changes only if later raw-file tracing replaces the accepted static-byte bundle, which V1.2 does not choose. |
...
export interface ApplicationAssociationPolicy<
  TCommand,
  TAssociation,
  TReadScope extends ApplicationReadScope,
> {
...
> {
  ...
  readonly associationPolicy: ApplicationAssociationPolicy<
    TCommand,
    TAssociation,
    TReadScope
  >;
...
> extends DraftConsistentReadScope<TQueryResult> {
  ...
  lockTargetForNewRequest(input: DraftDurableAssociationWithoutHashV1):
...
`withReadOnlyDraftAvailabilityScope` opens exactly one disclosed database transaction with `READ ONLY` and `REPEATABLE READ`, creates `ReadOnlyDraftAvailabilityScope`, invokes the availability sequence once, and closes the scope. It writes no row and exposes no transaction/mutation member. `DraftAiRunEnqueuePort.withGovernedTransaction` later creates the transaction subtype around Phase C's one governed enqueue transaction. Both subtypes are assignable to `DraftConsistentReadScope`, so target, context, feature, and configuration read repositories accept the same first parameter. Only the transaction subtype contains replay, selected-target/config locking, insert, and required-Audit abilities.

Every Production Draft `AiApplicationDefinition` binds its `TReadScope` parameter to `DraftConsistentReadScope<TQueryResult>`. Availability supplies the read-only subtype; request supplies the transaction subtype. Registry erasure closes over the already typed policy invocation and exposes no untyped scope cast to core.

The scope factory projects the current database/transaction object to `Pick<..., "select">` under the private symbol. `withDraftReadExecutor` and the scope expire with the callback. Phase B's Production import allowlist contains exactly `config/feature-gate-repository.ts` and `config/model-config-repository.ts`; target/context are typed ports only until Phase E supplies separately reviewed Domain adapters. Test-only target/context repository fixtures may import the helper under `src/ai/testing/**`. Phase E must add exact adapter paths to this allowlist rather than opening another database. An AST gate rejects `.for("update")`, raw locking SQL, `insert`, `update`, `delete`, `execute`, or `transaction` inside every allowed read callback. The only selected-row locks are the two named transaction methods above. There is no cast, duplicate repository, independently opened config transaction, or parallel database connection.
...
The generic claimed constructor receives the exact selected `ai_runs` row. After registry lookup it passes only those seven physical columns to the Draft application's registered persistence codec, which performs the decoder above and returns the opaque application association closure to core. Thus `core/**` remains application-neutral and never contains the Draft union, while no Worker can provide an unstored association snapshot. A future application registers its own persistence codec only after its separately reviewed forward Schema; Synthetic tests never use this `0020` codec.
...
The generic request orchestrator owns the callback ordering: authorize/context/fingerprint, `findReplay`, and only for a new request feature/config/Prompt preparation followed by `lockSelectedConfigForNewRequest` and `insertPreparedWithRequiredAudit`. The Phase C scope owns scoped run lookup, the unique-conflict fetch, target/config row locks, insert/Audit atomics, and transaction lifetime; it cannot call an adapter. Target/context/feature/config reads use the inherited common read capability. This split prevents a port from resolving config before replay and prevents core from issuing run SQL.
...
The internal core-owned constructor accepts the raw exact Phase C database row projection, not a caller-built `durableAssociation`. It strict-parses common columns, resolves the application entry, and invokes that entry's persistence decoder. For current Draft runs the decoder reads only `target_type`, all three nullable target FKs, `target_locale`, `expected_target_version`, and `target_snapshot_hash`, then performs the closed reconstruction above. The constructor resolves `outputSchemaId` only from durable `(application_class,use_case,output_schema_version)` through the immutable registry, verifies all required fields/non-null lease and post-dispatch state, deep-copies JSON values, deep-freezes the projection, and creates the brand. The Worker cannot nominate a schema ID, association JSON, snapshot JSON, actual Provider, or rendered request independently of the row. The brand proves only that the constructor ran. The durable row plus registered application definition, exact Prompt bytes, JCS hashes, envelope/policy identities, and lease/state checks are authority.
...
All four current Draft read ports accept the same common scope; none accepts a mutation-only scope or opens a connection itself:

```ts
interface DraftTargetReadRepository {
  authorizeAndReadTarget<TQueryResult extends PgQueryResultHKT>(
    scope: DraftConsistentReadScope<TQueryResult>,
...
The materialized scoped relation and aggregate are one statement/snapshot. In Phase B availability it runs in `ReadOnlyDraftAvailabilityScope`; in Phase C new-request preparation it receives the assignable `TransactionBoundDraftEnqueueScope`. Only after resolution succeeds does the orchestrator call that transaction scope's `lockSelectedConfigForNewRequest`; the repository itself cannot lock. No result is cached across requests. Immutable Prompt bytes may be cached by full tuple, but active configuration facts may not.
...
Positive fixtures compile calls from all four repositories with both scope subtypes. Separate negative `tsc --noEmit` fixtures intentionally attempt availability insert/Audit/lock, direct common-scope select/row-lock/mutation/raw execute/transaction, and construction without both private brands; each fixture must fail compilation. An AST assertion proves the four allowlisted internal read callbacks contain no cast (`as`, angle-bracket assertion, `any`, `unknown` round-trip), parallel DB import/open, row lock, mutation, raw execute, or transaction. Transaction-only fixtures prove replay, both locks, insert, and required Audit are unavailable outside `TransactionBoundDraftEnqueueScope`.
...
`inspectDraftAssistanceAvailability` performs the shared prelude inside the one disclosed `ReadOnlyDraftAvailabilityScope` (`READ ONLY`, `REPEATABLE READ`). Target, context, feature, and config repositories all receive that same scope and therefore the same snapshot. It then:
...
1. application policy rechecks actor/target scope/editability/current expected version through the inherited common read capability, locks the target only through `lockTargetForNewRequest`, and creates Section 8's exact identity/version target snapshot;
...
6. one consistent config resolution read through the inherited common capability, then `lockSelectedConfigForNewRequest` rechecks that sole row; adapter/envelope/Prompt/policy validation follows;
7. build Prompt variables/render only for preflight; persist no rendered request;
8. `insertPreparedWithRequiredAudit` inserts one `pending` row with exact snapshots and required enqueue Audit atomically; `actual_provider`, `provider_dispatched_at`, and `active_attempt_dispatched_at` are all null;
...
6. registry exact tuple; application persistence codec strict-decodes only the seven Draft target columns, reconstructs the closed union and exact snake-case snapshot, and verifies `target_snapshot_hash` using Section 8's literal contract;
...
- all Draft target/context/feature/config reads accept `DraftConsistentReadScope`; read-only and governed-transaction subtypes are assignable; read repositories contain no lock/mutation/raw execute/cast/connection-open path, while replay/target-config locks/insert/Audit occur only on `TransactionBoundDraftEnqueueScope`;
...
- **No implementation leakage from design remediation:** V1.2/Remediation V1.1 commit itself must contain documents only.
...
| common read scopes | both subtype assignability, four repositories with each subtype, conditional-key negatives, expected `tsc` failures, AST no-cast/no-lock/no-mutation/no-new-connection | availability read-only; transaction adds replay/locks/insert/Audit; no cast/parallel repository |
...
Only after V1.2 receives PASS from another Fresh Re-review:

1. **Generic contracts/JCS/application registry:** generic opaque command/association/result interfaces, exact Phase A resolved-config object/vector, RFC 8785 vectors, registry builder; tests prove no Draft import or pre-dispatch actual Provider.
2. **Draft application codecs and exact Production entries:** facade, closed three-kind association/snapshot/column codec with literal round trips, result/disposition policies, exact four keys; distinct Synthetic compile/proof in the same commit or immediately following test commit.
3. **Normative read scopes, context, and Product provenance:** one common scope plus two assignable subtypes, compile/AST negative fixtures, reconstructible context, narrow readers, field matrix, denial/limit tests; no business integration.
...
## 23. Exact V1.2 design acceptance checklist

The Fresh Re-review may return PASS only if all are true:

- [ ] exact V1.2 remediation start, immutable V1.0/V1.1/Remediation hashes, Fresh Re-review report/evidence/manifest hashes, branch/parent/ancestry/tag/acceptance identity pass;
- [ ] V1.2 and Remediation V1.1 are the only added files and all older artifacts are byte-identical;
...
- [ ] one common read capability accepts both read-only and governed-transaction subtypes; all four read repositories compile with both, while locking/mutation/Audit exist only on transaction subtype and no cast/parallel connection exists;
...
V1.0 consolidated Provider mechanics but left Draft association/result concepts in core and several integrity checks implicit. V1.1 correctly moved application semantics outward, but four seams still forced implementer invention: an unstored target snapshot, a non-Phase-A config hash/pre-dispatch actual Provider, incompatible read-scope signatures, and an impossible automatic English-support claim. V1.2 narrows each seam to its existing authority—accepted target columns, exact Phase A snapshot, one read capability, and finite structural checks plus human semantics—without adding a layer or state.
...
- compiled state: generic registry interfaces, four Draft definitions, strict three-kind association codec, common read-scope interfaces, strict source/output codecs with fixed human-review labels, error codes, empty Production Prompt manifest/generated tuple, provider registry;
...
Phase B increases compiled validation/test maintenance but adds no cross-process/persistent coordination. V1.2 adds no new runtime branch beyond strict validation; it removes implementer choice about snapshots, Provider timing, scope construction, and semantic proof. Phase C retains the already approved operational complexity without a fake or application-specific execution path.

## 26. Design conclusion and next gate

Conclusion: **CORRECTED DESIGN CANDIDATE V1.2 / NOT SELF-APPROVED / IMPLEMENTATION NOT YET ELIGIBLE.**

The next and only gate is **another Fresh Re-review by the original independent Phase B Design Reviewer** against the exact new commit, V1.2 SHA, Remediation V1.1 record, immutable V1.1/V1.0/older-remediation hashes, and repository baseline. Phase B implementation must not start until that Fresh Re-review returns PASS. No Provider call, credential, network, Staging/Production, Deploy, formal import, Publish, Index, or Push is authorized or performed by this design.
