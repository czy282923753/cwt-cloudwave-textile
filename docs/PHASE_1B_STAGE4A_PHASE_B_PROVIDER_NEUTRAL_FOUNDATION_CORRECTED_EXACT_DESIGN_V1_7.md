# CWT Stage 4A Phase B — Provider-neutral Foundation Corrected Exact Design V1.7

- Status: **CORRECTED EXACT DESIGN CANDIDATE V1.7 / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED**
- Design version: `1.7`
- Standalone basis: the complete V1.6 contract, which reproduces V1.5 and accepted Design V1.4, is reproduced here with the bounded lifecycle correction; an implementer does not combine older designs with remediation notes to recover the normative design
...
- V1.7 remediation scope: same-root `V15-M01` Remediation Attempt 2 only; it closes the normal Next-generated `next-env.d.ts` lifecycle hole without changing the 12-class selector, selected M02 grammar, or selected M03 discriminated database seam
...
V1.7 retains V1.4's complete independently accepted dual authorization/binder and construction design, V1.5's Owner-selected M02/M03 seams, and V1.6's unique classification of the original six executables. Generic `ApplicationReadScope` remains an unbranded structural constraint containing only `readonly mode: string`; it owns no constructor, factory, database, lock, transaction, or authority. Draft and Synthetic each own their module-private nominal brand, private executor capability, direct object-literal factories, and callback lifetime. Factory-returned values bind to the same opaque staged closures before generic core entry. The mode string alone cannot enter a Draft binder because the Draft-private carrier is absent. Core still receives no application scope, target, repository, factory, or lock method. V1.7 changes only the same-root actual-filesystem lifecycle proof: `next-env.d.ts` is absent in a source-clean tree and becomes one ignored, generated, uniquely classified root-control Candidate after the normal installed Next type-generation path.

## 2. Fixed authority and verified remediation identity

The mandatory pre-edit gate for this V1.7 Candidate is:
...
- the owned-file whitespace check and design/implementation scope gate show no historical Prompt mutation and a documentation/evidence-only V1.7 delta; immutable imported Reviewer hard-break bytes are reported separately rather than rewritten or misreported.
...
Only after the exact V1.7 commit receives Fresh Independent Design Review PASS and the Coordinator opens a new implementation task may the following implementation order be used:
...
## 23. Exact V1.7 design acceptance checklist

The Fresh Independent Design Review may return PASS only if all are true:

- [ ] exact V1.7 branch/parent/ancestry/tag identity, accepted V1.4/Remediation V1.3 hashes, V1.1 escalation hashes, Fresh Technical Escalation PASS report/evidence/5-item manifest and Owner Selection Record pass;
- [ ] accepted V1.4, escalation V1.1 and imported PASS artifacts are byte-identical; the V1.7 delta is exact docs/evidence-only scope;
- [ ] V1.7 is standalone and the machine contract map accounts for every preserved `H-01`, `H-02`, `M-01..M-06`, `L-01`, and `N-M01..N-M04` closure;
...
- [ ] selected M03 authority is the V2.2 profile sealed in the V1.7 manifest; the unchanged 12 classes, closed selector compiler, complete protected/excluded roots, exact five-edge Phase B root, reserved absent Phase D root/adapter zone and empty Production Provider registry pass;
...
Maintainers must keep application codec/policy/schema versions, both binders, and same-module scope factories aligned; retain the three target/hash and config/dispatch vectors; preserve construction/assignability/negative fixtures; maintain the finite A-01–A-10 lexicon and human-review boundary; append Prompt manifest/resources and regenerate the checked derivative; maintain RFC/raw-parser vectors; and version the one M03 selector profile whenever a genuinely new executable root/control/test semantic appears. The raw actual-tree gate makes that review visible instead of silently broadening a class. This cost is bounded and application-local. A new use case reuses its application's factories and adds one definition; a new application supplies its own private scope module, typed registry/binders/codecs, and forward persistence support; a Provider adds one adapter/evidence set. None requires adding a generic base factory or core switch.
...
Phase B increases compiled validation/test maintenance but adds no cross-process/persistent coordination. V1.3's two explicit entry compositions remain; V1.4 deleted one unusable generic brand and gave each application one direct construction seam. V1.5 selected one compiled classifier authority and one exhaustive existing-union switch. V1.6 retained the 12-class selector; V1.7 adds one exact member to its existing root-control class plus a two-state offline proof, not a classifier, wildcard, cleanup state or runtime branch. It adds no runtime choice, database, wrapper, cache, Provider, adapter, dependency or state authority. Phase C retains the already approved operational complexity without a fake, duplicate authorization, generic factory hierarchy, or application-specific core path.

## 26. Design conclusion and next gate

Conclusion: **CORRECTED EXACT DESIGN CANDIDATE V1.7 / OWNER SELECTIONS INCORPORATED / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED.**

The next and only gate is **Fresh Independent Design Review by the original Phase B Design Reviewer** against the exact V1.7 commit, Owner Selection Record, fixed V1.4/V1.5/V1.6 identities, both byte-identical FAIL packages, selected M02 and corrected M03 V2.2 profiles, two-state official-generation proof, machine verifier/mutations/type probes and actual-tree inventories. Phase B implementation must not start until that review returns PASS and a later task explicitly opens a new implementation cycle. If the same root remains open after that Fresh Review, only Attempt 3 remains; no compatibility layer or second selector is permitted. No Provider call, credential, network, Staging/Production, Deploy, formal import, Publish, Index, merge or Push is authorized or performed by this design.
