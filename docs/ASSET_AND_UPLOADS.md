...
Failed or recovered Batches remain explicit and retryable until a new authorized Finalize claim; dead work remains failed for investigation; successful Batches remain completed. Asset Library lists only the current actor and Auth Session's eligible pre-Manifest handoffs after the Upload Domain Service revalidates the authorized failure reason, retryable Recovery, unconsumed passed Intents, Private/Internal passed Assets, completed Staging Recoveries, absence of Manifest/Public compensation, unexpired state, available association targets and existing Private objects. The operator retries the original `batchId` through the existing Finalize entry point; no Intent, Asset, relation path or upload is recreated. The UI reports actionable pending/success/failure results without exposing Lease, Recovery or Manifest concepts.
