# CWT Stage 4A Phase B — Provider-neutral Foundation Corrected Exact Design V1.6

- Status: **CORRECTED EXACT DESIGN CANDIDATE V1.6 / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED**
- Design version: `1.6`
- Standalone basis: the complete V1.5 contract, which itself reproduces accepted Design V1.4, is reproduced here; an implementer does not combine older designs with remediation notes to recover the normative design
...
- V1.6 remediation scope: closes only `V15-M01` in the complete-graph inventory/classification proof; the selected M02 grammar and M03 discriminated database seam are unchanged
...
- Exact V1.6 authoring parent after byte-identical FAIL import: `b55fe1a7bcda91bce685392f2b4092e226bf560f`
...
V1.6 retains V1.4's complete independently accepted dual authorization/binder and construction design and V1.5's Owner-selected M02 and M03 seams. Generic `ApplicationReadScope` remains an unbranded structural constraint containing only `readonly mode: string`; it owns no constructor, factory, database, lock, transaction, or authority. Draft and Synthetic each own their module-private nominal brand, private executor capability, direct object-literal factories, and callback lifetime. Factory-returned values bind to the same opaque staged closures before generic core entry. The mode string alone cannot enter a Draft binder because the Draft-private carrier is absent. Core still receives no application scope, target, repository, factory, or lock method. V1.6 changes only the causal complete-tree classification contract and proof that V1.5 left six executable nodes unclassified.

## 2. Fixed authority and verified remediation identity

The mandatory pre-edit gate for this V1.6 Candidate is:

| Check | Verified identity | Result |
|---|---|---|
| V1.6 authoring parent | `b55fe1a7bcda91bce685392f2b4092e226bf560f` | PASS |
...
The table below is retained as the complete V1.4 historical design lineage that V1.6 preserves:
...
The historical V1.4 worktree was attached to its required design branch and clean before V1.4 was added. These fixed Fresh Re-review inputs, including every manifest-protected positive/negative probe, remain part of the accepted V1.4 proof inherited by V1.6:
...
V1.6 preserves every V1.5 contract and corrects only this exact proof defect:

| Proof boundary | V1.5 retained contract | V1.6 correction |
|---|---|---|
...
| `V15-M01` complete graph | exactly 12 classes and zero/overlap fail closed, but six accepted executables had zero matches | retain the same 12 classes and guarantee; replace precedence guessing with a closed, mutually exclusive selector AST plus raw actual-filesystem proof and required mutations |
...
Candidate executable suffixes are exactly `.ts`, `.tsx`, `.mts`, `.cts`, `.js`, `.jsx`, `.mjs`, and `.cjs`. Candidate protected resources are `.json` and `.md` only below `src/ai/prompts/resources/`, `src/ai/prompts/generated/`, `test-fixtures/ai-architecture/`, `test-fixtures/ai-server-bundle/`, `test-fixtures/ai-types/`, or `docs/review-evidence/`. The nine exact non-executable/control candidates are `package.json`, `pnpm-lock.yaml`, `tsconfig.json`, `next.config.ts`, `eslint.config.mjs`, `postcss.config.mjs`, `drizzle.config.ts`, `playwright.config.ts`, and `vitest.config.mts`. A new root executable or config-like name is not swallowed by `*.config.*`; it remains unclassified until the reviewed profile is versioned.

Tracked, untracked and ignored status is computed after inclusion and cannot narrow scope. Generated status also cannot narrow scope: every executable/resource below `src/ai/prompts/generated/` or carrying a declared `.generated.<executable>` suffix must be the exact protected `production-prompt-bundle.generated.ts`; any other generated-marked path fails closed. Symlink, alias, unmanifested `new URL`, resource or generated entry remains a failure even if its target bytes duplicate an allowed file.
...
`src/test/**` is likewise test-only rather than Production source. Root-control and test-only classification never grants Provider/adapter, Provider credential/endpoint, external Provider network, public entry, server entry or Production runtime authority. Existing migration configuration, loopback E2E/browser traffic and synthetic test credentials remain their pre-existing non-Production test/build concerns, not AI authority. Tests may exercise Production modules in isolation; dependency direction from any Production/public/server node into test/config nodes fails closed.

The actual-tree proof prints total candidates, executable count, every class's sorted members including empty arrays, `zeroClass=[]`, `ambiguous=[]`, source-state counts, excluded-root states, protected/test/control/composition/Phase D/adapter status, and three independent SHA-256 domains for full inventory, content and classification. Design-Candidate mode requires Phase B composition/protected implementation absent because implementation is unauthorized; future implementation-gate mode instead requires exactly one declared Phase B root while still requiring Phase D and adapter counts zero. Evidence `.txt` capture and `SHA256SUMS.txt` are envelopes; every executable probe/verifier and `.json`/`.md` evidence profile is itself enumerated and hashed before capture.
...
The graph verifier must also kill, in memory and without changing repository bytes: one new unclassified executable; one path matching two classes; removal of each of the six dispositions above; an added silent physical exclusion; symlink and canonical/hard-link alias cases; an unmanifested generated resource; early Phase D root; early adapter node; and an undeclared second `src/server/ai` composition root. Each mutation must reach its declared fail-closed reason. A verifier that checks only class count, only the six names, only Git-tracked paths, or only a few acquisition spellings is non-conforming.

### 2.4 Phase allocation after selection

| Phase | V1.6 authority |
|---|---|
...
| Phase C | later accepted durable `ai_runs` enqueue/claim/lease/retry/cancel/budget/Audit composition; must consume the V1.6 seams and cannot create a temporary or parallel runtime |
...
- all Production code `src/**/*.{ts,tsx,mts,cts,js,jsx,mjs,cjs}` whose V2.1 class/stage is Production, excluding every declared test semantic and `src/ai/testing/**` from the Production module graph, plus classified JSON/resources in the same inventory;
...
- the exact six `V15-M01` paths have their Section 2.3 sole classes, every `src/test/**` node is test-only, and no test/root-control node is reachable from a Production/public/server entry; test/config classification grants no AI Provider/adapter, external Provider network, credential/endpoint or Production runtime authority;
- the actual-tree proof records sorted members for all 12 classes, empty `zeroClass`/`ambiguous`, physical-exclusion status and inventory/content/classification hashes. Adding an unknown root executable, silently adding an exclusion, using Git ignore to hide a file, or changing a matcher without updating the one sealed V2.1 profile fails before module analysis;
...
- **No implementation leakage from corrected Design:** the V1.6 Candidate delta from `377181cd...` contains only versioned `docs/` and `docs/review-evidence/` artifacts; accepted V1.4, escalation V1.1, imported PASS, source, Schema/Migration, ADR, package and lock bytes remain unchanged.
...
- Before any future implementation proof, run the V1.6 design verifier under exact Node/V8/ICU/Unicode/CLDR tuple. Its raw actual-tree output must list every class member, total executable/candidate counts, empty zero/ambiguous arrays, excluded-root/source-state/composition/Phase D/adapter status and stable inventory/content/classification hashes.
- Run every V15-M01 in-memory mutation: unclassified executable, double match, six independent disposition removals, silent exclusion, symlink, canonical/hard-link alias, unmanifested generated resource, early Phase D, early adapter and second composition root. Every case must fail with its exact reason; changing a priority or Git ignore state cannot make a failure pass.
- Re-run M02's full Unicode transition challenge and runtime mismatch negative, the selected database branch-local positive plus unnarrowed-union/cross-driver negatives, the exact 21/21 + 96/96 Schema map, all 13 historical closures and V1.5 byte-identity checks.
...
- the owned-file whitespace check and design/implementation scope gate show no historical Prompt mutation and a documentation/evidence-only V1.6 delta; immutable imported Reviewer hard-break bytes are reported separately rather than rewritten or misreported.
...
Only after the exact V1.6 commit receives Fresh Independent Design Review PASS and the Coordinator opens a new implementation task may the following implementation order be used:
...
## 23. Exact V1.6 design acceptance checklist

The Fresh Independent Design Review may return PASS only if all are true:

- [ ] exact V1.6 branch/parent/ancestry/tag identity, accepted V1.4/Remediation V1.3 hashes, V1.1 escalation hashes, Fresh Technical Escalation PASS report/evidence/5-item manifest and Owner Selection Record pass;
- [ ] accepted V1.4, escalation V1.1 and imported PASS artifacts are byte-identical; the V1.6 delta is exact docs/evidence-only scope;
- [ ] V1.6 is standalone and the machine contract map accounts for every preserved `H-01`, `H-02`, `M-01..M-06`, `L-01`, and `N-M01..N-M04` closure;
...
- [ ] selected M03 authority is the V2.1 profile sealed in the V1.6 manifest; the unchanged 12 classes, closed selector compiler, complete protected/excluded roots, exact five-edge Phase B root, reserved absent Phase D root/adapter zone and empty Production Provider registry pass;
...
- [ ] the three root configurations classify only as `root-control-file`; all three `tests/e2e` paths and every `src/test/**` node classify only as `other-test-fixtures`; test/control classes grant no Provider/adapter/external Provider network/credential/Production runtime authority and are unreachable from Production/public/server entries;
- [ ] all required V15-M01 mutation negatives fail closed, including each of the six independent classification removals, silent exclusion, alias/symlink/generated bypass, early Phase D/adapter and second composition root;
...
Phase B increases compiled validation/test maintenance but adds no cross-process/persistent coordination. V1.3's two explicit entry compositions remain; V1.4 deleted one unusable generic brand and gave each application one direct construction seam. V1.5 selected one compiled classifier authority and one exhaustive existing-union switch. V1.6 adds only the bounded static selector/inventory proof required to make that graph total; it adds no runtime choice, database, wrapper, cache, Provider, adapter, dependency or state authority. Phase C retains the already approved operational complexity without a fake, duplicate authorization, generic factory hierarchy, or application-specific core path.

## 26. Design conclusion and next gate

Conclusion: **CORRECTED EXACT DESIGN CANDIDATE V1.6 / OWNER SELECTIONS INCORPORATED / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED.**

The next and only gate is **Fresh Independent Design Review by the original Phase B Design Reviewer** against the exact V1.6 commit, Owner Selection Record, fixed V1.4/V1.5 identities, byte-identical V1.5 FAIL, selected M02 and corrected M03 V2.1 profiles, machine verifier/mutations/type probes and actual-tree inventory. Phase B implementation must not start until that review returns PASS and a later task explicitly opens a new implementation cycle. No Provider call, credential, network, Staging/Production, Deploy, formal import, Publish, Index, merge or Push is authorized or performed by this design.
