# CWT Stage 4A Phase B — Provider-neutral Foundation Corrected Exact Design V1.9

- Status: **CORRECTED EXACT DESIGN CANDIDATE V1.9 / V18-M01–M03 REMEDIATED / OWNER IMP2-NM01 DIRECTION PRESERVED / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED**
- Design version: `1.9`
- Standalone basis: the complete V1.8 contract is reproduced here with only the bounded V18-M01/M02/M03 taxonomy, claimed-replay-order, and proof-authority correction; an implementer does not combine V1.8 with remediation notes to recover the normative design
...
- V1.9 correction scope: `V18-M01`, `V18-M02`, and `V18-M03` only; map traversal cardinality failures to the existing closed `context_provenance_mismatch`, make one machine sequence the source for every claimed-replay rendering, and bind all recursive traversal/proof mutations to one compiled profile identity while preserving the accepted context shape/bytes, selected M02 grammar, M03 discriminated seam, V1.7 lifecycle graph, and every non-finding V1.8 clause
- Prepared: `2026-08-11` (Asia/Shanghai)
- Corrected Design branch: `codex/phase-1b-stage4a-phase-b-corrected-design-v1-9`
- Exact V1.9 parent / immutable V1.8 Candidate: `a2cd31f53c34dad479862a1c67260ab71fda9805`
...
V1.9 retains V1.4's complete independently accepted dual authorization/binder and construction design, V1.5's Owner-selected M02/M03 seams, V1.6's unique classification of the original six executables, V1.7's two-state `next-env.d.ts` lifecycle proof, and V1.8's Owner-selected typed context-domain direction. Generic `ApplicationReadScope` remains an unbranded structural constraint containing only `readonly mode: string`; it owns no constructor, factory, database, lock, transaction, or authority. Draft and Synthetic each own their module-private nominal brand, private executor capability, direct object-literal factories, and callback lifetime. Factory-returned values bind to the same opaque staged closures before generic core entry. The mode string alone cannot enter a Draft binder because the Draft-private carrier is absent. Core still receives no application scope, target, repository, factory, or lock method. V1.9 changes only design-package exactness: traversal errors use the existing closed error code, one machine order controls claimed replay, and one compiled profile identity makes every demotion or reclassification fail before recursive validation. Every human/business/Provider-evidence surface still uses the exact selected M02 classifier identity.

## 2. Fixed authority and verified remediation identity

The V1.9 remediation identity is closed before any normative contract below is considered:

| Check | Verified identity | Required result |
|---|---|---|
| exact V1.8 Candidate / V1.9 parent | `a2cd31f53c34dad479862a1c67260ab71fda9805` | PASS; V1.8 ref is not moved |
| V1.8 direct parent | `8eac88210d41b5e37ab5963acc7ee28d007c4fd3` | PASS |
...
| immutable checkpoint ref / target | `codex/checkpoint/phase-1b-stage4a-phase-b-nm01-pre-design-v1` / `b7ad96b24da45de00cae2cdb961a9aefcbc99496` | PASS before and after V1.9; never move/rebuild |
...
| V1.9 branch | `codex/phase-1b-stage4a-phase-b-corrected-design-v1-9` | dedicated linear docs/evidence-only branch |

Rollback of the V1.9 remediation is its exact parent `a2cd31f...`; rollback of the full IMP2-NM01 design chain remains the immutable checkpoint `b7ad96b...`; accepted V1.7 remains the full design rollback `3f475e13...`. No Tag, source, test, fixture, Schema, Migration, snapshot, journal, seed, ADR, dependency, package, lockfile, Prompt runtime, Provider or environment is changed.
...
The sole machine-readable context-domain, closed error projection, recursive evidence grammar, and claimed-replay-order authority is [V18 M01/M02/M03 Machine Profile V1.0](review-evidence/phase-1b-stage4a-phase-b-corrected-exact-design-v1-9-v18-remediation-v1/V18_M01_M02_M03_MACHINE_PROFILE_V1_0.json), profile `cwt.phase1b.stage4a.phaseb.corrected-design.v1_9.v18-remediation.v1`. Its one traversal ID remains `ctx.draft-assistance.v1.field-domain-traversal.v1`. The implementation compiles that closed typed profile into the one context validator; prose, a consumer path list, a second visitor, a copied order array, or a second error map cannot select a domain, error, or replay step.
...
2. `CR-05`: compile the exact profile identity projection and require SHA-256 `48d2b635a4719a20c2f3210e81e8bb4512745c0351a5bb8a7e66201b5f099026`, the exact 35 assignment records, three domains, recursive grammar, selected M02 identity, closed 69-code taxonomy, and 14-step order. Any demotion/reclassification/table edit fails `context_provenance_mismatch` before payload traversal.
...
<!-- V1_9_CLAIMED_REPLAY_ORDER: CR-01,CR-02,CR-03,CR-04,CR-05,CR-06,CR-07,CR-08,CR-09,CR-10,CR-11,CR-12,CR-13,CR-14 -->
...
The design-level type proof imports the JSON profile, derives `ProfileAiErrorCode` from `errorTaxonomy.codes[number].code`, asserts both `Exclude<TraversalReturnCode,ProfileAiErrorCode>` and its inverse for the selected traversal outputs are `never`, compares all 69 entries to `aiErrorCodes`/`aiFailure`, and proves the unknown catch arm returns only `internal_failure`. The offline verifier rejects a profile containing an unknown return code, a duplicate/missing tuple member, category/boolean drift, or a second mapping.
...
| IMP2-NM01 field-domain traversal | materialize every required/optional/container and recursively nested object/array/key/value node; remove/duplicate each assignment; add unknown/overlap; demote any protected assignment to either non-protected domain | one compiled profile identity/interpreter; every node exactly one domain; zero/multiple and profile drift return only `context_provenance_mismatch`; every demotion actually rejects before payload validation and cannot be counted without a caught failure |
...
- Run `VERIFY_CORRECTED_EXACT_DESIGN_V1_9.mjs` offline under the exact selected runtime tuple. It must interpret one profile, verify its compiled identity, prove the full 69-code error projection, extract the same `CR-01..CR-14` order from §13.2.3/§18.4/§18.5, recursively enumerate nested object/array evidence, report zero missing/duplicate nodes, accept 1,000/1,000 deterministic valid Revision hashes, accept the Reviewer hash only on recomputed equality, and reject every integrity/domain/demotion/protected/machine/order/projection mutation through an executed assertion. The parent process must run a child self-test that deliberately restores the V1.8 false demotion counter and require that child to exit nonzero; a zero child exit fails the parent. Persisted bytes/JCS/`input_hash`, no association leak, one selected M02 identity, M03, and `21/21`/`96/96` remain non-regressed. Two fresh normal outputs must be byte-identical. This is design-consistency proof, not an implementation claim.
...
- the owned-file whitespace check and design scope gate show no historical Prompt mutation, preserve all V1.8 bytes, and require the V1.9 delta itself to contain docs/evidence only; immutable imported Reviewer hard-break bytes are reported separately rather than rewritten or misreported.
...
Only after the exact V1.9 commit receives Fresh Independent Design Review PASS and the Coordinator explicitly opens implementation correction attempt 3 may the following implementation order be used:
...
## 23. Exact V1.9 design acceptance checklist

The Fresh Independent Design Review may return PASS only if all are true:

- [ ] exact V1.9 branch/parent/V1.8 ancestry/tag identity passes; checkpoint remains pinned to `b7ad96b...` before and after design; formal worktree is clean;
...
- [ ] accepted V1.7 and immutable V1.8 design/profile/vectors/verifier/capture/manifest remain byte-identical; V1.9 is standalone and the delta is exact docs/evidence-only scope;
- [ ] V1.9 accounts for every preserved `H-01`, `H-02`, `M-01..M-06`, `L-01`, and `N-M01..N-M04` closure plus exact IMP2-NH01/M01/M02/M03/M04/M05/L01/L02/NM01 review dispositions without implementation claims;
- [ ] V18-M01 is closed by one 69-member `AiErrorCode` tuple/projection; zero/multiple/profile-identity failures use only `context_provenance_mismatch`, all categories/retry/manual/telemetry/persistence/Phase C/precedence projections are exact, and unknown alone maps `internal_failure`;
...
- [ ] V18-M03 is closed by actual caught rejection for every mutation, a compiled profile identity gate before traversal, complete recursive object/array/key/value coverage, and a child old-bug self-test whose intentional regression exits nonzero;
...
Phase B increases compiled validation/test maintenance but adds no cross-process/persistent coordination. V1.3's two explicit entry compositions remain; V1.4 deleted one unusable generic brand and gave each application one direct construction seam. V1.5 selected one compiled classifier authority and one exhaustive existing-union switch. V1.6 retained the 12-class selector; V1.7 added one exact member to its existing root-control class plus a two-state offline proof. V1.8 replaced one overly broad traversal contract with one typed domain traversal. V1.9 replaces only inconsistent documentation/proof projections with one versioned profile identity; it adds no classifier, error map, copied order, exception path, runtime choice, database, wrapper, cache, Provider, adapter, dependency, persistent state, Schema, Migration or ADR. Phase C retains the already approved operational complexity without a fake, duplicate authorization, generic factory hierarchy or application-specific core path.

## 26. Design conclusion and next gate

Conclusion: **CORRECTED EXACT DESIGN CANDIDATE V1.9 / V18-M01–M03 REMEDIATED / OWNER IMP2-NM01 DIRECTION PRESERVED / NOT SELF-APPROVED / IMPLEMENTATION NOT AUTHORIZED.**

The next and only gate is **Fresh Independent Corrected Design V1.9 Review by the original Phase B Design Reviewer** against the exact V1.9 commit, imported byte-identical V1.8 FAIL package, immutable checkpoint, IMP2-NM01 Owner Decision Record, fixed V1.7/V1.8 authorities, selected M02 and unchanged M03 V2.2 profile, the single V1.9 machine profile, recursive verifier/mutations, type proof and captured output. Implementation correction attempt 3 must not start until that review returns PASS and a later task explicitly opens it. No self-review, Provider call, credential, network, Staging/Production, Deploy, formal import, Publish, Index, merge or Push is authorized or performed by this design.
